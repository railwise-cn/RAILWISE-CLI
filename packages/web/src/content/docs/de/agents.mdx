...
Es gibt zwei Arten von Agenten in RAILWISE (甬算); Primäragenten und Subagenten.
...
RAILWISE (甬算) verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Wenn keine Temperatur angegeben ist, verwendet RAILWISE (甬算) modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale RAILWISE (甬算)-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer RAILWISE (甬算)-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [RAILWISE (甬算) Zen](/docs/zen) verwenden, würden Sie `railwise/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
