...
Aby debugować problemy z yonsoon, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\yonsoon\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `yonsoon --log-level DEBUG`.

---

## Przechowywanie danych

yonsoon przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/yonsoon/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\yonsoon`
...
yonsoon Desktop uruchamia w tle lokalny serwer yonsoon (sidecar `yonsoon-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `YONSOON (甬算)` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/yonsoon/yonsoon.jsonc` (lub `~/.config/yonsoon/yonsoon.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/yonsoon/yonsoon.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\yonsoon\yonsoon.jsonc`
...
yonsoon może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/yonsoon/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\yonsoon\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby yonsoon mógł ją odbudować.

1. Całkowicie zamknij yonsoon Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/yonsoon`
- **Linux**: usuń `~/.cache/yonsoon` (lub uruchom `rm -rf ~/.cache/yonsoon`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\yonsoon`

3. Uruchom ponownie yonsoon Desktop.

---

### Napraw problemy z połączeniem z serwerem

yonsoon Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `yonsoon.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `YONSOON_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `YONSOON_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows yonsoon Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami yonsoon.

---

### Brak powiadomień

yonsoon Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla yonsoon w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie yonsoon Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji yonsoon Desktop):

- `yonsoon.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `yonsoon.global.dat` i `yonsoon.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
3. Upewnij się, że masz najnowszą wersję z `yonsoon upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `yonsoon models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\yonsoon`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. yonsoon dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\yonsoon`

2. Uruchom ponownie kod yonsoon, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to yonsoon do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
yonsoon wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
