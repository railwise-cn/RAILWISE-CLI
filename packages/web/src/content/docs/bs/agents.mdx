...
Postoje dvije vrste agenata u YONSOON (甬算): primarni agenti i podagenti.
...
YONSOON (甬算) dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
YONSOON (甬算) dolazi sa dva ugrađena subagenta, **General** i **Explore**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

YONSOON (甬算) dolazi sa dva ugrađena primarna agenta i dva ugrađena subagenta.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `yonsoon.json`:
...
- Globalno: `~/.config/yonsoon/agents/`
- Po projektu: `.yonsoon/agents/`
...
Ako temperatura nije navedena, YONSOON (甬算) koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu YONSOON (甬算) konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj YONSOON (甬算) konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [YONSOON (甬算) Zen](/docs/zen), koristili biste `yonsoon/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `yonsoon models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/anomalyco/yonsoon).
:::
