...
Erstellen Sie `.railwise/commands/test.md`:

```md title=".railwise/commands/test.md"
---
...
Sie können benutzerdefinierte Befehle über die RAILWISE (甬算)-Konfiguration hinzufügen oder indem Sie Markdown-Dateien im Verzeichnis `commands/` erstellen.

---

### JSON

Verwenden Sie die Option `command` in Ihrem RAILWISE (甬算) [config](/docs/config):

```json title="railwise.jsonc" {4-12}
{
...
- Global: `~/.config/railwise/commands/`
- Pro Projekt: `.railwise/commands/`

```markdown title="~/.config/railwise/commands/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".railwise/commands/component.md"
---
...
```md title=".railwise/commands/create-file.md"
---
...
```md title=".railwise/commands/analyze-coverage.md"
---
...
```md title=".railwise/commands/review-changes.md"
---
...
```md title=".railwise/commands/review-component.md"
---
...
RAILWISE (甬算) enthält mehrere integrierte Befehle wie `/init`, `/undo`, `/redo`, `/share`, `/help`; [mehr erfahren](/docs/tui#commands).
