---
title: TUI
description: Utilisation de l’interface utilisateur du terminal YONSOON (甬算).
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

YONSOON (甬算) fournit une interface de terminal interactif ou TUI pour travailler sur vos projets avec un LLM.

L’exécution de YONSOON (甬算) démarre le TUI pour le répertoire actuel.
...
Lorsque vous utilisez le YONSOON (甬算) TUI, vous pouvez taper `/` suivi d'un nom de commande pour exécuter rapidement des actions. Par exemple:
...
Ajoutez un fournisseur à YONSOON (甬算). Vous permet de sélectionner parmi les fournisseurs disponibles et d’ajouter leurs clés API.
...
Quittez YONSOON (甬算). *Alias* : `/quit`, `/q`
...
Vous pouvez personnaliser le comportement de TUI via votre fichier de configuration YONSOON (甬算).
