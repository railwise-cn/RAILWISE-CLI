...
يوجد نوعان من الوكلاء في YONSOON (甬算): وكلاء أساسيون ووكلاء فرعيون.
...
يأتي YONSOON (甬算) مع وكيلين أساسيين مدمجين: **Build** و **Plan**. سنلقي نظرة عليهما أدناه.
...
يأتي YONSOON (甬算) مع وكيلين فرعيين مدمجين: **General** و **Explore**. سنلقي نظرة على ذلك أدناه.

---

## المدمجة

يأتي YONSOON (甬算) مع وكيلين أساسيين مدمجين ووكيلين فرعيين مدمجين.
...
هيّئ الوكلاء في ملف إعدادات `yonsoon.json`:
...
- عام: `~/.config/yonsoon/agents/`
- لكل مشروع: `.yonsoon/agents/`
...
إذا لم يتم تحديد temperature، يستخدم YONSOON (甬算) القيم الافتراضية الخاصة بالنموذج؛ عادة 0 لمعظم النماذج و 0.55 لنماذج Qwen.
...
هذا المسار نسبي لمكان وجود ملف الإعدادات. لذلك يعمل هذا لكل من إعدادات YONSOON (甬算) العامة وإعدادات المشروع.
...
يستخدم معرّف النموذج في إعدادات YONSOON (甬算) الصيغة `provider/model-id`. على سبيل المثال، إذا كنت تستخدم [YONSOON (甬算) Zen](/docs/zen)، فستستخدم `yonsoon/gpt-5.1-codex` لـ GPT 5.1 Codex.
...
:::tip
شغّل `yonsoon models` لعرض قائمة بالنماذج المتاحة.
:::
...
:::tip
هل لديك وكيل تود مشاركته؟ [قدّم PR](https://github.com/anomalyco/yonsoon).
:::
