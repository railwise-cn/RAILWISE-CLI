...
Du kan tilføje eksterne værktøjer til RAILWISE ved hjælp af _Model Context Protocol_ eller MCP. RAILWISE understøtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i din [RAILWISE Config](https://railwise.ai/docs/config/) under `mcp`. Tilføj hver MCP med et unikt navn. Du kan henvise til denne MCP ved navn, når du spørger LLM.
...
RAILWISE håndterer automatisk OAuth-godkendelse for eksterne MCP-servere. Når en server kræver godkendelse, vil RAILWISE:
...
Hvis serveren kræver godkendelse, vil RAILWISE bede dig om at godkende, når du prøver at bruge den første gang. Hvis ikke, kan du [manually trigger the flow](#authenticating) med `railwise mcp auth <server-name>`.
...
Kommandoen `mcp auth` åbner din browser for godkendelse. Når du har godkendt, vil RAILWISE opbevare tokens sikkert i `~/.local/share/railwise/mcp-auth.json`.
...
Dine MCPs er tilgængelige som værktøjer i **TK\_** sammen med indbyggede værktøjer. Så du kan administrere dem gennem RAILWISE-konfigurationen som ethvert andet værktøj.
...
Dette åbner et browservindue for at fuldføre OAuth-flowet og forbinde RAILWISE til din Sentry-konto.
