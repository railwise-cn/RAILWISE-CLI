...
:::caution
Modi werden jetzt über die Option `agent` in der YONSOON (甬算)-Konfiguration konfiguriert. Der
Die Option `mode` ist jetzt veraltet. [Learn more](/docs/agents).
:::

Mit den Modi in YONSOON (甬算) können Sie das Verhalten, die Tools und die Eingabeaufforderungen für verschiedene Anwendungsfälle anpassen.

Es verfügt über zwei integrierte Modi: **Build** und **Plan**. Sie können anpassen
diese oder konfigurieren Sie Ihre eigenen über die YONSOON (甬算)-Konfiguration.
...
YONSOON (甬算) verfügt über zwei integrierte Modi.
...
- `edit` – Vorhandene Dateien können nicht geändert werden, mit Ausnahme der Dateien unter `.yonsoon/plans/*.md`, um den Plan selbst detailliert darzustellen
...
Konfigurieren Sie Modi in Ihrer `yonsoon.json`-Konfigurationsdatei:
...
Wenn keine Temperatur angegeben ist, verwendet YONSOON (甬算) modellspezifische Standardwerte (normalerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle).
...
sowohl die globale YONSOON (甬算)-Konfiguration als auch die projektspezifische Konfiguration.
...
Erstellen Sie Modusdateien in `.yonsoon/modes/` für projektspezifische Modi oder `~/.config/yonsoon/modes/` für globale Modi:
