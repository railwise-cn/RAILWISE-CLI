...
Możesz dodać dodatkowe informacje do railwise za pomocą _Model Context Protocol_, w skrócie MCP. railwise obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [railwise Config](https://railwise.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/railwise`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
railwise automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, railwise:
...
Jeśli serwer wymaga uwierzytelnienia, railwise poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `railwise mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji railwise bezpieczne przechowa tokeny w `~/.local/share/railwise/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w railwise, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie railwise, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć railwise z kontem Sentry.
