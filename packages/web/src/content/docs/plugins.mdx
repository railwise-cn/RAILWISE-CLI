---
title: Plugins
description: Write your own plugins to extend RAILWISE (甬算).
---

Plugins allow you to extend RAILWISE (甬算) by hooking into various events and customizing behavior. You can create plugins to add new features, integrate with external services, or modify RAILWISE (甬算)'s default behavior.
...
- `.railwise/plugins/` - Project-level plugins
- `~/.config/railwise/plugins/` - Global plugins
...
**npm plugins** are installed automatically using Bun at startup. Packages and their dependencies are cached in `~/.cache/railwise/node_modules/`.
...
3. Global plugin directory (`~/.config/railwise/plugins/`)
4. Project plugin directory (`.railwise/plugins/`)
...
RAILWISE (甬算) runs `bun install` at startup to install these. Your plugins and tools can then import them.
...
- `client`: An railwise SDK client for interacting with the AI.
...
Here are some examples of plugins you can use to extend railwise.
...
:::note
If you’re using the RAILWISE (甬算) desktop app, it can send system notifications automatically when a response is ready or when a session errors.
:::
...
Prevent railwise from reading `.env` files:
...
Plugins can also add custom tools to railwise:
...
The `tool` helper creates a custom tool that railwise can call. It takes a Zod schema function and returns a tool definition with:

- `description`: What the tool does
- `args`: Zod schema for the tool's arguments
- `execute`: Function that runs when the tool is called

Your custom tools will be available to railwise alongside built-in tools.
...
Levels: `debug`, `info`, `warn`, `error`. See [SDK documentation](https://railwise.ai/docs/sdk) for details.
