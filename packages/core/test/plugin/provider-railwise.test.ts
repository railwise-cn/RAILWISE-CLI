  ...
  it.effect("uses a public key and disables paid models without credentials", () =>
    withEnv({ RAILWISE_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        expect((yield* catalog.provider.get(ProviderV2.ID.railwise)).request.body.apiKey).toBe("public")
  ...
  it.effect("uses RAILWISE_API_KEY as credentials", () =>
    withEnv({ RAILWISE_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        ...
        expect((yield* catalog.provider.get(ProviderV2.ID.railwise)).request.body.apiKey).toBeUndefined()
  ...
  it.effect("uses configured apiKey as credentials", () =>
    withEnv({ RAILWISE_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        expect((yield* catalog.provider.get(ProviderV2.ID.railwise)).request.body.apiKey).toBe("configured")
        expect((yield* catalog.model.get(ProviderV2.ID.railwise, ModelV2.ID.make("paid"))).enabled).toBe(true)
      }),
    ),
  )
