...
:::caution
Modlar artık railwise yapılandırmasındaki `agent` seçeneği aracılığıyla yapılandırılıyor.
`mode` seçeneği artık kullanımdan kaldırıldı. [Daha fazla bilgi](/docs/agents).
:::

railwise'daki modlar, farklı kullanım durumları için davranışı, araçları ve istemleri özelleştirmenize olanak tanır.

İki yerleşik modla birlikte gelir: **build** ve **plan**. Kişiselleştirebilirsiniz
bunları veya railwise yapılandırması aracılığıyla kendinizinkini yapılandırın.
...
railwise iki yerleşik modla birlikte gelir.
...
- `edit` - Planın kendisini detaylandırmak için `.railwise/plans/*.md` adresinde bulunan dosyalar dışında mevcut dosyalar değiştirilemez
...
`railwise.json` yapılandırma dosyanızdaki modları yapılandırın:
...
Sıcaklık belirtilmezse, railwise modele özgü varsayılanları kullanır (çoğu model için genellikle 0, Qwen modelleri için 0,55).
...
hem global railwise yapılandırması hem de projeye özel yapılandırma.
...
Projeye özel modlar için `.railwise/modes/` veya genel modlar için `~/.config/railwise/modes/`'de mod dosyaları oluşturun:
