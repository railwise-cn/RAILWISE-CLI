...
As habilidades do agente permitem que o railwise descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O railwise pesquisa nesses locais:

- Configuração do projeto: `.railwise/skills/<name>/SKILL.md`
- Configuração global: `~/.config/railwise/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o railwise sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.railwise/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/railwise/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.railwise/skills/git-release/SKILL.md` assim:
...
O railwise lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `railwise.json`:
...
**Para agentes embutidos** (em `railwise.json`):
