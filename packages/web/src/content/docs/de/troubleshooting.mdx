...
Wenn RAILWISE Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\railwise\log`
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\railwise`
...
RAILWISE Desktop startet im Hintergrund einen lokalen RAILWISE-Server (`railwise-cli`-Sidecar).
...
- Nur macOS: `RAILWISE`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/railwise/railwise.jsonc` (oder `~/.config/railwise/railwise.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/railwise/railwise.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\railwise\railwise.jsonc`
...
RAILWISE kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/railwise/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\railwise\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit RAILWISE ihn neu aufbauen kann.

1. Beende RAILWISE Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/railwise`
- **Linux**: loesche `~/.cache/railwise` (oder fuehre aus: `rm -rf ~/.cache/railwise`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\railwise`

3. Starte RAILWISE Desktop neu.

---

### Server-Verbindungsprobleme beheben

RAILWISE Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `railwise.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `RAILWISE_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `RAILWISE_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt RAILWISE Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit RAILWISE-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

RAILWISE Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer RAILWISE in den OS-Einstellungen aktiviert sind, und
...
1. Beende RAILWISE Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von RAILWISE Desktop):

- `railwise.settings.dat` (desktop default server URL)
- `railwise.global.dat` und `railwise.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit RAILWISE hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `railwise models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\railwise`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. railwise installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\railwise`

2. Starte railwise neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt railwise, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
railwise erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
