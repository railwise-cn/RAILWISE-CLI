---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di RAILWISE (甬算).
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

RAILWISE (甬算) fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire RAILWISE (甬算) avvia la TUI per la directory corrente.
...
Quando usi la TUI di RAILWISE (甬算), puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a RAILWISE (甬算). Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da RAILWISE (甬算). _Alias_: `/quit`, `/q`
...
Puoi personalizzare il comportamento della TUI tramite il file di config di RAILWISE (甬算).
