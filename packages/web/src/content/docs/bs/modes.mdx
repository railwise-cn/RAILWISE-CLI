...
ove ili konfigurirajte svoje putem yonsoon config.
...
yonsoon dolazi sa dva ugrađena načina rada.
...
- `edit` - Ne mogu modificirati postojeće fajlove, osim fajlova koji se nalaze na `.yonsoon/plans/*.md` radi detaljiziranja samog plana
...
Konfigurirajte načine rada u svom `yonsoon.json` konfiguracijskom fajlu:
...
- Globalno: `~/.config/yonsoon/modes/`
- Projekat: `.yonsoon/modes/`
...
Ako temperatura nije navedena, yonsoon koristi podrazumijevane postavke specifične za model (obično 0 za većinu modela i 0.55 za Qwen modele).
...
Kreirajte fajlove načina u `.yonsoon/modes/` za specifične načine rada ili `~/.config/yonsoon/modes/` za globalne načine:
