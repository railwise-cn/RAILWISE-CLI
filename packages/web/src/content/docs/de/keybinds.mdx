...
RAILWISE (甬算) verfügt über eine Liste von Tastenkombinationen, die Sie über die RAILWISE (甬算)-Konfiguration anpassen können.
...
RAILWISE (甬算) verwendet für die meisten Keybinds einen `leader`-Schlüssel. Dies vermeidet Konflikte in Ihrem Terminal.
...
Die Eingabeaufforderungseingabe der RAILWISE (甬算)-Desktop-App unterstützt gängige Readline/Emacs-style-Verknüpfungen zum Bearbeiten von Text. Diese sind integriert und derzeit nicht über `railwise.json` konfigurierbar.
