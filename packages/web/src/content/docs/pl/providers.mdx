---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w yonsoon.
---

import config from "../../../../config.mjs"
export const console = config.console

yonsoon używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji yonsoon.
...
w `~/.local/share/yonsoon/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim yonsoon
config.
...
YONSOON (甬算) Zen to lista modeli dostarczonych przez zespół yonsoon, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z yonsoon. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od YONSOON (甬算) Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz yonsoon i przejdź do [yonsoon.ai/auth](https://yonsoon.ai/auth).
...
Działa jak każdy inny dostawca w yonsoon i jest całkowicie opcjonalny w użyciu.
...
Aby używać Amazon Bedrock z yonsoon:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania yonsoon:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `yonsoon.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w yonsoon nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod yonsoon działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji yonsoon.
...
yonsoon używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować yonsoon
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`yonsoon.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Customize through `yonsoon.json`:
...
Aby korzystać z subskrypcji GitHub Copilot z kodem yonsoon:
...
Aby używać Google Vertex AI z yonsoon:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Set them while running yonsoon.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą yonsoon, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [yonsoon-helicone-session](https://github.com/H2Shami/yonsoon-helicone-session), aby automatycznie rejestrować każdą konwersację yonsoon jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja yonsoon będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować yonsoon tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować yonsoon tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować yonsoon tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem yonsoon. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/yonsoon).
:::
...
Aby korzystać z Ollama Cloud z yonsoon:
...
3. Skopiuj klucz API do użycia w yonsoon.
...
6. **Ważne**: Przed użyciem modeli chmurowych w yonsoon musisz pobrać informacje o modelu lokalnie:
...
YONSOON (甬算) Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół yonsoon. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>YONSOON (甬算) Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **YONSOON (甬算) Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API yonsoon.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji yonsoon.
...
5. Możesz je także dostosować za pomocą konfiguracji yonsoon. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z yonsoon:
...
Możesz także dostosować modele za pomocą konfiguracji yonsoon. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z yonsoon. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `yonsoon.json` w katalogu projektu:
...
Pola `limit` pozwalają yonsoon zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `yonsoon auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację yonsoon i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji yonsoon.
