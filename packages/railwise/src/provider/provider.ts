...
export namespace Provider {
  ...
  const CUSTOM_LOADERS: Record<string, CustomLoader> = {
    ...
    async railwise(input) {
      const hasKey = await (async () => {
        const env = Env.all()
        if (input.env.some((item) => env[item])) return true
        if (await Auth.get(input.id)) return true
        const config = await Config.get()
        if (config.provider?.["railwise"]?.options?.apiKey) return true
        return false
      })()
    ...
    try {
      ...
      options["fetch"] = async (input: any, init?: BunFetchRequestInit) => {
        ...
        const finalOpts = {
          ...
          timeout: false,
          // @ts-ignore bun-specific tls option for custom CA environments
  ...
  export async function getSmallModel(providerID: string) {
    ...
    // Check if railwise provider is available before using it
    const railwiseProvider = await state().then((state) => state.providers["railwise"])
    if (railwiseProvider && railwiseProvider.models["gpt-5-nano"]) {
