...
import { ToolFailure } from "@railwise/llm"
...
export class ConfigService extends Context.Service<ConfigService, Config>()("@railwise/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.RAILWISE_WEBSEARCH_PROVIDER === "exa" || process.env.RAILWISE_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.RAILWISE_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("RAILWISE_EXPERIMENTAL") || truthy("RAILWISE_ENABLE_EXA") || truthy("RAILWISE_EXPERIMENTAL_EXA"),
    enableParallel: truthy("RAILWISE_ENABLE_PARALLEL") || truthy("RAILWISE_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `railwise/${InstallationVersion}`,
