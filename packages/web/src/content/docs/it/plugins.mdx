---
title: Plugin
description: Scrivi plugin per estendere YONSOON (甬算).
---

I plugin ti permettono di estendere YONSOON (甬算) agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalita', integrare servizi esterni o modificare il comportamento predefinito di YONSOON (甬算).
...
- `.yonsoon/plugins/` - plugin a livello progetto
- `~/.config/yonsoon/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/yonsoon/node_modules/`.
...
1. Config globale (`~/.config/yonsoon/yonsoon.json`)
2. Config di progetto (`yonsoon.json`)
3. Directory plugin globale (`~/.config/yonsoon/plugins/`)
4. Directory plugin di progetto (`.yonsoon/plugins/`)
...
YONSOON (甬算) esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di yonsoon per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere yonsoon.
...
:::note
Se usi l'app desktop di YONSOON (甬算), puo' inviare automaticamente notifiche di sistema quando una risposta e' pronta o quando una sessione va in errore.
:::
...
Impedisci a yonsoon di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a yonsoon:
...
L'helper `tool` crea uno strumento personalizzato che yonsoon puo' chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in yonsoon insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://yonsoon.ai/docs/sdk) per i dettagli.
