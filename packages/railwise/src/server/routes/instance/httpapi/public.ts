...
import { RAILWISEHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for railwise serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(RAILWISEHttpApi)
...
export const PublicApi = RAILWISEHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "railwise",
    version: "1.0.0",
