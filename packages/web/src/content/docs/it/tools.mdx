...
Gli strumenti permettono all'LLM di eseguire azioni nel tuo codebase. YONSOON (甬算) include un set di strumenti integrati, ma puoi estenderlo con [strumenti personalizzati](/docs/custom-tools) o con [server MCP](/docs/mcp-servers).
...
Ecco tutti gli strumenti integrati disponibili in YONSOON (甬算).
...
:::note
Questo strumento e' disponibile solo quando `YONSOON_EXPERIMENTAL_LSP_TOOL=true` (oppure `YONSOON_EXPERIMENTAL=true`).
:::
...
:::note
Questo strumento e' disponibile solo quando usi il provider YONSOON (甬算) o quando la variabile d'ambiente `YONSOON_ENABLE_EXA` e' impostata a un valore truthy (ad esempio `true` o `1`).

Per abilitarlo all'avvio di YONSOON (甬算):
