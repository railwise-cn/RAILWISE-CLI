...
Le skill degli agenti permettono a YONSOON (甬算) di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
YONSOON (甬算) cerca in queste posizioni:

- Config di progetto: `.yonsoon/skills/<name>/SKILL.md`
- Config globale: `~/.config/yonsoon/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, YONSOON (甬算) risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.yonsoon/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/yonsoon/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.yonsoon/skills/git-release/SKILL.md` cosi':
...
YONSOON (甬算) elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `yonsoon.json`:
...
**Per agenti integrati** (in `yonsoon.json`):
