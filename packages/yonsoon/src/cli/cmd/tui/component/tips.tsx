...
const TIPS = [
  ...
  "Run {highlight}/share{/highlight} to create a public link to your conversation at yonsoon.ai",
  ...
  "Create {highlight}yonsoon.json{/highlight} in project root for project-specific settings",
  "Place settings in {highlight}~/.config/yonsoon/yonsoon.json{/highlight} for global config",
  ...
  "YONSOON (甬算) auto-handles OAuth for remote MCP servers requiring auth",
  "Add {highlight}.md{/highlight} files to {highlight}.yonsoon/command/{/highlight} to define reusable custom prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks in commands to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.yonsoon/agent/{/highlight} for specialized AI personas",
  "Configure per-agent permissions for {highlight}edit{/highlight}, {highlight}bash{/highlight}, and {highlight}webfetch{/highlight} tools",
  'Use patterns like {highlight}"git *": "allow"{/highlight} for granular bash permissions',
  'Set {highlight}"rm -rf *": "deny"{/highlight} to block destructive commands',
  'Configure {highlight}"git push": "ask"{/highlight} to require approval before pushing',
  "YONSOON (甬算) auto-formats files using prettier, gofmt, ruff, and more",
  'Set {highlight}"formatter": false{/highlight} in config to disable all auto-formatting',
  "Define custom formatter commands with file extensions in config",
  "YONSOON (甬算) uses LSP servers for intelligent code analysis",
  "Create {highlight}.ts{/highlight} files in {highlight}.yonsoon/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.yonsoon/plugin/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent YONSOON (甬算) from reading sensitive files",
  "Use {highlight}yonsoon run{/highlight} for non-interactive scripting",
  "Use {highlight}yonsoon --continue{/highlight} to resume the last session",
  "Use {highlight}yonsoon run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}yonsoon serve{/highlight} for headless API access to YONSOON (甬算)",
  "Use {highlight}yonsoon run --attach{/highlight} to connect to a running server",
  "Run {highlight}yonsoon upgrade{/highlight} to update to the latest version",
  "Run {highlight}yonsoon auth list{/highlight} to see all configured providers",
  "Run {highlight}yonsoon agent create{/highlight} for guided agent creation",
  "Use {highlight}/yonsoon{/highlight} in GitHub issues/PRs to trigger AI actions",
  "Run {highlight}yonsoon github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/yonsoon fix this{/highlight} on issues to auto-create PRs",
  "Comment {highlight}/oc{/highlight} on PR code lines for targeted code reviews",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.yonsoon/themes/{/highlight} directory",
  ...
  "Run {highlight}yonsoon debug config{/highlight} to troubleshoot configuration",
  ...
  "Run {highlight}docker run -it --rm ghcr.io/anomalyco/yonsoon{/highlight} for containerized use",
  "Use {highlight}/connect{/highlight} with YONSOON (甬算) Zen for curated, tested models",
