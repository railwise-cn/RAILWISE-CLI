---
title: Plugins
description: Escreva seus próprios plugins para estender o yonsoon.
---

Plugins permitem que você estenda o yonsoon conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do yonsoon.
...
- `.yonsoon/plugins/` - Plugins em nível de projeto
- `~/.config/yonsoon/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/yonsoon/node_modules/`.
...
1. Configuração global (`~/.config/yonsoon/yonsoon.json`)
2. Configuração do projeto (`yonsoon.json`)
3. Diretório de plugins global (`~/.config/yonsoon/plugins/`)
4. Diretório de plugins do projeto (`.yonsoon/plugins/`)
...
O yonsoon executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do yonsoon para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o yonsoon.
...
:::note
Se você estiver usando o aplicativo desktop yonsoon, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o yonsoon de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao yonsoon:
...
O helper `tool` cria uma ferramenta personalizada que o yonsoon pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o yonsoon junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://yonsoon.ai/docs/sdk) para detalhes.
