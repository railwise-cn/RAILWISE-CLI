...
Da biste otklonili probleme s RAILWISE (甬算), počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\railwise\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `railwise --log-level DEBUG`.

---

## Pohrana

RAILWISE (甬算) pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/railwise/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\railwise`
...
RAILWISE (甬算) Desktop pokreće lokalni RAILWISE (甬算) server (`railwise-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `RAILWISE (甬算)` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/railwise/railwise.jsonc` (ili `~/.config/railwise/railwise.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/railwise/railwise.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\railwise\railwise.jsonc`
...
RAILWISE (甬算) također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/railwise/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\railwise\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga RAILWISE (甬算) mogao ponovo izgraditi.

1. Potpuno zatvorite RAILWISE (甬算) Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/railwise`
- **Linux**: obrišite `~/.cache/railwise` (ili pokrenite `rm -rf ~/.cache/railwise`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\railwise`

3. Ponovo pokrenite RAILWISE (甬算) Desktop.

---

### Rješavanje problema sa vezom na serveru

RAILWISE (甬算) Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `railwise.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `RAILWISE_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `RAILWISE_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, RAILWISE (甬算) Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa RAILWISE (甬算) karakteristikama.

---

### Obavještenja se ne prikazuju

RAILWISE (甬算) Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za RAILWISE (甬算) u postavkama vašeg OS-a, i
...
1. Zatvorite RAILWISE (甬算) Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka RAILWISE (甬算) Desktop aplikacije):

- `railwise.settings.dat` (URL zadanog servera za desktop)
- `railwise.global.dat` i `railwise.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Ako imate problema s RAILWISE (甬算):
...
Da saznate kojim modelima imate pristup, pokrenite `railwise models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\railwise`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. RAILWISE (甬算) dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\railwise`

2. Ponovo pokrenite RAILWISE (甬算) da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti RAILWISE (甬算) da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
RAILWISE (甬算) će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
