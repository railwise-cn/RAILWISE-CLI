...
test("首页右侧项目栏显示最近会话与执行状态", async ({ launchApp }) => {
  ...
  await expect(page.locator("[data-testid=home-capability-rail]")).toContainText("执行")
  await expect(page.locator("[data-testid=home-agent-capability-preview]")).toContainText("RAILWISE 将调用")
  await expect(page.locator("[data-testid=home-capability-rail]")).toContainText("RAILWISE 默认协作")
