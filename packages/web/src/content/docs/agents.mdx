...
There are two types of agents in RAILWISE (甬算); primary agents and subagents.
...
RAILWISE (甬算) comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
RAILWISE (甬算) comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

RAILWISE (甬算) comes with two built-in primary agents and two built-in subagents.
...
If no temperature is specified, RAILWISE (甬算) uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global RAILWISE (甬算) config and the project specific config.
...
The model ID in your RAILWISE (甬算) config uses the format `provider/model-id`. For example, if you're using [RAILWISE (甬算) Zen](/docs/zen), you would use `railwise/gpt-5.1-codex` for GPT 5.1 Codex.
