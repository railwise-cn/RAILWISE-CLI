...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. RAILWISE (甬算) posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w RAILWISE (甬算).
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `RAILWISE_EXPERIMENTAL_LSP_TOOL=true` (lub `RAILWISE_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy RAILWISE (甬算) lub gdy zmienna środowiskowa `RAILWISE_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu RAILWISE (甬算):
