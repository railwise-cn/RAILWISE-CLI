...
RAILWISE (甬算) ima listu veza tipki koje možete prilagoditi preko RAILWISE (甬算) konfiguracije.
...
RAILWISE (甬算) koristi `leader` (vodeću) tipku za većinu povezivanja tipki. Ovo izbjegava sukobe u vašem terminalu.
...
Unos prompta aplikacije RAILWISE (甬算) za desktop podržava uobičajene prečice u stilu Readline/Emacs za uređivanje teksta. One su ugrađene i trenutno se ne mogu konfigurirati putem `railwise.json`.
