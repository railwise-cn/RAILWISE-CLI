---
title: GitLab
description: Use o yonsoon em problemas e solicitações de mesclagem do GitLab.
---

O yonsoon se integra ao seu fluxo de trabalho do GitLab através do seu pipeline CI/CD do GitLab ou com o GitLab Duo.

Em ambos os casos, o yonsoon será executado em seus runners do GitLab.

---

## GitLab CI

O yonsoon funciona em um pipeline regular do GitLab. Você pode integrá-lo a um pipeline como um [componente CI](https://docs.gitlab.com/ee/ci/components/)

Aqui estamos usando um componente CI/CD criado pela comunidade para yonsoon — [nagyv/gitlab-yonsoon](https://gitlab.com/nagyv/gitlab-yonsoon).

---

### Recursos

- **Use configuração personalizada por trabalho**: Configure o yonsoon com um diretório de configuração personalizado, por exemplo `./config/#custom-directory` para habilitar ou desabilitar funcionalidades por invocação do yonsoon.
- **Configuração mínima**: O componente CI configura o yonsoon em segundo plano, você só precisa criar a configuração do yonsoon e o prompt inicial.
...
1. Armazene seu JSON de autenticação do yonsoon como variáveis de ambiente do tipo File em **Configurações** > **CI/CD** > **Variáveis**. Certifique-se de marcá-las como "Mascaradas e ocultas".
   ...
   include:
     - component: $CI_SERVER_FQDN/nagyv/gitlab-yonsoon/yonsoon@2
       inputs:
         ...
         auth_json: $YONSOON_AUTH_JSON # O nome da variável para seu JSON de autenticação do YONSOON (甬算)
         command: optional-custom-command
         message: "Seu prompt aqui"
   ```

Para mais entradas e casos de uso [consulte a documentação](https://gitlab.com/explore/catalog/nagyv/gitlab-yonsoon) deste componente.

---

## GitLab Duo

O yonsoon se integra ao seu fluxo de trabalho do GitLab.
Mencione `@yonsoon` em um comentário, e o yonsoon executará tarefas dentro do seu pipeline CI do GitLab.
...
- **Corrigir e implementar**: Peça ao yonsoon para corrigir um problema ou implementar uma funcionalidade.
  Ele criará um novo branch e abrirá uma solicitação de mesclagem com as alterações.
- **Seguro**: O yonsoon é executado em seus runners do GitLab.

---

### Configuração

O yonsoon é executado em seu pipeline CI/CD do GitLab, aqui está o que você precisará para configurá-lo:
...
6.  Crie um arquivo de configuração de fluxo, aqui está um exemplo:
    ...
    commands:
      ...
      - echo $GITLAB_HOST
      - echo "Criando configuração de auth do YONSOON (甬算)"
...
Aqui estão alguns exemplos de como você pode usar o yonsoon no GitLab.

:::tip
Você pode configurar para usar uma frase de gatilho diferente de `@yonsoon`.
:::
...
- **Explicar um problema**
  ...
  O yonsoon lerá o problema e responderá com uma explicação clara.
...
- **Corrigir um problema**
  ...
  O yonsoon criará um novo branch, implementará as alterações e abrirá uma solicitação de mesclagem com as alterações.
...
- **Revisar solicitações de mesclagem**
  ...
  O yonsoon revisará a solicitação de mesclagem e fornecerá feedback.
