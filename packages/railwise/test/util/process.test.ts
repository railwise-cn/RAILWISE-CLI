...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.RAILWISE_TEST ?? "")'), {
      env: {
        RAILWISE_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "RAILWISE_TEST_SHELL"], {
      shell: true,
      env: {
        RAILWISE_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("RAILWISE_TEST_SHELL=ok")
  })
