...
Agent Skills erlauben RAILWISE (甬算), wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
RAILWISE (甬算) sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft RAILWISE (甬算) vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.railwise/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/railwise/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.railwise/skills/git-release/SKILL.md` zum Beispiel so:
...
RAILWISE (甬算) listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `railwise.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `railwise.json`):
