...
import { Ripgrep } from "@railwise/core/filesystem/ripgrep"
...
const configLayer = TestConfig.layer({
  directories: () => InstanceState.directory.pipe(Effect.map((dir) => [path.join(dir, ".railwise")])),
})
...
describe("tool.registry", () => {
  ...
  it.instance("loads tools from .railwise/tool (singular)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const railwise = path.join(test.directory, ".railwise")
      const tool = path.join(railwise, "tool")
  ...
  it.instance("ignores non-tool exports in .railwise/tool files", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tool = path.join(test.directory, ".railwise", "tool")
      yield* Effect.promise(() => fs.mkdir(tool, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  // came in through `plugin.list()` — `oh-my-railwise` was registering a tool
  ...
  it.instance("loads tools from .railwise/tools (plural)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const railwise = path.join(test.directory, ".railwise")
      const tools = path.join(railwise, "tools")
  ...
  it.instance("loads Zod-schema custom tools with JSON Schema and validation", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const customTools = path.join(test.directory, ".railwise", "tools")
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const railwise = path.join(test.directory, ".railwise")
        const customTools = path.join(railwise, "tools")
        const plugin = path.join(railwise, "node_modules", "@railwise", "plugin")
        yield* Effect.promise(() => fs.mkdir(path.join(plugin, "dist"), { recursive: true }))
        yield* Effect.promise(() => fs.mkdir(customTools, { recursive: true }))
        yield* Effect.promise(() =>
          fs.cp(path.dirname(fileURLToPath(import.meta.resolve("zod"))), path.join(railwise, "node_modules", "zod"), {
            dereference: true,
            recursive: true,
          }),
        )
        yield* Effect.promise(() =>
          Bun.write(
            ...
            JSON.stringify({ name: "@railwise/plugin", type: "module", exports: { ".": "./dist/index.js" } }),
          ),
        )
        yield* Effect.promise(() =>
          Bun.write(
  ...
  it.instance("loads legacy JSON-schema-shaped custom tools with wire schema", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tools = path.join(test.directory, ".railwise", "tools")
      yield* Effect.promise(() => fs.mkdir(tools, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  it.instance("loads tools with external dependencies without crashing", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(railwise, "package.json"),
          JSON.stringify({
            name: "custom-tools",
            dependencies: {
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(railwise, "package-lock.json"),
          JSON.stringify({
            name: "custom-tools",
            lockfileVersion: 3,
            packages: {
      ...
      const cowsay = path.join(railwise, "node_modules", "cowsay")
