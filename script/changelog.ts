...
import { $ } from "bun"
import { createRailwise } from "@railwise/sdk/v2"
...
export async function getLatestRelease(skip?: string) {
  const data = await fetch("https://api.github.com/repos/anomalyco/railwise/releases?per_page=100").then((res) => {
...
export async function getCommits(from: string, to: string): Promise<Commit[]> {
  ...
  const compare =
    await $`gh api "/repos/anomalyco/railwise/compare/${fromRef}...${toRef}" --jq '.commits[] | {sha: .sha, login: .author.login, message: .commit.message}'`.text()
  ...
  for (const hash of hashes) {
    ...
    for (const file of files.split("\n").filter(Boolean)) {
      if (file.startsWith("packages/railwise/src/cli/cmd/")) areas.add("tui")
      else if (file.startsWith("packages/railwise/")) areas.add("core")
...
async function summarizeCommit(railwise: Awaited<ReturnType<typeof createRailwise>>, message: string): Promise<string> {
  console.log("summarizing commit:", message)
  const session = await railwise.client.session.create()
  const result = await railwise.client.session
    .prompt(
      {
        sessionID: session.data!.id,
        model: { providerID: "railwise", modelID: "claude-sonnet-4-5" },
        tools: {
          "*": false,
        },
        parts: [
...
export async function generateChangelog(commits: Commit[], railwise: Awaited<ReturnType<typeof createRailwise>>) {
  // Summarize commits in parallel with max 10 concurrent requests
  const BATCH_SIZE = 10
  const summaries: string[] = []
  for (let i = 0; i < commits.length; i += BATCH_SIZE) {
    const batch = commits.slice(i, i + BATCH_SIZE)
    const results = await Promise.all(batch.map((c) => summarizeCommit(railwise, c.message)))
...
export async function getContributors(from: string, to: string) {
  ...
  const toRef = to === "HEAD" ? to : to.startsWith("v") ? to : `v${to}`
  const compare =
    await $`gh api "/repos/anomalyco/railwise/compare/${fromRef}...${toRef}" --jq '.commits[] | {login: .author.login, message: .commit.message}'`.text()
...
export async function buildNotes(from: string, to: string) {
  ...
  const railwise = await createRailwise({ port: 0 })
  const notes: string[] = []

  try {
    const lines = await generateChangelog(commits, railwise)
    notes.push(...lines)
