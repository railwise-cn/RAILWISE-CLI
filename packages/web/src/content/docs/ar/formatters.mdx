---
title: المُنسِّقات
description: يستخدم RAILWISE (甬算) مُنسِّقات خاصة بكل لغة.
---

يُنسِّق RAILWISE (甬算) الملفات تلقائيا بعد كتابتها أو تعديلها باستخدام مُنسِّقات خاصة بكل لغة. يضمن ذلك أن الشيفرة التي يتم توليدها تتبع أساليب التنسيق المعتمدة في مشروعك.

---

## مُضمَّنة

يأتي RAILWISE (甬算) مع عدة مُنسِّقات مُضمَّنة للغات وأطر العمل الشائعة. فيما يلي قائمة بالمُنسِّقات وامتدادات الملفات المدعومة والأوامر أو خيارات الإعداد التي تحتاجها.
...
لذا إذا كان مشروعك يتضمن `prettier` ضمن `package.json`، فسيستخدمه RAILWISE (甬算) تلقائيا.

---

## كيف يعمل

عندما يكتب RAILWISE (甬算) ملفا أو يحرره، فإنه:
...
يمكنك تخصيص المُنسِّقات عبر قسم `formatter` في إعدادات RAILWISE (甬算).
