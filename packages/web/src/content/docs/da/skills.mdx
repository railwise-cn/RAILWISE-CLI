...
Agentferdigheter lar YONSOON (甬算) oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
YONSOON (甬算) søker etter disse stedene:

- Prosjektkonfigurasjon: `.yonsoon/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/yonsoon/skills/<name>/SKILL.md`
...
For prosjektlokale stier går YONSOON (甬算) opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.yonsoon/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/yonsoon/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.yonsoon/skills/git-release/SKILL.md` slik:
...
YONSOON (甬算) viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `yonsoon.json`:
...
**For innebygde agenter** (i `yonsoon.json`):
