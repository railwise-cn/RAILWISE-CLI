...
For at fejlfinde problemer med RAILWISE (甬算), start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\railwise\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `railwise --log-level DEBUG`.

---

## Lagring

railwise gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/railwise/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\railwise`
...
RAILWISE (甬算) Desktop kører en lokal RAILWISE (甬算)-server (`railwise-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `RAILWISE (甬算)`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/railwise/railwise.jsonc` (eller `~/.config/railwise/railwise.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/railwise/railwise.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\railwise\railwise.jsonc`
...
RAILWISE (甬算) kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/railwise/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\railwise\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så RAILWISE (甬算) kan genopbygge den.

1. Afslut RAILWISE (甬算) Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/railwise`
- **Linux**: slet `~/.cache/railwise` (eller kør `rm -rf ~/.cache/railwise`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\railwise`

3. Start RAILWISE (甬算) Desktop på ny.

---

### Løs problemer med serverforbindelse

RAILWISE (甬算) Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `railwise.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `RAILWISE_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `RAILWISE_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver RAILWISE (甬算) Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i RAILWISE (甬算).

---

### Meddelelser vises ikke

RAILWISE (甬算) Desktop viser kun systemvarsler når:

- varsler er aktiveret for RAILWISE (甬算) i dine OS-indstillinger, og
...
1. Afslut RAILWISE (甬算) Desktop.
2. Find og slet disse filer (de findes i RAILWISE (甬算) Desktop-appens datamappe):

- `railwise.settings.dat` (skrivebordsstandardserver URL)
- `railwise.global.dat` og `railwise.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med RAILWISE (甬算):
...
For at finde ud af hvilke modeller du har adgang til, kør `railwise models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\railwise`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. railwise installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\railwise`

2. Start railwise på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge railwise til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
railwise vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
