---
title: Server
description: Interact with railwise server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `railwise serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an railwise client can use.
...
Set `RAILWISE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `railwise`, or set `RAILWISE_SERVER_USERNAME` to override it. This applies to both `railwise serve` and `railwise web`.
...
When you run `railwise` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the railwise server to interact with railwise programmatically.
:::

This architecture lets railwise support multiple clients and allows you to interact with railwise programmatically.

You can run `railwise serve` to start a standalone server. If you have the
railwise TUI running, `railwise serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the RAILWISE (甬算) [IDE](/docs/ide) plugins.
...
The railwise server exposes the following APIs.
