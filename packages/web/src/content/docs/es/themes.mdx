...
Con RAILWISE (甬算) puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, RAILWISE (甬算) usa nuestro propio tema `railwise`.
...
RAILWISE (甬算) viene con varios temas integrados.
...
- Quiere que RAILWISE (甬算) coincida con la apariencia de su terminal
...
RAILWISE (甬算) admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/railwise/themes/*.json` o `$XDG_CONFIG_HOME/railwise/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.railwise/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.railwise/themes/*.json`
