...
Dzięki railwise możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie railwise używa naszego własnego motywu `railwise`.
...
railwise ma kilka wbudowanych motywów.
...
- Chcą, aby RAILWISE pasował do wyglądu ich terminala
...
railwise obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/railwise/themes/*.json` lub `$XDG_CONFIG_HOME/railwise/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.railwise/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.railwise/themes/*.json`
