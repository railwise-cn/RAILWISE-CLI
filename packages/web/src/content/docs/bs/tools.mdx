...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. YONSOON (甬算) dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u YONSOON (甬算).
...
:::note
Ovaj alat je dostupan samo kada je `YONSOON_EXPERIMENTAL_LSP_TOOL=true` (ili `YONSOON_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz YONSOON (甬算) provajdera ili kada je varijabla `YONSOON_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju YONSOON (甬算):
