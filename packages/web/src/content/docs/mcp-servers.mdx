...
You can add external tools to RAILWISE (甬算) using the _Model Context Protocol_, or MCP. RAILWISE (甬算) supports both local and remote servers.
...
You can define MCP servers in your [RAILWISE (甬算) Config](https://railwise.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/railwise` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
RAILWISE (甬算) automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, RAILWISE (甬算) will:
...
If the server requires authentication, RAILWISE (甬算) will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `railwise mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, RAILWISE (甬算) will store the tokens securely in `~/.local/share/railwise/mcp-auth.json`.
...
Your MCPs are available as tools in RAILWISE (甬算), alongside built-in tools. So you can manage them through the RAILWISE (甬算) config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect RAILWISE (甬算) to your Sentry account.
