...
description: أنشئ أدوات يمكن لـ LLM استدعاؤها في railwise.
---

الأدوات المخصصة هي دوال تنشئها يمكن لـ LLM استدعاؤها أثناء المحادثات. تعمل جنبا إلى جنب مع [الأدوات المدمجة](/docs/tools) في railwise مثل `read` و`write` و`bash`.
...
- محليا بوضعها في مجلد `.railwise/tools/` داخل مشروعك.
- أو عالميا بوضعها في `~/.config/railwise/tools/`.
