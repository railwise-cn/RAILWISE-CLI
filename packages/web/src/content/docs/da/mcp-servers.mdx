...
Du kan tilføje eksterne værktøjer til RAILWISE (甬算) ved hjælp af _Model Context Protocol_ eller MCP. RAILWISE (甬算) understøtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i din [RAILWISE (甬算) Config](https://railwise.ai/docs/config/) under `mcp`. Tilføj hver MCP med et unikt navn. Du kan henvise til denne MCP ved navn, når du spørger LLM.
...
Organisationer kan levere standard MCP-servere via deres `.well-known/railwise`-slutpunkt. Disse servere kan være deaktiveret som standard, hvilket giver brugerne mulighed for at tilmelde sig dem, de har brug for.
...
RAILWISE (甬算) håndterer automatisk OAuth-godkendelse for eksterne MCP-servere. Når en server kræver godkendelse, vil RAILWISE (甬算):
...
Hvis serveren kræver godkendelse, vil RAILWISE (甬算) bede dig om at godkende, når du prøver at bruge den første gang. Hvis ikke, kan du [manually trigger the flow](#authenticating) med `railwise mcp auth <server-name>`.
...
Kommandoen `mcp auth` åbner din browser for godkendelse. Når du har godkendt, vil RAILWISE (甬算) opbevare tokens sikkert i `~/.local/share/railwise/mcp-auth.json`.
...
Dine MCPs er tilgængelige som værktøjer i **TK\_** sammen med indbyggede værktøjer. Så du kan administrere dem gennem RAILWISE (甬算)-konfigurationen som ethvert andet værktøj.
...
Dette åbner et browservindue for at fuldføre OAuth-flowet og forbinde RAILWISE (甬算) til din Sentry-konto.
