...
async function markPluginDependenciesReady(dir: string) {
  ...
  await Bun.write(
    ...
    JSON.stringify({ packages: { "": { dependencies: { "@railwise/plugin": "0.0.0" } } } }),
  )
}
...
test("ModelNotFoundError suggests catalog models for unloaded providers", async () => {
  ...
  await WithInstance.provide({
    directory: tmp.path,
    fn: async () => {
      remove("RAILWISE_API_KEY")
      try {
        await getModel(ProviderID.railwise, ModelID.make("claude-haiku-fake-model"))
        ...
        JSON.stringify({
          ...
          provider: {
            "cloudflare-ai-gateway": {
              options: {
                metadata: { invoked_by: "test", project: "railwise" },
              },
            },
          },
        }),
...
test("plugin config enabled and disabled providers are honored", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const root = path.join(dir, ".railwise", "plugin")
      await mkdir(root, { recursive: true })
      await Bun.write(
...
test("railwise loader keeps paid models when config apiKey is present", async () => {
...
test("railwise loader keeps paid models when auth exists", async () => {
  await using base = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "railwise.json"),
