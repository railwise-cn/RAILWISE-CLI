---
title: TUI
description: RAILWISE (甬算) ターミナルユーザーインターフェイスの使用。
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

RAILWISE (甬算) は、LLM を使用してプロジェクトを作業するための対話型ターミナルインターフェイスまたは TUI を提供します。

RAILWISE (甬算) を実行すると、現在のディレクトリの TUI が開始されます。
...
RAILWISE (甬算) TUI を使用する場合、「`/`」に続いてコマンド名を入力すると、アクションをすばやく実行できます。例えば：
...
RAILWISE (甬算) にプロバイダーを追加します。利用可能なプロバイダーから選択し、その API キーを追加できます。
...
RAILWISE (甬算) を終了します。 _エイリアス_: `/quit`、`/q`
...
RAILWISE (甬算) 設定ファイルを通じて TUI の動作をカスタマイズできます。
