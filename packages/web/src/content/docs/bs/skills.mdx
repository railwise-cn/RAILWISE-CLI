...
Agent skills omogucavaju YONSOON (甬算) da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
YONSOON (甬算) pretrazuje ove lokacije:

- Konfiguracija projekta: `.yonsoon/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/yonsoon/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, YONSOON (甬算) ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.yonsoon/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/yonsoon/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.yonsoon/skills/git-release/SKILL.md` ovako:
...
YONSOON (甬算) navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `yonsoon.json`:
...
**Za ugradene agente** (u `yonsoon.json`):
