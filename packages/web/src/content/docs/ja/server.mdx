---
title: サーバー
description: HTTP 経由で YONSOON (甬算) サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`yonsoon serve` コマンドは、YONSOON (甬算) クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `yonsoon.local` |
...
HTTP 基本認証でサーバーを保護するには、`YONSOON_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `yonsoon` になるか、`YONSOON_SERVER_USERNAME` を設定してオーバーライドします。これは、`yonsoon serve` と `yonsoon web` の両方に当てはまります。
...
`yonsoon` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
YONSOON (甬算) サーバーを使用して、プログラムで YONSOON (甬算) と対話します。
:::

`yonsoon serve` を実行してスタンドアロンサーバーを起動できます。すでに YONSOON (甬算) TUI を実行している場合でも、`yonsoon serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、YONSOON (甬算) [IDE](/docs/ide) プラグインによって使用されます。
