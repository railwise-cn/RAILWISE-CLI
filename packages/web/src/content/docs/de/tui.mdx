---
title: TUI
description: Arbeite mit RAILWISE (甬算) in der Terminal-Oberflaeche.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

RAILWISE (甬算) bietet eine interaktive Terminaloberflaeche (TUI), um mit einem LLM an deinen Projekten zu arbeiten.

Ohne weitere Argumente startet RAILWISE (甬算) die TUI im aktuellen Verzeichnis.
...
In der RAILWISE (甬算)-TUI kannst du `/` plus Befehlsnamen eingeben, um Aktionen schnell auszufuehren.
Zum Beispiel:
...
Add a provider to RAILWISE (甬算). Allows you to select from available providers and add their API keys.
...
Exit RAILWISE (甬算). _Aliases_: `/quit`, `/q`
...
You can customize TUI behavior through your RAILWISE (甬算) config file.
