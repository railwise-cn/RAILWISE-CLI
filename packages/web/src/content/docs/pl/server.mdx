---
title: Serwer
description: Interakcja z serwerem railwise za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `railwise serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient railwise.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `railwise.local` |
...
Ustaw `RAILWISE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `railwise` lub ustaw `RAILWISE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `railwise serve`, jak i `railwise web`.
...
Kiedy uruchomisz `railwise`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera railwise do programowej interakcji z railwise.
:::

Ta architektura umożliwia obsługę wielu klientów przez railwise i programową interakcję z kodem otwartym.

Możesz uruchomić `railwise serve`, aby uruchomić samodzielny serwer. Jeśli masz
railwise TUI działa, `railwise serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki railwise [IDE](/docs/ide).
...
Serwer railwise udostępnia następujące interfejsy API.
