...
O yonsoon tem uma lista de atalhos de teclado que você pode personalizar através da configuração do yonsoon.
...
O yonsoon usa uma tecla `leader` para a maioria dos atalhos de teclado. Isso evita conflitos no seu terminal.
...
O prompt de entrada do aplicativo desktop yonsoon suporta atalhos comuns de estilo Readline/Emacs para edição de texto. Estes são embutidos e atualmente não são configuráveis via `yonsoon.json`.
