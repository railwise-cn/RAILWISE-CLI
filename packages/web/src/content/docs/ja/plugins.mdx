---
title: プラグイン
description: RAILWISE を拡張する独自のプラグインを作成します。
---

プラグインを使用すると、さまざまなイベントにフックして動作をカスタマイズすることで RAILWISE を拡張できます。プラグインを作成して、新しい機能を追加したり、外部サービスと統合したり、RAILWISE のデフォルトの動作を変更したりできます。
...
RAILWISE は起動時に `bun install` を実行してこれらをインストールします。プラグインとツールはそれらをインポートできるようになります。
...
- `client`: AI と対話するための RAILWISE SDK クライアント。
...
ここでは、RAILWISE を拡張するために使用できるプラグインの例をいくつか示します。
...
:::note
RAILWISE デスクトップアプリを使用している場合は、応答の準備ができたとき、またはセッションエラーが発生したときにシステム通知を自動的に送信できます。
:::
...
RAILWISE が `.env` ファイルを読み取らないようにします。
...
プラグインは RAILWISE にカスタムツールを追加することもできます。
...
`tool` ヘルパーは、RAILWISE が呼び出すことができるカスタムツールを作成します。 Zod スキーマ関数を受け取り、次のようなツール定義を返します。

- `description`: ツールの機能
- `args`: ツールの引数の Zod スキーマ
- `execute`: ツールが呼び出されたときに実行される関数

カスタムツールは、組み込みツールと並行して RAILWISE で使用できます。
