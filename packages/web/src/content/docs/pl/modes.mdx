...
:::caution
Tryby są teraz konfigurowane za pomocą opcji `agent` w konfiguracji railwise.
...
te lub skonfiguruj własne za pomocą konfiguracji railwise.
...
- `edit` — Nie można zastosować naruszenia plików, z naruszeniem praw autorskich w `.railwise/plans/*.md` w celu uszczegółowienia samego planu
...
Skonfiguruj tryb w pliku konfiguracyjnym `railwise.json`:
...
- Globalnie: `~/.config/railwise/modes/`
...
Jeśli nie ma wpływu na temperaturę, railwise stosuje się narzędzia badawcze dla modelu (zwykle 0 dla największych modeli, 0,55 dla modeli Qwen).
...
zarówno globalna railwise, jak i specjalna dla projektu.
...
Utwórz pliki trybów w `.railwise/modes/` dla trybów zapisanych dla projektu lub `~/.config/railwise/modes/` dla trybów globalnych:
