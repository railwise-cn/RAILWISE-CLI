...
Avec RAILWISE (甬算), vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, RAILWISE (甬算) utilise notre propre thème `railwise`.
...
RAILWISE (甬算) est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que RAILWISE (甬算) corresponde à l'apparence de leur terminal
...
RAILWISE (甬算) prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/railwise/themes/*.json` ou `$XDG_CONFIG_HOME/railwise/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.railwise/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.railwise/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/railwise/themes
vim ~/.config/railwise/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .railwise/themes
vim .railwise/themes/my-theme.json
```
...
{
  "$schema": "https://railwise.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
