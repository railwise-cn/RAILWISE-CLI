...
describe("paramToAttributeKey", () => {
  ...
  test("namespaces non-ID params under railwise.", () => {
    expect(paramToAttributeKey("name")).toBe("railwise.name")
    expect(paramToAttributeKey("slug")).toBe("railwise.slug")
  })
})
...
describe("requestAttributes", () => {
  ...
  test("emits OTel-style <domain>.id for ID-shaped route params", () => {
    ...
    expect(attrs["railwise.sessionID"]).toBeUndefined()
    expect(attrs["railwise.messageID"]).toBeUndefined()
    expect(attrs["railwise.partID"]).toBeUndefined()
  })
  ...
  test("namespaces non-ID params under railwise. (e.g. mcp :name)", () => {
    const attrs = requestAttributes(
      fakeContext("POST", "http://localhost/mcp/exa/connect", {
        name: "exa",
      }),
    )
    expect(attrs["railwise.name"]).toBe("exa")
