...
description: Skriv dine egne programtillegg for å utvide yonsoon.
---

Plugins lar deg utvide YONSOON (甬算) ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til YONSOON (甬算).
...
- `.yonsoon/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/yonsoon/node_modules/`.
...
1. Global konfigurasjon (`~/.config/yonsoon/yonsoon.json`)
2. Prosjektkonfigurasjon (`yonsoon.json`)
3. Global plugin-katalog (`~/.config/yonsoon/plugins/`)
4. Prosjektpluginkatalog (`.yonsoon/plugins/`)
...
YONSOON (甬算) kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En YONSOON (甬算) SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide YONSOON (甬算).
...
:::note
Hvis du bruker YONSOON (甬算)-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre YONSOON (甬算) fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til YONSOON (甬算):
...
`tool`-hjelperen lager et tilpasset verktøy som YONSOON (甬算) kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for YONSOON (甬算) sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://yonsoon.ai/docs/sdk) for detaljer.
