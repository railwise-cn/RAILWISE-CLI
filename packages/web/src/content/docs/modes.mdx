...
:::caution
Modes are now configured through the `agent` option in the yonsoon config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in yonsoon allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the yonsoon config.
...
yonsoon comes with two built-in modes.
...
- `edit` - Cannot modify existing files, except for files located at `.yonsoon/plans/*.md` to detail the plan itself
...
Configure modes in your `yonsoon.json` config file:
...
If no temperature is specified, yonsoon uses model-specific defaults (typically 0 for most models, 0.55 for Qwen models).
...
both the global yonsoon config and the project specific config.
...
Create mode files in `.yonsoon/modes/` for project-specific modes or `~/.config/yonsoon/modes/` for global modes:
