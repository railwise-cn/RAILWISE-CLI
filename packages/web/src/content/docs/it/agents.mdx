...
In RAILWISE (甬算) esistono due tipi di agenti: agenti primari e subagenti.
...
RAILWISE (甬算) include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
RAILWISE (甬算) include due subagenti integrati: **General** e **Explore**. Li vediamo sotto.

---

## Integrati

RAILWISE (甬算) include due agenti primari integrati e due subagenti integrati.
...
Se non specifichi una temperature, RAILWISE (甬算) usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di RAILWISE (甬算) sia per la config specifica del progetto.
...
L'ID modello nella configurazione RAILWISE (甬算) usa il formato `provider/model-id`. Per esempio, se stai usando [RAILWISE (甬算) Zen](/docs/zen), useresti `railwise/gpt-5.1-codex` per GPT 5.1 Codex.
