...
Practical reference for extracting a future `packages/server` from the current `packages/railwise` monolith while `packages/core` is still being migrated to Effect.
...
- `packages/core` - all railwise services, Effect-first source of truth
- `packages/server` - railwise server, with separate contract and implementation, still producing `openapi.json`
...
- import from `server` and embed the full railwise server into an existing Node app
- spawn the CLI and talk to the server through that boundary

## Current state

Everything still lives in `packages/railwise`.
...
- those slices are mounted into the Hono server behind `RAILWISE_EXPERIMENTAL_HTTPAPI`
...
The intended direction here is the same, but the current `railwise` package split is earlier in the migration.
...
- the current server host still lives in `packages/railwise`
...
- if `packages/server` imports services from `packages/railwise` while `packages/railwise` imports `packages/server` to host routes, we create a package cycle immediately
...
4. keep `packages/railwise` as the temporary Hono host
...
- `packages/server` must not import from `packages/railwise`

Allowed in phase 1:

- `packages/railwise` imports `packages/server`
...
- `packages/railwise` shrinks or disappears as package responsibilities are fully split
...
- `packages/railwise` keeps generating legacy OpenAPI from Hono routes
...
- expose host adapters needed by `packages/railwise`
...
Keep in `packages/railwise` for now:
...
- host mounting from `packages/railwise`
...
- no imports from `packages/railwise` inside `packages/server`
...
- `packages/server` must still not import from `packages/railwise`
...
### PR 4. Mount `packages/server` question from `packages/railwise`

Scope:

- replace local experimental question route wiring in `packages/railwise`
...
- mount it in parallel from `packages/railwise`
...
- move adapter selection and server startup out of `packages/railwise`

Rules:

- do not start this while `packages/server` still depends on `packages/railwise`
...
- host logic no longer lives in `packages/railwise`
...
3. the route is mounted from the current host in `packages/railwise`
...
- `packages/server` imports services or runtime from `packages/railwise`
- `packages/railwise` imports route definitions or handlers from `packages/server`

Avoid by:

- keeping phase-1 `packages/server` free of `packages/railwise` imports
...
- [ ] mount `question` from `packages/railwise`
...
2. keep `packages/railwise` as the temporary host
...
If a proposed PR would make `packages/server` import from `packages/railwise`, stop and restructure the boundary first.
