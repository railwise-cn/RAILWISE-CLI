---
title: Plugins
description: Skriv dine egne plugins for at udvide YONSOON (甬算).
---

Plugins giver dig mulighed for at udvide YONSOON (甬算) ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre YONSOON (甬算)s standardadfærd.
...
- `.yonsoon/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/yonsoon/node_modules/`.
...
1. Global konfiguration (`~/.config/yonsoon/yonsoon.json`)
2. Projektkonfiguration (`yonsoon.json`)
3. Global plugin-mappe (`~/.config/yonsoon/plugins/`)
4. Projekt plugin bibliotek (`.yonsoon/plugins/`)
...
YONSOON (甬算) kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En yonsoon SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide yonsoon.
...
:::note
Hvis du bruger YONSOON (甬算) desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå yonsoon i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til yonsoon:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som yonsoon kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for yonsoon sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://yonsoon.ai/docs/sdk) for detaljer.
