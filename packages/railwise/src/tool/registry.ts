...
export namespace ToolRegistry {
  ...
  export class Service extends ServiceMap.Service<Service, Interface>()("@railwise/ToolRegistry") {}
  ...
  > = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const all = Effect.fn("ToolRegistry.all")(function* (custom: Tool.Info[]) {
        ...
        const question = ["app", "cli", "desktop"].includes(Flag.RAILWISE_CLIENT) || Flag.RAILWISE_ENABLE_QUESTION_TOOL
        ...
        return [
          ...
          skill,
          patch,
          ...(Flag.RAILWISE_EXPERIMENTAL_LSP_TOOL ? [lsp] : []),
          ...(cfg.experimental?.batch_tool === true ? [batch] : []),
          ...(Flag.RAILWISE_EXPERIMENTAL_PLAN_MODE && Flag.RAILWISE_CLIENT === "cli" ? [plan] : []),
          ...custom,
        ]
      })
      ...
      const tools = Effect.fn("ToolRegistry.tools")(function* (
        ...
        const filtered = allTools.filter((tool) => {
          if (tool.id === "codesearch" || tool.id === "websearch") {
            return model.providerID === ProviderID.railwise || Flag.RAILWISE_ENABLE_EXA
          }

          const usePatch =
            !!Env.get("RAILWISE_E2E_LLM_URL") ||
