...
グローバル RAILWISE 設定を `~/.config/railwise/railwise.json` に配置します。テーマ、プロバイダー、キーバインドなどのユーザー全体の設定にはグローバル設定を使用します。
...
`small_model` オプションは、タイトル生成などの軽量タスク用に別のモデルを構成します。デフォルトでは、RAILWISE は、プロバイダーから安価なモデルが入手可能な場合は、より安価なモデルを使用しようとします。そうでない場合は、メインモデルにフォールバックします。
...
`theme` オプションを使用して、RAILWISE 設定で使用するテーマを構成できます。
