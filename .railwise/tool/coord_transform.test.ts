...
describe("gauss_forward", () => {
  test("known point near Hangzhou (CGCS2000, CM=120)", async () => {
    const r = parse(
      await gauss_forward.execute(
        { lat: 30.25, lon: 120.15, centralMeridian: 120, datum: "CGCS2000", addFalseEasting: true, zonePrefix: false },
        ctx,
      ),
    )
  ...
  test("point on central meridian yields y ≈ 500000 with false easting", async () => {
    const r = parse(
      await gauss_forward.execute(
        { lat: 31.0, lon: 121.0, centralMeridian: 121, datum: "CGCS2000", addFalseEasting: true, zonePrefix: false },
        ctx,
      ),
    )
  ...
  test("without false easting, y near 0 on central meridian", async () => {
    const r = parse(
      await gauss_forward.execute(
        { lat: 31.0, lon: 121.0, centralMeridian: 121, datum: "WGS84", addFalseEasting: false, zonePrefix: false },
        ctx,
      ),
    )
  ...
  test("zone prefix adds band number to y", async () => {
    const r = parse(
      await gauss_forward.execute(
        { lat: 30.0, lon: 120.5, centralMeridian: 120, datum: "CGCS2000", addFalseEasting: true, zonePrefix: true },
        ctx,
      ),
    )
  ...
  test("Beijing54 ellipsoid produces different x than CGCS2000", async () => {
    const a = parse(
      await gauss_forward.execute(
        { lat: 30.0, lon: 114.0, centralMeridian: 114, datum: "CGCS2000", addFalseEasting: false, zonePrefix: false },
        ctx,
      ),
    )
    const b = parse(
      await gauss_forward.execute(
        { lat: 30.0, lon: 114.0, centralMeridian: 114, datum: "Beijing54", addFalseEasting: false, zonePrefix: false },
        ctx,
      ),
    )
...
describe("gauss_inverse", () => {
  test("round-trip forward then inverse recovers original BL", async () => {
    ...
    const fwd = parse(
      await gauss_forward.execute(
        { lat: lat0, lon: lon0, centralMeridian: cm, datum: "CGCS2000", addFalseEasting: true, zonePrefix: false },
        ctx,
      ),
    )
    const inv = parse(
      await gauss_inverse.execute(
        {
          x: fwd.output.x_m,
          y: fwd.output.y_m,
          centralMeridian: cm,
          datum: "CGCS2000",
          hasFalseEasting: true,
          hasZonePrefix: false,
        },
        ctx,
      ),
  ...
  test("round-trip with zone prefix", async () => {
    ...
    const fwd = parse(
      await gauss_forward.execute(
        { lat: lat0, lon: lon0, centralMeridian: cm, datum: "Xian80", addFalseEasting: true, zonePrefix: true },
        ctx,
      ),
    )
    const inv = parse(
  ...
  test("inverse on known coordinates near Beijing (Beijing54)", async () => {
    ...
    const fwd = parse(
      await gauss_forward.execute(
        { lat: lat0, lon: lon0, centralMeridian: cm, datum: "Beijing54", addFalseEasting: false, zonePrefix: false },
        ctx,
      ),
    )
    const inv = parse(
...
describe("datum_transform", () => {
  test("same datum returns identity", async () => {
    const r = parse(
      await datum_transform.execute({ lat: 30.0, lon: 120.0, h: 50, from: "CGCS2000", to: "CGCS2000" }, ctx),
    )
  ...
  test("WGS84 ↔ CGCS2000 is near-identity (< 0.001° difference)", async () => {
    const r = parse(
      await datum_transform.execute({ lat: 31.23, lon: 121.47, h: 10, from: "WGS84", to: "CGCS2000" }, ctx),
    )
  ...
  test("Beijing54 → CGCS2000 produces visible shift", async () => {
    const r = parse(
      await datum_transform.execute({ lat: 30.0, lon: 114.0, h: 0, from: "Beijing54", to: "CGCS2000" }, ctx),
    )
  ...
  test("round-trip Beijing54 → CGCS2000 → Beijing54 recovers original", async () => {
    const lat0 = 39.9,
      ...
      h0 = 50
    const mid = parse(
      await datum_transform.execute({ lat: lat0, lon: lon0, h: h0, from: "Beijing54", to: "CGCS2000" }, ctx),
    )
    const back = parse(
      await datum_transform.execute(
        { lat: mid.output.lat_deg, lon: mid.output.lon_deg, h: mid.output.h_m, from: "CGCS2000", to: "Beijing54" },
        ctx,
      ),
    )
  ...
  test("Xian80 → WGS84 produces shift", async () => {
    const r = parse(
      await datum_transform.execute({ lat: 34.26, lon: 108.94, h: 400, from: "Xian80", to: "WGS84" }, ctx),
    )
  ...
  test("Beijing54 → Xian80 uses direct transform", async () => {
    const r = parse(
      await datum_transform.execute({ lat: 30.0, lon: 114.0, h: 0, from: "Beijing54", to: "Xian80" }, ctx),
    )
  ...
  test("custom 7-parameters override built-in", async () => {
    ...
    const r = parse(
      await datum_transform.execute(
        { lat: 30.0, lon: 114.0, h: 0, from: "Beijing54", to: "CGCS2000", customParams: custom },
        ctx,
      ),
    )
