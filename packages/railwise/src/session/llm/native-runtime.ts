...
import { tool as nativeTool, ToolFailure, type JsonSchema, type LLMEvent } from "@railwise/llm"
import type { LLMClientShape } from "@railwise/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("railwise"))
    return { type: "unsupported", reason: "provider is not openai, railwise, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @railwise/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains railwise-owned. The native runtime only adapts
      // the @railwise/llm tool call back into the AI SDK Tool.execute shape.
      nativeTool({
