---
title: Konfiguracija
description: Korištenje RAILWISE (甬算) JSON konfiguracije.
---

Možete konfigurirati RAILWISE (甬算) koristeći JSON konfiguracijski fajl.

---

## Format

RAILWISE (甬算) podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
Na primjer, ako vaša globalna konfiguracija postavlja `theme: "railwise"` i `autoupdate: true`, a vaša projektna konfiguracija postavlja `model: "anthropic/claude-sonnet-4-5"`, konačna konfiguracija će uključivati ​​sve tri postavke.
...
2. **Globalna konfiguracija** (`~/.config/railwise/railwise.json`) - korisničke postavke
...
Postavite svoju globalnu RAILWISE (甬算) konfiguraciju u `~/.config/railwise/railwise.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su teme, provajderi ili veze tipki.
...
:::
Kada se RAILWISE (甬算) pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš RAILWISE (甬算) server.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj RAILWISE (甬算) konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, RAILWISE (甬算) pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Možete konfigurirati temu koju želite koristiti u svojoj RAILWISE (甬算) konfiguraciji putem opcije `theme`.
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, RAILWISE (甬算) će se vratiti na `"build"` s upozorenjem.
...
RAILWISE (甬算) će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, RAILWISE (甬算) **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju RAILWISE (甬算) sa prilagođenim alatima, kukicama i integracijama.
...
Ovo je korisno kada želite da ograničite RAILWISE (甬算) da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
:::note
`disabled_providers` ima prioritet nad `enabled_providers`.
:::
