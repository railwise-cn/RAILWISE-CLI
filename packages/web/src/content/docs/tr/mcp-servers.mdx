...
_Model Bağlam Protokolü_ veya MCP kullanarak yonsoon'a harici araçlar ekleyebilirsiniz. yonsoon hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [yonsoon Config](https://yonsoon.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/yonsoon` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
yonsoon, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde yonsoon şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa yonsoon, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `yonsoon mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından yonsoon, belirteçleri `~/.local/share/yonsoon/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra yonsoon'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi yonsoon yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve yonsoon'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
