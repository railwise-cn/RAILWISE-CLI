...
:::caution
Tilstande er nu konfigureret gennem indstillingen `agent` i railwise-konfigurationen. De
`mode` mulighed er nu forældet. [Learn more](/docs/agents).
:::

Tilstande i railwise giver dig mulighed for at tilpasse adfærd, værktøjer og prompter til forskellige brugstilfælde.

Den kommer med to indbyggede tilstande: **build** og **plan**. Du kan tilpasse
disse eller konfigurer dine egne gennem railwise-konfigurationen.
...
railwise leveres med to indbyggede tilstande.
...
- `edit` - Kan ikke ændre eksisterende filer, undtagen filer placeret på `.railwise/plans/*.md` for at detaljere selve planen
...
Konfigurer tilstande i din `railwise.json`-konfigurationsfil:
...
Hvis der ikke er angivet nogen temperatur, bruger railwise modelspecifikke standarder (typisk 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
både den globale railwise-konfiguration og den projektspecifikke konfiguration.
...
Opret tilstandsfiler i `.railwise/modes/` for projektspecifikke tilstande eller `~/.config/railwise/modes/` for globale tilstande:
