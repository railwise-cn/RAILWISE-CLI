...
RAILWISE-CLI 当前内置 12 个工程测绘业务智能体，默认由 `chief_manager` 作为 RAILWISE 默认协作统一调度。安装后无需手工复制 `.railwise/agent`，npm、curl、Homebrew 和 GitHub Release 包都会随 CLI 带上这些资源。

## 智能体清单

| 智能体 | 角色 | 主要职责 |
| --- | --- | --- |
| `chief_manager` | RAILWISE 默认协作 | 拆解任务、安排协作顺序、控制质量闸门 |
