...
import { ModelsDev } from "@railwise/core/models"
...
const AGENT_USERNAME = "railwise-agent[bot]"
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/railwise.yml"
    ...
    yield* Effect.promise(async () => {
      {
        ...
        async function installGitHubApp() {
          ...
          // Open browser
          const url = "https://github.com/apps/railwise-agent"
          const command =
          ...
          async function getInstallation() {
            return await fetch(
              `https://api.railwise.ai/get_github_app_installation?owner=${app.owner}&repo=${app.repo}`,
            )
              .then((res) => res.json())
...
jobs:
  railwise:
    ...
    steps:
      ...
      - name: Run railwise
        uses: anomalyco/railwise/github@latest${envStr}
        with:
...
export const GithubRunCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.github.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      const shareBaseUrl = isMock ? "https://dev.railwise.ai" : "https://railwise.ai"
      ...
      function normalizeOidcBaseUrl(): string {
        ...
        if (!value) return "https://api.railwise.ai"
      ...
      async function getOidcToken() {
        try {
          return await core.getIDToken("railwise-github-action")
        } catch (error) {
      ...
      function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
        ...
        if (type === "schedule" || type === "dispatch") {
          ...
          return `railwise/${type}-${hex}-${timestamp}`
        }
        return `railwise/${type}${issueId}-${timestamp}`
      }
      ...
      function footer(opts?: { image?: boolean }) {
        const image = (() => {
          ...
          return `<a href="${shareBaseUrl}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/railwise-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
        })()
        const shareUrl = shareId ? `[railwise session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
    ...
    issue(number: $number) {
      ...
      function buildPromptDataForIssue(issue: GitHubIssue) {
        ...
        return [
          ...
          "- Git push and PR creation are handled AUTOMATICALLY by the railwise infrastructure after your response",
