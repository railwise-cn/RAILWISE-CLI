...
export namespace Config {
  ...
  export const state = Instance.state(async () => {
    ...
    if (!Flag.OPENCODE_DISABLE_PROJECT_CONFIG) {
      for (const file of ["yonsoon.jsonc", "yonsoon.json", "opencode.jsonc", "opencode.json"]) {
    ...
    for (const dir of unique(directories)) {
      if (dir.endsWith(".yonsoon") || dir.endsWith(".opencode") || dir === Flag.OPENCODE_CONFIG_DIR) {
        for (const file of ["yonsoon.jsonc", "yonsoon.json", "opencode.jsonc", "opencode.json"]) {
  ...
  function globalConfigFile() {
    const candidates = ["yonsoon.jsonc", "yonsoon.json", "opencode.jsonc", "opencode.json", "config.json"].map((file) =>
