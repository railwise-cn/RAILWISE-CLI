...
使用 GitHub Actions 的 `Desktop Release` 流程构建 macOS 和 Windows 安装包。发布产物上传到 Release 和对象存储，更新元数据由 `updates.railwise.cn` 返回。签名 secrets 和触发步骤见 `docs/admin/06-desktop-release-runbook.md`。
