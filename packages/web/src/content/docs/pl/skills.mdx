...
Umiejętności agenta pozwalają railwise odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
railwise przeszukuje te lokalizacje:

- Project config: `.railwise/skills/<name>/SKILL.md`
- Global config: `~/.config/railwise/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu railwise przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.railwise/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/railwise/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.railwise/skills/git-release/SKILL.md` w ten sposób:
...
railwise wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `railwise.json`:
...
**Dla agentów wbudowanych** (w `railwise.json`):
