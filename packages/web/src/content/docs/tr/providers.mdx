---
title: Sağlayıcılar
description: yonsoon'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

yonsoon, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı yonsoon yapılandırmanızda yapılandırın.
...
`~/.local/share/yonsoon/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları yonsoon'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
YONSOON (甬算) Zen, yonsoon ekibi tarafından sağlanan modellerin bir listesidir.
yonsoon ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz YONSOON (甬算) Zen ile başlamanızı öneririz.
:::
...
yonsoon'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.
...
Amazon Bedrock'u yonsoon ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   yonsoon'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `yonsoon.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi yonsoon'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   yonsoon'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   Yonsoon yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
to store token in yonsoon auth storage.
:::

##### Kendi Kendine Barındırılan GitLab

:::note[uyumluluk notu]
yonsoon, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. yonsoon'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`yonsoon.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
`yonsoon.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi yonsoon'la kullanmak için:
...
Google Vertex AI'yi yonsoon ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları yonsoon çalıştırılırken ayarlayın.
...
Helicone'un yonsoon aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her yonsoon görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [yonsoon-helicone-session](https://github.com/H2Shami/yonsoon-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her yonsoon görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
yonsoon'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla yonsoon'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla yonsoon'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini yonsoon için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/yonsoon)'a bakın.
:::
...
Ollama Cloud'u yonsoon ile kullanmak için:
...
3. yonsoon'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: yonsoon'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
YONSOON (甬算) Zen, yonsoon ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>YONSOON (甬算) Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **YONSOON (甬算) Zen**'i arayın.

   ```txt
   /connect
   ```

3. yonsoon API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   yonsoon yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca yonsoon yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi Yonsoon ile kullanmak için:
...
Modelleri yonsoon yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı yonsoon ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `yonsoon.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları yonsoon'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `yonsoon auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için yonsoon yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, yonsoon yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
