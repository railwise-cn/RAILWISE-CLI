---
title: TUI
description: YONSOON (甬算) ターミナルユーザーインターフェイスの使用。
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

YONSOON (甬算) は、LLM を使用してプロジェクトを作業するための対話型ターミナルインターフェイスまたは TUI を提供します。

YONSOON (甬算) を実行すると、現在のディレクトリの TUI が開始されます。
...
YONSOON (甬算) TUI を使用する場合、「`/`」に続いてコマンド名を入力すると、アクションをすばやく実行できます。例えば：
...
YONSOON (甬算) にプロバイダーを追加します。利用可能なプロバイダーから選択し、その API キーを追加できます。
...
YONSOON (甬算) を終了します。 _エイリアス_: `/quit`、`/q`
...
YONSOON (甬算) 設定ファイルを通じて TUI の動作をカスタマイズできます。
