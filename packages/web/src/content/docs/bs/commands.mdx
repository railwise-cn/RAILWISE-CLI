...
Kreiraj `.railwise/commands/test.md`:
...
Možete dodati prilagođene komande kroz RAILWISE (甬算) konfiguraciju ili kreiranjem markdown datoteka u direktoriju `commands/`.

---

### JSON

Koristite opciju `command` u svom RAILWISE (甬算) [config](/docs/config):
...
- Globalno: `~/.config/railwise/commands/`
- Po projektu: `.railwise/commands/`
...
RAILWISE (甬算) uključuje nekoliko ugrađenih naredbi kao što su `/init`, `/undo`, `/redo`, `/share`, `/help`; [saznaj više](/docs/tui#commands).
:::note
