---
title: Server
description: Komunicirajte s railwise serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `railwise serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji railwise klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `railwise.local` |
...
Postavite `RAILWISE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `railwise`, ili postavite `RAILWISE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `railwise serve` i za `railwise web`.
...
Kada pokrenete `railwise`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite railwise server za programsku interakciju sa railwise-om.
:::

Ova arhitektura omogucava railwise podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `railwise serve` da startate standalone server. Ako je railwise TUI vec pokrenut, `railwise serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste RAILWISE (甬算) [IDE](/docs/ide) pluginovi.
...
railwise server izlaže sljedece API-je.
