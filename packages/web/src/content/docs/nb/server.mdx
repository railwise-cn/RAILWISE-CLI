---
title: Tjener
description: Samhandle med YONSOON (甬算)-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `yonsoon serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en YONSOON (甬算)-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `yonsoon.local` |
...
Sett `YONSOON_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `yonsoon` som standard, eller sett `YONSOON_SERVER_USERNAME` for å overstyre det. Dette gjelder både `yonsoon serve` og `yonsoon web`.
...
Når du kjører `yonsoon` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk YONSOON (甬算)-serveren til å samhandle med YONSOON (甬算) programmatisk.
:::

Denne arkitekturen lar YONSOON (甬算) støtte flere klienter og lar deg samhandle med YONSOON (甬算) programmatisk.

Du kan kjøre `yonsoon serve` for å starte en frittstående server. Hvis du har
YONSOON (甬算) TUI kjører, vil `yonsoon serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av YONSOON (甬算) [IDE](/docs/ide) plugins.
...
yonsoon-serveren viser følgende APIer.
