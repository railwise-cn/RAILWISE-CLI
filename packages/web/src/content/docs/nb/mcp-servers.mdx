...
Du kan legge til eksterne verktøy i RAILWISE (甬算) ved å bruke _Model Context Protocol_ eller MCP. RAILWISE (甬算) støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [RAILWISE (甬算) Config](https://railwise.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="railwise.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/railwise`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
RAILWISE (甬算) håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil RAILWISE (甬算):
...
Hvis serveren krever autentisering, vil RAILWISE (甬算) be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `railwise mcp auth <server-name>`.
...
Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil RAILWISE (甬算) lagre tokens sikkert i `~/.local/share/railwise/mcp-auth.json`.
...
MCP-ene dine er tilgjengelige som verktøy i RAILWISE (甬算), sammen med innebygde verktøy. Så du kan administrere dem gjennom RAILWISE (甬算)-konfigurasjonen som alle andre verktøy.
...
```json title="railwise.json" {11, 14-18}
{
  "$schema": "https://railwise.ai/config.json",
  "mcp": {
    "my-mcp": {
...
Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble RAILWISE (甬算) til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver RAILWISE (甬算)-samtale oppført som en separat økt.
