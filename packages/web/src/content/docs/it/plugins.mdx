...
I plugin ti permettono di estendere RAILWISE agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di RAILWISE.
...
:::note
Se usi l'app desktop di RAILWISE, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
L'helper `tool` crea uno strumento personalizzato che railwise può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
