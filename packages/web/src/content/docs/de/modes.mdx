...
:::caution
Modi werden jetzt über die Option `agent` in der RAILWISE (甬算)-Konfiguration konfiguriert. Der
Die Option `mode` ist jetzt veraltet. [Learn more](/docs/agents).
:::

Mit den Modi in RAILWISE (甬算) können Sie das Verhalten, die Tools und die Eingabeaufforderungen für verschiedene Anwendungsfälle anpassen.

Es verfügt über zwei integrierte Modi: **Build** und **Plan**. Sie können anpassen
diese oder konfigurieren Sie Ihre eigenen über die RAILWISE (甬算)-Konfiguration.
...
RAILWISE (甬算) verfügt über zwei integrierte Modi.
...
- `edit` – Vorhandene Dateien können nicht geändert werden, mit Ausnahme der Dateien unter `.railwise/plans/*.md`, um den Plan selbst detailliert darzustellen
...
Konfigurieren Sie Modi in Ihrer `railwise.json`-Konfigurationsdatei:
...
Wenn keine Temperatur angegeben ist, verwendet RAILWISE (甬算) modellspezifische Standardwerte (normalerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle).
...
sowohl die globale RAILWISE (甬算)-Konfiguration als auch die projektspezifische Konfiguration.
...
Erstellen Sie Modusdateien in `.railwise/modes/` für projektspezifische Modi oder `~/.config/railwise/modes/` für globale Modi:
