---
title: Formateri
description: RAILWISE (甬算) koristi formatere specifične za jezik.
---

RAILWISE (甬算) automatski formatira datoteke nakon što su napisane ili uređene pomoću formatera specifičnih za jezik. Ovo osigurava da kod koji se generira prati stilove koda vašeg projekta.

---

## Ugrađeni

RAILWISE (甬算) dolazi sa nekoliko ugrađenih formatera za popularne jezike i okvire. Ispod je lista formatera, podržanih ekstenzija datoteka i naredbi ili opcija konfiguracije koje su mu potrebne.
...
Dakle, ako vaš projekat ima `prettier` u vašem `package.json`, RAILWISE (甬算) će ga automatski koristiti.

---

## Kako radi

Kada RAILWISE (甬算) piše ili uređuje datoteku, on:
...
Možete prilagoditi formatere kroz `formatter` odjeljak u vašoj RAILWISE (甬算) konfiguraciji.
