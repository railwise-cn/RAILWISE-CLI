...
Il existe deux types d'agents dans YONSOON (甬算) ; agents primaires et sous-agents.
...
YONSOON (甬算) est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
YONSOON (甬算) est livré avec deux sous-agents intégrés, **General** et **Explore**. Nous verrons cela ci-dessous.

---

## Agents intégrés

YONSOON (甬算) est livré avec deux agents primaires intégrés et deux sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `yonsoon.json` :
...
- Global : `~/.config/yonsoon/agents/`
- Par projet : `.yonsoon/agents/`

```markdown title="~/.config/yonsoon/agents/review.md"
---
...
Si aucune température n'est spécifiée, YONSOON (甬算) utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale YONSOON (甬算) et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration YONSOON (甬算) utilise le format `provider/model-id`. Par exemple, si vous utilisez [YONSOON (甬算) Zen](/docs/zen), vous utiliserez `yonsoon/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```json title="yonsoon.json" {3-6,9-12}
{
  "$schema": "https://yonsoon.ai/config.json",
  "tools": {
    "write": true,
...
```json title="yonsoon.json" {3-5,8-10}
{
  "$schema": "https://yonsoon.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Exécutez `yonsoon models` pour voir une liste des modèles disponibles.
:::
...
:::tip
Avez-vous un agent que vous aimeriez partager ? [Soumettez un PR](https://github.com/anomalyco/yonsoon).
:::
...
```markdown title="~/.config/yonsoon/agents/docs-writer.md"
---
...
```markdown title="~/.config/yonsoon/agents/security-auditor.md"
---
