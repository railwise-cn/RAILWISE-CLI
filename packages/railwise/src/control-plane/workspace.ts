...
import { Slug } from "@railwise/shared/util/slug"
...
export const create = fn(CreateInput, async (input) => {
  ...
  const env = {
    RAILWISE_AUTH_CONTENT: JSON.stringify(await AppRuntime.runPromise(Auth.Service.use((auth) => auth.all()))),
...
async function startSync(space: Info) {
  if (!Flag.RAILWISE_EXPERIMENTAL_WORKSPACES) return
