...
RAILWISE Go je povoljna pretplata — **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno** — koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

Go radi kao bilo koji drugi provajder u RAILWISE-u. Pretplatite se na RAILWISE Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili RAILWISE.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa RAILWISE-om.
:::
...
RAILWISE Go vam daje pristup ovim modelima za **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno**.

---

## Kako funkcioniše

RAILWISE Go radi kao bilo koji drugi provajder u RAILWISE-u.

1. Prijavite se na **<a href={console}>RAILWISE Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `RAILWISE Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na RAILWISE Go.
:::
...
RAILWISE Go uključuje sljedeća ograničenja:

- **Ograničenje od 5 sati** — $12 potrošnje
- **Sedmično ograničenje** — $30 potrošnje
- **Mjesečno ograničenje** — $60 potrošnje

Ograničenja su definisana u dolarskoj vrijednosti. To znači da vaš stvarni broj zahtjeva zavisi od modela koji koristite. Jeftiniji modeli poput DeepSeek V4 Flash omogućavaju više zahtjeva, dok skuplji modeli poput GLM-5.1 omogućavaju manje.
...
[Model id](/docs/config/#models) u vašoj RAILWISE konfiguraciji
koristi format `railwise-go/<model-id>`. Na primjer, za Kimi K2.6, koristili biste
`railwise-go/kimi-k2.6` u svojoj konfiguraciji.
...
Napravili smo RAILWISE Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz RAILWISE koristite i bilo kojeg drugog provajdera.
