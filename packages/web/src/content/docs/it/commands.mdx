...
Puoi aggiungere comandi personalizzati tramite la config di YONSOON (甬算) o creando file markdown nella directory `commands/`.

---

### JSON

Usa l'opzione `command` nella [config](/docs/config) di YONSOON (甬算):
...
- Globale: `~/.config/yonsoon/commands/`
- Per progetto: `.yonsoon/commands/`
...
yonsoon include diversi comandi integrati come `/init`, `/undo`, `/redo`, `/share`, `/help`; [scopri di piu](/docs/tui#commands).
