---
title: プラグイン
description: RAILWISE (甬算) を拡張する独自のプラグインを作成します。
---

プラグインを使用すると、さまざまなイベントにフックして動作をカスタマイズすることで RAILWISE (甬算) を拡張できます。プラグインを作成して、新しい機能を追加したり、外部サービスと統合したり、RAILWISE (甬算) のデフォルトの動作を変更したりできます。
...
- `.railwise/plugins/` - プロジェクトレベルのプラグイン
- `~/.config/railwise/plugins/` - グローバルプラグイン
...
**npm プラグイン** は、起動時に Bun を使用して自動的にインストールされます。パッケージとその依存関係は `~/.cache/railwise/node_modules/` にキャッシュされます。
...
1. グローバル設定 (`~/.config/railwise/railwise.json`)
2. プロジェクト設定 (`railwise.json`)
3. グローバルプラグインディレクトリ (`~/.config/railwise/plugins/`)
4. プロジェクトプラグインディレクトリ (`.railwise/plugins/`)
...
RAILWISE (甬算) は起動時に `bun install` を実行してこれらをインストールします。プラグインとツールはそれらをインポートできるようになります。
...
- `client`: AI と対話するための RAILWISE (甬算) SDK クライアント。
...
ここでは、RAILWISE (甬算) を拡張するために使用できるプラグインの例をいくつか示します。
...
:::note
RAILWISE (甬算) デスクトップアプリを使用している場合は、応答の準備ができたとき、またはセッションエラーが発生したときにシステム通知を自動的に送信できます。
:::
...
RAILWISE (甬算) が `.env` ファイルを読み取らないようにします。
...
プラグインは RAILWISE (甬算) にカスタムツールを追加することもできます。
...
`tool` ヘルパーは、RAILWISE (甬算) が呼び出すことができるカスタムツールを作成します。 Zod スキーマ関数を受け取り、次のようなツール定義を返します。

- `description`: ツールの機能
- `args`: ツールの引数の Zod スキーマ
- `execute`: ツールが呼び出されたときに実行される関数

カスタムツールは、組み込みツールと並行して RAILWISE (甬算) で使用できます。
...
レベル: `debug`、`info`、`warn`、`error`。詳細については、[SDK ドキュメント](https://railwise.ai/docs/sdk) を参照してください。
