...
Sie können externe Tools zu RAILWISE (甬算) hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. RAILWISE (甬算) unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [RAILWISE (甬算) Config](https://railwise.ai/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
Organisationen können über ihren `.well-known/railwise`-Endpunkt Standard-MCP-Server bereitstellen. Diese Server sind möglicherweise standardmäßig deaktiviert, sodass Benutzer sich für den Server entscheiden können, die sie benötigen.
...
RAILWISE (甬算) übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird RAILWISE (甬算) Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von RAILWISE (甬算) beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `railwise mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert RAILWISE (甬算) die Token sicher in `~/.local/share/railwise/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in RAILWISE (甬算) verfügbar. Sie können sie also wie jedes andere Tool über die RAILWISE (甬算)-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und RAILWISE (甬算) mit Ihrem Sentry-Konto zu verbinden.
