---
title: Server
description: Interager med railwise-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `railwise serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en railwise-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `railwise.local` |
...
Indstil `RAILWISE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `railwise`, eller indstil `RAILWISE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `railwise serve` og `railwise web`.
...
Når du kører `railwise` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug railwise-serveren til at interagere med railwise programmatisk.
:::

Denne arkitektur lader railwise understøtte flere klienter og lader dig interagere med railwise programmatisk.

Du kan køre `railwise serve` for at starte en selvstændig server. Hvis du har
railwise TUI kørende, vil `railwise serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af RAILWISE (甬算) [IDE](/docs/ide) plugins.
...
RAILWISE (甬算)-serveren viser følgende API'er.
