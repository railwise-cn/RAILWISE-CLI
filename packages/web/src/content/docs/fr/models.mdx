...
Les fournisseurs les plus populaires sont préchargés par défaut. Si vous avez ajouté les informations d'identification d'un fournisseur via la commande `/connect`, elles seront disponibles lorsque vous démarrerez RAILWISE (甬算).
...
Voici plusieurs modèles qui fonctionnent bien avec RAILWISE (甬算), sans ordre particulier. (Cette liste n’est pas exhaustive et n’est pas nécessairement à jour) :
...
Pour définir l'un d'entre eux comme modèle par défaut, vous pouvez définir la clé `model` dans votre configuration RAILWISE (甬算).

```json title="railwise.json" {3}
{
...
Ici, l'identifiant complet est `provider_id/model_id`. Par exemple, si vous utilisez [RAILWISE (甬算) Zen](/docs/zen), vous utiliserez `railwise/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```jsonc title="railwise.jsonc" {7-12,19-24}
{
  "$schema": "https://railwise.ai/config.json",
  "provider": {
    "openai": {
...
```jsonc title="railwise.jsonc" {6-21}
{
  "$schema": "https://railwise.ai/config.json",
  "provider": {
    "railwise": {
...
De nombreux modèles prennent en charge plusieurs variantes avec différentes configurations. RAILWISE (甬算) est livré avec des variantes par défaut intégrées pour les fournisseurs populaires.

### Variantes intégrées

RAILWISE (甬算) est livré avec des variantes par défaut pour de nombreux fournisseurs :
...
```jsonc title="railwise.jsonc" {7-18}
{
  "$schema": "https://railwise.ai/config.json",
  "provider": {
    "openai": {
...
Lorsque RAILWISE (甬算) démarre, il recherche les modèles dans l'ordre de priorité suivant :
...
2. La liste des modèles dans la configuration RAILWISE (甬算).
