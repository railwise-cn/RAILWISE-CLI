---
title: Server
description: Komunicirajte s yonsoon serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `yonsoon serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji yonsoon klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `yonsoon.local` |
...
Postavite `YONSOON_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `yonsoon`, ili postavite `YONSOON_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `yonsoon serve` i za `yonsoon web`.
...
Kada pokrenete `yonsoon`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite yonsoon server za programsku interakciju sa yonsoon-om.
:::

Ova arhitektura omogucava yonsoon podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `yonsoon serve` da startate standalone server. Ako je yonsoon TUI vec pokrenut, `yonsoon serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste YONSOON (甬算) [IDE](/docs/ide) pluginovi.
...
yonsoon server izlaže sljedece API-je.
