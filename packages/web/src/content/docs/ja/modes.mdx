...
:::caution
モードは、YONSOON (甬算) 設定の `agent` オプションを通じて設定されるようになりました。
...
これらを使用するか、YONSOON (甬算) 設定を通じて独自の設定を行います。
...
YONSOON (甬算) には 2 つの組み込みモードが付属しています。
...
- `edit` - 計画自体の詳細を示す `.yonsoon/plans/*.md` にあるファイルを除き、既存のファイルを変更できません
- `patch` - パッチを適用できません
...
- グローバル: `~/.config/yonsoon/modes/`
- プロジェクト: `.yonsoon/modes/`
...
温度が指定されていない場合、YONSOON (甬算) はモデル固有のデフォルトを使用します (通常、ほとんどのモデルでは 0、Qwen モデルでは 0.55)。
...
モードファイルをプロジェクト固有モードの場合は `.yonsoon/modes/` に、グローバルモードの場合は `~/.config/yonsoon/modes/` に作成します。
