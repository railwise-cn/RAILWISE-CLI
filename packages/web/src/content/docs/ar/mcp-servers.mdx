...
يمكنك إضافة أدوات خارجية إلى RAILWISE باستخدام _Model Context Protocol_، أو MCP. يدعم RAILWISE خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات RAILWISE](https://railwise.ai/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يتولى RAILWISE تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم RAILWISE بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك RAILWISE المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `railwise mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن RAILWISE الرموز بشكل آمن في `~/.local/share/railwise/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل RAILWISE إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات RAILWISE مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط RAILWISE بحساب Sentry الخاص بك.
