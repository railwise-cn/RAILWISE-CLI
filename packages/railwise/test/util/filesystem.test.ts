...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "railwise.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "railwise.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "railwise.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "railwise.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["railwise.json", "railwise.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "railwise.json"),
        path.join(project, "railwise.jsonc"),
      ])
    })
  })
