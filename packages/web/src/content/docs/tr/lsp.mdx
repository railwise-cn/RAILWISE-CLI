...
railwise, LLM'in kod tabanınızla etkileşime girmesine yardımcı olmak için Dil Sunucusu Protokolünüzle (LSP) bütünleşir. LLM'ye geri bildirim sağlamak için tanılamayı kullanır.
...
Railwise bir dosyayı açtığında:
...
LSP sunucularını, railwise yapılandırmanızdaki `lsp` bölümü aracılığıyla özelleştirebilirsiniz.
