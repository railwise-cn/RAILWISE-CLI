---
title: TUI
description: Arbeite mit YONSOON (甬算) in der Terminal-Oberflaeche.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

YONSOON (甬算) bietet eine interaktive Terminaloberflaeche (TUI), um mit einem LLM an deinen Projekten zu arbeiten.

Ohne weitere Argumente startet YONSOON (甬算) die TUI im aktuellen Verzeichnis.
...
In der YONSOON (甬算)-TUI kannst du `/` plus Befehlsnamen eingeben, um Aktionen schnell auszufuehren.
Zum Beispiel:
...
Add a provider to YONSOON (甬算). Allows you to select from available providers and add their API keys.
...
Exit YONSOON (甬算). _Aliases_: `/quit`, `/q`
...
You can customize TUI behavior through your YONSOON (甬算) config file.
