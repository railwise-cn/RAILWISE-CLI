---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en RAILWISE (甬算).
---

import config from "../../../../config.mjs"
export const console = config.console

RAILWISE (甬算) utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration RAILWISE (甬算).
...
Vous pouvez personnaliser les fournisseurs via la section `provider` de votre RAILWISE (甬算)
configuration.
...
RAILWISE (甬算) Zen est une liste de modèles fournis par l'équipe RAILWISE (甬算) qui ont été
testé et vérifié pour fonctionner correctement avec RAILWISE (甬算). [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par RAILWISE (甬算) Zen.
:::
...
Il fonctionne comme n’importe quel autre fournisseur dans RAILWISE (甬算) et son utilisation est totalement facultative.
...
Pour utiliser Amazon Bedrock avec RAILWISE (甬算) :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans RAILWISE (甬算) n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
RAILWISE (甬算) utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller RAILWISE (甬算)
...
Pour utiliser Google Vertex AI avec RAILWISE (甬算) :
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [railwise-helicone-session](https://github.com/H2Shami/railwise-helicone-session) pour enregistrer automatiquement chaque conversation RAILWISE (甬算) en tant que session dans Helicone.
...
Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation RAILWISE (甬算) répertoriée comme une session distincte.
...
:::tip
Ollama peut se configurer automatiquement pour RAILWISE (甬算). Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/railwise) pour plus de détails.
:::
...
Pour utiliser Ollama Cloud avec RAILWISE (甬算) :
...
3. Copiez la clé API à utiliser dans RAILWISE (甬算).
...
6. **Important** : Avant d'utiliser des modèles cloud dans RAILWISE (甬算), vous devez extraire les informations du modèle localement :
...
RAILWISE (甬算) Zen est une liste de modèles testés et vérifiés fournie par l'équipe RAILWISE (甬算). [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>RAILWISE (甬算) Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **RAILWISE (甬算) Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé RAILWISE (甬算) API.
...
Les champs `limit` permettent à RAILWISE (甬算) de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
