...
import { RelativePath } from "@railwise/core/schema"
...
describe("Ripgrep", () => {
  it.live("allows caller globs to re-include git metadata", () =>
    Effect.acquireUseRelease(
      ...
      (tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() => fs.mkdir(path.join(tmp.path, ".railwise")))
          yield* Effect.promise(() => fs.writeFile(path.join(tmp.path, ".railwise", "config"), "needle\n"))
          ...
          expect(files.map((item) => item.path)).toContain(RelativePath.make(".railwise/config"))
          ...
          expect(matches.map((item) => item.entry.path)).toContain(RelativePath.make(".railwise/config"))
