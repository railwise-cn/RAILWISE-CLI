...
# パッケージマネージャー
npm i -g railwise-ai@latest        # bun/pnpm/yarn でもOK
scoop install railwise             # Windows
choco install railwise             # Windows
brew install anomalyco/tap/railwise # macOS と Linux（推奨。常に最新）
brew install railwise              # macOS と Linux（公式 brew formula。更新頻度は低め）
sudo pacman -S railwise            # Arch Linux (Stable)
paru -S railwise-bin               # Arch Linux (Latest from AUR)
mise use -g railwise               # どのOSでも
nix run nixpkgs#railwise           # または github:anomalyco/railwise で最新 dev ブランチ
```
...
1. `$RAILWISE_INSTALL_DIR` - カスタムのインストールディレクトリ
2. `$XDG_BIN_DIR` - XDG Base Directory Specification に準拠したパス
3. `$HOME/bin` - 標準のユーザー用バイナリディレクトリ（存在する場合、または作成できる場合）
4. `$HOME/.railwise/bin` - デフォルトのフォールバック
...
[agents](https://railwise.ai/docs/agents) の詳細はこちら。
...
**コミュニティに参加** [Discord](https://discord.gg/railwise) | [X.com](https://x.com/railwise)
