...
:::caution
Modlar artık yonsoon yapılandırmasındaki `agent` seçeneği aracılığıyla yapılandırılıyor.
`mode` seçeneği artık kullanımdan kaldırıldı. [Daha fazla bilgi](/docs/agents).
:::

yonsoon'daki modlar, farklı kullanım durumları için davranışı, araçları ve istemleri özelleştirmenize olanak tanır.

İki yerleşik modla birlikte gelir: **build** ve **plan**. Kişiselleştirebilirsiniz
bunları veya yonsoon yapılandırması aracılığıyla kendinizinkini yapılandırın.
...
yonsoon iki yerleşik modla birlikte gelir.
...
- `edit` - Planın kendisini detaylandırmak için `.yonsoon/plans/*.md` adresinde bulunan dosyalar dışında mevcut dosyalar değiştirilemez
...
`yonsoon.json` yapılandırma dosyanızdaki modları yapılandırın:
...
Sıcaklık belirtilmezse, yonsoon modele özgü varsayılanları kullanır (çoğu model için genellikle 0, Qwen modelleri için 0,55).
...
hem global yonsoon yapılandırması hem de projeye özel yapılandırma.
...
Projeye özel modlar için `.yonsoon/modes/` veya genel modlar için `~/.config/yonsoon/modes/`'de mod dosyaları oluşturun:
