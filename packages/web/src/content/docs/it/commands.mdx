...
Crea `.railwise/commands/test.md`:
...
Puoi aggiungere comandi personalizzati tramite la config di RAILWISE (甬算) o creando file markdown nella directory `commands/`.

---

### JSON

Usa l'opzione `command` nella [config](/docs/config) di RAILWISE (甬算):
...
- Globale: `~/.config/railwise/commands/`
- Per progetto: `.railwise/commands/`
...
railwise include diversi comandi integrati come `/init`, `/undo`, `/redo`, `/share`, `/help`; [scopri di piu](/docs/tui#commands).
