---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i RAILWISE.
---

import config from "../../../../config.mjs"
export const console = config.console

RAILWISE bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i RAILWISE-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/railwise/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i RAILWISE-konfigurationen.
...
RAILWISE Zen er en liste over modeller leveret af RAILWISE-teamet, som er blevet testet og verificeret til at fungere godt med RAILWISE. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med RAILWISE Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg railwise og gå til [railwise.ai/auth](https://railwise.ai/auth).
...
Det fungerer som alle andre udbydere i RAILWISE og er helt valgfrit at bruge.

---

## RAILWISE Go

RAILWISE Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af RAILWISE-teamet, som er testet og verificeret til at fungere godt med RAILWISE.

1. Kør kommandoen `/connect` i TUI, vælg `RAILWISE Go`, og gå til [railwise.ai/auth](https://railwise.ai/zen).
...
Sådan bruger du Amazon Bedrock med RAILWISE:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører railwise:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `railwise.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i RAILWISE understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere railwise til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at railwise skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem railwise-konfigurationen.
...
gemme tokenet i railwise auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
RAILWISE bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse RAILWISE
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`railwise.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [railwise-gitlab-auth](https://www.npmjs.com/package/railwise-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `railwise.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med RAILWISE:
...
Sådan bruger du Google Vertex AI med RAILWISE:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører railwise.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem railwise, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [railwise-helicone-session](https://github.com/H2Shami/railwise-helicone-session) plugin for automatisk at logge hver RAILWISE-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver RAILWISE-samtale opført som en separat session.
...
Du kan konfigurere railwise til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere railwise til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere railwise til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for RAILWISE. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/railwise) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med RAILWISE:
...
3. Kopier API-nøglen til brug i RAILWISE.
...
6. **Vigtigt**: Før du bruger skymodeller i RAILWISE, skal du hente modelinformationen lokalt:
...
RAILWISE Zen er en liste over testede og verificerede modeller leveret af RAILWISE-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>RAILWISE Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **RAILWISE Zen**.

   ```txt
   /connect
   ```

3. Indtast RAILWISE API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem railwise-konfigurationen.
...
5. Du kan også tilpasse dem gennem railwise-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med RAILWISE:
...
Du kan også tilpasse modeller gennem railwise-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med railwise. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `railwise.json`-filen i projektkataloget:
...
`limit`-felterne lader RAILWISE forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `railwise auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek railwise-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i railwise-konfigurationen.
