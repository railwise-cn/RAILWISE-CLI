---
title: サーバー
description: HTTP 経由で RAILWISE サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`railwise serve` コマンドは、RAILWISE クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
:::tip
RAILWISE サーバーを使用して、プログラムで RAILWISE と対話します。
:::

`railwise serve` を実行してスタンドアロンサーバーを起動できます。すでに RAILWISE TUI を実行している場合でも、`railwise serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、RAILWISE [IDE](/docs/ide) プラグインによって使用されます。
