---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en YONSOON (甬算).
---

import config from "../../../../config.mjs"
export const console = config.console

YONSOON (甬算) utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration YONSOON (甬算).
...
en `~/.local/share/yonsoon/auth.json`.

---

### Configuration

Vous pouvez personnaliser les fournisseurs via la section `provider` de votre YONSOON (甬算)
configuration.
...
YONSOON (甬算) Zen est une liste de modèles fournis par l'équipe YONSOON (甬算) qui ont été
testé et vérifié pour fonctionner correctement avec YONSOON (甬算). [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par YONSOON (甬算) Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez yonsoon et dirigez-vous vers [yonsoon.ai/auth](https://yonsoon.ai/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans YONSOON (甬算) et son utilisation est totalement facultative.
...
Pour utiliser Amazon Bedrock avec YONSOON (甬算) :
...
Définissez l'une de ces variables d'environnement lors de l'exécution de yonsoon :

```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY yonsoon

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile yonsoon

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX yonsoon
```
...
Pour une configuration spécifique au projet ou persistante, utilisez `yonsoon.json` :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans YONSOON (甬算) n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que yonsoon fonctionne correctement.
   :::
...
5. Définissez le nom de votre ressource comme variable d'environnement :
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX yonsoon
   ```
...
5. Définissez le nom de votre ressource comme variable d'environnement :
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX yonsoon
   ```
...
Vous pouvez également ajouter des modèles via votre configuration yonsoon.
...
pour stocker le jeton dans le stockage d'authentification yonsoon.
:::

##### GitLab auto-hébergé

:::note[note de conformité]
YONSOON (甬算) utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller YONSOON (甬算)
pour utiliser uniquement votre propre instance hébergée par GitLab, ajoutez ce qui suit à votre
Fichier `yonsoon.json`. Il est également recommandé de désactiver le partage de session.
...
Personnalisez via `yonsoon.json` :
...
{
  "$schema": "https://yonsoon.ai/config.json",
  "plugin": ["@gitlab/yonsoon-gitlab-plugin"]
}
```
...
Pour utiliser votre abonnement GitHub Copilot avec yonsoon :
...
Pour utiliser Google Vertex AI avec YONSOON (甬算) :
...
Définissez-les lors de l'exécution de yonsoon.

```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id yonsoon
```
...
Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via yonsoon, vous pouvez toujours le configurer vous-même.
...
```jsonc title="~/.config/yonsoon/yonsoon.jsonc"
{
  "$schema": "https://yonsoon.ai/config.json",
  "provider": {
    "helicone": {
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [yonsoon-helicone-session](https://github.com/H2Shami/yonsoon-helicone-session) pour enregistrer automatiquement chaque conversation YONSOON (甬算) en tant que session dans Helicone.

```bash
npm install -g yonsoon-helicone-session
```
...
```json title="yonsoon.json"
{
  "plugin": ["yonsoon-helicone-session"]
}
```

Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation YONSOON (甬算) répertoriée comme une session distincte.
...
Vous pouvez configurer yonsoon pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)

```json title="yonsoon.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://yonsoon.ai/config.json",
  "provider": {
    "llama.cpp": {
...
Vous pouvez configurer yonsoon pour utiliser des modèles locaux via LM Studio.

```json title="yonsoon.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://yonsoon.ai/config.json",
  "provider": {
    "lmstudio": {
...
Vous pouvez configurer yonsoon pour utiliser des modèles locaux via Ollama.

:::tip
Ollama peut se configurer automatiquement pour YONSOON (甬算). Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/yonsoon) pour plus de détails.
:::

```json title="yonsoon.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://yonsoon.ai/config.json",
  "provider": {
    "ollama": {
...
Pour utiliser Ollama Cloud avec YONSOON (甬算) :
...
3. Copiez la clé API à utiliser dans YONSOON (甬算).
...
6. **Important** : Avant d'utiliser des modèles cloud dans YONSOON (甬算), vous devez extraire les informations du modèle localement :
...
YONSOON (甬算) Zen est une liste de modèles testés et vérifiés fournie par l'équipe YONSOON (甬算). [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>YONSOON (甬算) Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **YONSOON (甬算) Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé YONSOON (甬算) API.
...
Vous pouvez également ajouter des modèles supplémentaires via votre configuration yonsoon.
...
5. Vous pouvez également les personnaliser via votre configuration yonsoon. Voici un exemple de spécification d'un fournisseur
...
```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' yonsoon
```
...
4. Définissez éventuellement l'ID de déploiement et le groupe de ressources :
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group yonsoon
   ```
...
Pour utiliser [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) avec Yonsoon :
...
Vous pouvez également personnaliser les modèles via votre configuration yonsoon. Voici un exemple de spécification de l'ordre de routage du fournisseur.
...
:::tip
Vous pouvez utiliser n'importe quel fournisseur compatible OpenAI avec yonsoon. La plupart des fournisseurs d'IA modernes proposent des API compatibles OpenAI.
:::
...
3. Entrez votre clé API pour le fournisseur.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in yonsoon.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Créez ou mettez à jour votre fichier `yonsoon.json` dans le répertoire de votre projet :

   ```json title="yonsoon.json" ""myprovider"" {5-15}
   {
     "$schema": "https://yonsoon.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="yonsoon.json" {9,11,17-20}
{
  "$schema": "https://yonsoon.ai/config.json",
  "provider": {
    "myprovider": {
...
Les champs `limit` permettent à YONSOON (甬算) de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
...
1. **Vérifiez la configuration de l'authentification** : exécutez `yonsoon auth list` pour voir si les informations d'identification
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration yonsoon et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration yonsoon.
