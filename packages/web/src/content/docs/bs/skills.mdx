...
Agent skills omogucavaju RAILWISE (甬算) da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
RAILWISE (甬算) pretrazuje ove lokacije:

- Konfiguracija projekta: `.railwise/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/railwise/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, RAILWISE (甬算) ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.railwise/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/railwise/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.railwise/skills/git-release/SKILL.md` ovako:
...
RAILWISE (甬算) navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `railwise.json`:
...
**Za ugradene agente** (u `railwise.json`):
