...
RAILWISE Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, RAILWISE'daki diğer sağlayıcılar gibi çalışır. RAILWISE Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve RAILWISE'u kullanmak için bunu kullanmanıza gerek yoktur.
...
RAILWISE Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

RAILWISE Go, RAILWISE'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>RAILWISE Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `RAILWISE Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye RAILWISE Go'ya abone olabilir.
:::
...
RAILWISE Go aşağıdaki limitleri içerir:

- **5 saatlik limit** — 12$ kullanım
- **Haftalık limit** — 30$ kullanım
- **Aylık limit** — 60$ kullanım

Limitler dolar değeri üzerinden belirlenmiştir. Bu, gerçek istek sayınızın kullandığınız modele bağlı olduğu anlamına gelir. DeepSeek V4 Flash gibi daha ucuz modeller daha fazla isteğe izin verirken, GLM-5.1 gibi yüksek maliyetli modeller daha azına izin verir.
...
- MiniMax M3 — İstek başına 810 girdi, 62.000 önbelleğe alınmış, 225 çıktı token'ı
...
RAILWISE yapılandırmanızdaki [model id](/docs/config/#models) formatı `railwise-go/<model-id>` şeklindedir. Örneğin, Kimi K2.6 için yapılandırmanızda `railwise-go/kimi-k2.6` kullanmalısınız.
...
RAILWISE Go'yu şu amaçlarla oluşturduk:
...
4. RAILWISE ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
