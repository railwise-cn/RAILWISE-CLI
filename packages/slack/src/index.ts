...
import { createRailwise, type ToolPart } from "@railwise/sdk"
...
console.log("🚀 Starting railwise server...")
const railwise = await createRailwise({
  port: 0,
})
console.log("✅ Railwise server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
;(async () => {
  const events = await railwise.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new railwise session...")
    const { client, server } = railwise
    ...
    console.log("✅ Created railwise session:", createResult.data.id)
  ...
  console.log("📝 Sending to railwise:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Railwise response:", JSON.stringify(result, null, 2))
