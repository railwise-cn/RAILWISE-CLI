...
Delingsfunksjonen i RAILWISE (甬算) lar deg lage offentlige lenker til dine RAILWISE (甬算)-samtaler, slik at du kan samarbeide med lagkamerater eller få hjelp fra andre.
...
Som standard bruker RAILWISE (甬算) manuell delingsmodus. Økter deles ikke automatisk, men du kan dele dem manuelt ved å bruke kommandoen `/share`:
...
For å håndheve dette på tvers av teamet ditt for et gitt prosjekt, legg det til `railwise.json` i prosjektet ditt og sjekk det inn i Git.
...
[Finn ut mer](/docs/enterprise) om bruk av railwise i organisasjonen din.
