...
- **macOS/Linux**: `~/.config/opencode/yonsoon.jsonc`（或 `~/.config/opencode/yonsoon.json`）
- **macOS/Linux**（舊版安裝）: `~/.local/share/opencode/yonsoon.jsonc`
- **Windows**: 按 `WIN+R` 並貼上 `%USERPROFILE%\.config\opencode\yonsoon.jsonc`
