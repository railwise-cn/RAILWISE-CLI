...
Las habilidades del agente permiten a YONSOON (甬算) descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
YONSOON (甬算) busca estas ubicaciones:

- Configuración del proyecto: `.yonsoon/skills/<name>/SKILL.md`
- Configuración global: `~/.config/yonsoon/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, YONSOON (甬算) sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.yonsoon/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/yonsoon/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.yonsoon/skills/git-release/SKILL.md` así:
...
YONSOON (甬算) enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `yonsoon.json`:
...
**Para agentes integrados** (en `yonsoon.json`):
