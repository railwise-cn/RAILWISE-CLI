...
const modelsSnapshot = path.join(dir, "src/provider/models-snapshot.ts")

async function models() {
  if (process.env.MODELS_DEV_API_JSON) return Bun.file(process.env.MODELS_DEV_API_JSON).text()

  const response = await fetch(`${modelsUrl}/api.json`).catch((error) => error)
  if (response instanceof Response) {
    if (response.ok) return response.text()
    throw new Error(`Failed to fetch ${modelsUrl}/api.json: ${response.status} ${response.statusText}`)
  }

  if (await Bun.file(modelsSnapshot).exists()) {
    console.warn(`Unable to refresh models.dev snapshot; reusing ${path.relative(dir, modelsSnapshot)}`)
    return undefined
  }
...
const modelsData = await models()
if (modelsData) {
  await Bun.write(modelsSnapshot, `// Auto-generated by build.ts - do not edit\nexport const snapshot = ${modelsData} as const\n`)
