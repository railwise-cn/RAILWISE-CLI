...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall yonsoon and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result =
        method === "choco"
          ? await $`echo Y | choco uninstall yonsoon -y -r`.quiet().nothrow()
  ...
  if (method === "curl" && targets.binary) {
    ...
    if (binDir.includes(".yonsoon") || binDir.includes(".opencode")) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using YONSOON!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# yonsoon") || content.includes(".yonsoon/bin") || content.includes("# opencode") || content.includes(".opencode/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (trimmed === "# yonsoon" || trimmed === "# opencode") {
      skip = true
      continue
    }
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".yonsoon/bin") || trimmed.includes(".opencode/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && (trimmed.includes(".yonsoon/bin") || trimmed.includes(".opencode/bin"))) ||
      (trimmed.startsWith("fish_add_path") && (trimmed.includes(".yonsoon") || trimmed.includes(".opencode")))
    ) {
      continue
    }
