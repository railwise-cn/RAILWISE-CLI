...
const TUI_SCHEMA_URL = "https://railwise.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from railwise.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const railwise = await railwiseFiles(input)
  for (const file of railwise) {
...
async function railwiseFiles(input: { directories: string[]; managed: string }) {
  const project = Flag.RAILWISE_DISABLE_PROJECT_CONFIG
    ? []
    : await ConfigPaths.projectFiles("railwise", Instance.directory, Instance.worktree)
  const files = [...project, ...ConfigPaths.fileInDirectory(Global.Path.config, "railwise")]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "railwise"))
  }
  if (Flag.RAILWISE_CONFIG) files.push(Flag.RAILWISE_CONFIG)
  files.push(...ConfigPaths.fileInDirectory(input.managed, "railwise"))
