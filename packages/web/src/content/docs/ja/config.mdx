---
title: 設定
description: RAILWISE (甬算) JSON 設定を使用します。
---

JSON 設定ファイルを使用して RAILWISE (甬算) を構成できます。

---

## 形式

RAILWISE (甬算) は、**JSON** と **JSONC** (コメント付きの JSON) 形式の両方をサポートしています。
...
たとえば、グローバル設定で `theme: "railwise"` と `autoupdate: true` が設定され、プロジェクト設定で `model: "anthropic/claude-sonnet-4-5"` が設定されている場合、最終的な設定には 3 つの設定がすべて含まれます。
...
グローバル RAILWISE (甬算) 設定を `~/.config/railwise/railwise.json` に配置します。テーマ、プロバイダー、キーバインドなどのユーザー全体の設定にはグローバル設定を使用します。
...
RAILWISE (甬算) が起動すると、現在のディレクトリで設定ファイルを検索するか、最も近い Git ディレクトリまで移動します。
...
設定ファイルには、[**`railwise.ai/config.json`**](https://railwise.ai/config.json) を使用します。
...
- `mdns` - mDNS サービス検出を有効にします。これにより、ネットワーク上の他のデバイスが RAILWISE (甬算) サーバーを検出できるようになります。
...
`provider`、`model`、および `small_model` オプションを使用して、RAILWISE (甬算) 設定で使用するプロバイダーとモデルを構成できます。
...
`small_model` オプションは、タイトル生成などの軽量タスク用に別のモデルを構成します。デフォルトでは、RAILWISE (甬算) は、プロバイダーから安価なモデルが入手可能な場合は、より安価なモデルを使用しようとします。そうでない場合は、メインモデルにフォールバックします。
...
`theme` オプションを使用して、RAILWISE (甬算) 設定で使用するテーマを構成できます。
...
デフォルトのエージェントはプライマリエージェントである必要があります (サブエージェントではありません)。これは、`"build"` や `"plan"` のような組み込みエージェント、または定義した[カスタムエージェント](/docs/agents) にすることができます。指定されたエージェントが存在しないか、サブエージェントである場合、RAILWISE (甬算) は警告とともに `"build"` にフォールバックします。
...
RAILWISE (甬算) は起動時に新しいアップデートを自動的にダウンロードします。 `autoupdate` オプションを使用してこれを無効にできます。
...
[Plugins](/docs/plugins) は、カスタムツール、フック、統合を使用して RAILWISE (甬算) を拡張します。
...
これは、RAILWISE (甬算) を 1 つずつ無効にするのではなく、特定のプロバイダーのみを使用するように制限したい場合に便利です。
