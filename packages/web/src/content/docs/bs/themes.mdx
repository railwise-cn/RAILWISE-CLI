...
U YONSOON (甬算) mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, YONSOON (甬算) koristi nasu `yonsoon` temu.
...
YONSOON (甬算) dolazi sa vise ugradenih tema.
...
- Zele da YONSOON (甬算) odgovara izgledu njihovog terminala
...
YONSOON (甬算) podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/yonsoon/themes/*.json` ili `$XDG_CONFIG_HOME/yonsoon/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.yonsoon/themes/*.json`
4. **Trenutni radni direktorij** - `./.yonsoon/themes/*.json`
