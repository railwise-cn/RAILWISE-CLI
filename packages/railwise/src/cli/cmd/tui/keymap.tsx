...
export const RAILWISE_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const RAILWISE_MODE_KEY = "railwise.mode"
...
export function createOpencodeModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(RAILWISE_MODE_KEY, RAILWISE_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(RAILWISE_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(RAILWISE_MODE_KEY, stack.at(-1)?.mode ?? RAILWISE_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? RAILWISE_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(RAILWISE_MODE_KEY, undefined)
