...
`.yonsoon/commands/test.md` を作成します。
...
カスタムコマンドは、YONSOON (甬算) 設定を通じて、または `commands/` ディレクトリに Markdown ファイルを作成することによって追加できます。

---

### JSON

YONSOON (甬算) [設定](/docs/config)で `command` オプションを使用します:
...
- グローバル: `~/.config/yonsoon/commands/`
- プロジェクトごと: `.yonsoon/commands/`
...
YONSOON (甬算) には、`/init`、`/undo`、`/redo`、`/share`、`/help` などのいくつかの組み込みコマンドが含まれています。 [詳細はこちら](/docs/tui#commands)。
