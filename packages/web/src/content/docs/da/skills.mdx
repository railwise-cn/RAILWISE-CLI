...
Agentferdigheter lar RAILWISE (甬算) oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
RAILWISE (甬算) søker etter disse stedene:

- Prosjektkonfigurasjon: `.railwise/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/railwise/skills/<name>/SKILL.md`
...
For prosjektlokale stier går RAILWISE (甬算) opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.railwise/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/railwise/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.railwise/skills/git-release/SKILL.md` slik:
...
RAILWISE (甬算) viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `railwise.json`:
...
**For innebygde agenter** (i `railwise.json`):
