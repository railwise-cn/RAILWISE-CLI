...
import type { NamedError } from "@railwise/util/error"
...
describe("session.retry.retryable", () => {
  test("stops after five retries", () => {
    const error = apiError()
    expect(SessionRetry.retryable(error, SessionRetry.RETRY_MAX_RETRIES - 1)).toBe("boom")
    expect(SessionRetry.retryable(error, SessionRetry.RETRY_MAX_RETRIES)).toBeUndefined()
  })
