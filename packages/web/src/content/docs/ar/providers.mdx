---
title: المزوّدون
description: استخدام أي موفّر LLM في RAILWISE (甬算).
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم RAILWISE (甬算) ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات RAILWISE (甬算).
...
يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات RAILWISE (甬算).
...
RAILWISE (甬算) Zen هي قائمة نماذج يوفّرها فريق RAILWISE (甬算) وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع RAILWISE (甬算). [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع RAILWISE (甬算) Zen.
:::
...
يعمل مثل أي مزوّد آخر في RAILWISE (甬算) واستخدامه اختياري بالكامل.
...
لاستخدام Amazon Bedrock مع RAILWISE (甬算):
...
:::info
استخدام اشتراك Claude Pro/Max داخل RAILWISE (甬算) غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
:::note[ملاحظة امتثال]
يستخدم RAILWISE (甬算) نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر RAILWISE (甬算)
...
لاستخدام Google Vertex AI مع RAILWISE (甬算):
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [railwise-helicone-session](https://github.com/H2Shami/railwise-helicone-session) لتسجيل كل محادثة RAILWISE (甬算) تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة RAILWISE (甬算) مدرجة كجلسة مستقلة.
...
:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع RAILWISE (甬算). راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/railwise) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع RAILWISE (甬算):
...
3. انسخ مفتاح API لاستخدامه في RAILWISE (甬算).
...
6. **مهم**: قبل استخدام نماذج السحابة في RAILWISE (甬算)، يجب سحب معلومات النموذج محليا:
...
RAILWISE (甬算) Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق RAILWISE (甬算). [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>RAILWISE (甬算) Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **RAILWISE (甬算) Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ RAILWISE (甬算).
...
تسمح حقول `limit` لـ RAILWISE (甬算) بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
