...
RAILWISE (甬算) has a list of keybinds that you can customize through the RAILWISE (甬算) config.
...
RAILWISE (甬算) uses a `leader` key for most keybinds. This avoids conflicts in your terminal.
...
The RAILWISE (甬算) desktop app prompt input supports common Readline/Emacs-style shortcuts for editing text. These are built-in and currently not configurable via `railwise.json`.
