...
Las herramientas permiten que LLM realice acciones en su código base. RAILWISE (甬算) viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en RAILWISE (甬算).
...
:::note
Esta herramienta solo está disponible cuando `RAILWISE_EXPERIMENTAL_LSP_TOOL=true` (o `RAILWISE_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor RAILWISE (甬算) o cuando la variable de entorno `RAILWISE_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar RAILWISE (甬算):

```bash
RAILWISE_ENABLE_EXA=1 railwise
```
