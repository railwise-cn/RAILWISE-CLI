...
Možete dodati vanjske alate u YONSOON (甬算) koristeći _Model Context Protocol_, ili MCP. YONSOON (甬算) podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [YONSOON (甬算) Config](https://yonsoon.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/yonsoon`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
YONSOON (甬算) automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, YONSOON (甬算) će:
...
Ako server zahtijeva autentifikaciju, YONSOON (甬算) će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `yonsoon mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, YONSOON (甬算) sigurno čuva tokene u `~/.local/share/yonsoon/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u YONSOON (甬算), zajedno s ugrađenim alatima. Možete njima upravljati kroz YONSOON (甬算) konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete YONSOON (甬算) sa vašim Sentry nalogom.
