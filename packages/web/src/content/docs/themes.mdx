...
With RAILWISE (甬算) you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, RAILWISE (甬算) uses our own `railwise` theme.
...
RAILWISE (甬算) comes with several built-in themes.
...
- Want RAILWISE (甬算) to match their terminal's appearance
...
RAILWISE (甬算) supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/railwise/themes/*.json` or `$XDG_CONFIG_HOME/railwise/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.railwise/themes/*.json`
4. **Current working directory** - Defined in `./.railwise/themes/*.json`
