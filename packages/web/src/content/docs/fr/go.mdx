...
RAILWISE Go est un abonnement à bas coût — **5 $ pour votre premier mois**, puis **10 $/mois** — qui vous donne un accès fiable aux modèles de codage ouverts populaires.

Go fonctionne comme n'importe quel autre fournisseur dans RAILWISE. Vous vous abonnez à RAILWISE Go et obtenez votre clé d'API. C'est **totalement facultatif** et vous n'avez pas besoin de l'utiliser pour utiliser RAILWISE.
...
RAILWISE Go vous donne accès à ces modèles pour **5 $ pour votre premier mois**, puis **10 $/mois**.

---

## Comment ça marche

RAILWISE Go fonctionne comme n'importe quel autre fournisseur dans RAILWISE.

1. Vous vous connectez à **<a href={console}>RAILWISE Zen</a>**, vous vous abonnez à Go et copiez votre clé d'API.
2. Vous exécutez la commande `/connect` dans la TUI, sélectionnez `RAILWISE Go` et collez votre clé d'API.
3. Exécutez `/models` dans la TUI pour voir la liste des modèles disponibles via Go.

:::note
Un seul membre par espace de travail peut s'abonner à RAILWISE Go.
:::
...
RAILWISE Go inclut les limites suivantes :

- **Limite de 5 heures** — 12 $ d'utilisation
- **Limite hebdomadaire** — 30 $ d'utilisation
- **Limite mensuelle** — 60 $ d'utilisation

Les limites sont définies en valeur monétaire (dollars). Cela signifie que votre nombre réel de requêtes dépend du modèle que vous utilisez. Les modèles moins chers comme DeepSeek V4 Flash permettent plus de requêtes, tandis que les modèles plus coûteux comme GLM-5.1 en permettent moins.
...
- MiniMax M3 — 810 tokens en entrée, 62,000 en cache, 225 tokens en sortie par requête
...
| ----------------- | ----------------- | ------------------------------------------------ | --------------------------- |
| GLM-5.1           | glm-5.1           | `https://railwise.ai/zen/go/v1/chat/completions` | `@ai-sdk/openai-compatible` |
| GLM-5             | glm-5             | `https://railwise.ai/zen/go/v1/chat/completions` | `@ai-sdk/openai-compatible` |
| Kimi K2.5         | kimi-k2.5         | `https://railwise.ai/zen/go/v1/chat/completions` | `@ai-sdk/openai-compatible` |
| Kimi K2.6         | kimi-k2.6         | `https://railwise.ai/zen/go/v1/chat/completions` | `@ai-sdk/openai-compatible` |
| DeepSeek V4 Pro   | deepseek-v4-pro   | `https://railwise.ai/zen/go/v1/chat/completions` | `@ai-sdk/openai-compatible` |
| DeepSeek V4 Flash | deepseek-v4-flash | `https://railwise.ai/zen/go/v1/chat/completions` | `@ai-sdk/openai-compatible` |
| MiMo-V2.5         | mimo-v2.5         | `https://railwise.ai/zen/go/v1/chat/completions` | `@ai-sdk/openai-compatible` |
| MiMo-V2.5-Pro     | mimo-v2.5-pro     | `https://railwise.ai/zen/go/v1/chat/completions` | `@ai-sdk/openai-compatible` |
| MiniMax M3        | minimax-m3        | `https://railwise.ai/zen/go/v1/messages`         | `@ai-sdk/anthropic`         |
| MiniMax M2.7      | minimax-m2.7      | `https://railwise.ai/zen/go/v1/messages`         | `@ai-sdk/anthropic`         |
| MiniMax M2.5      | minimax-m2.5      | `https://railwise.ai/zen/go/v1/messages`         | `@ai-sdk/anthropic`         |
| Qwen3.7 Max       | qwen3.7-max       | `https://railwise.ai/zen/go/v1/messages`         | `@ai-sdk/anthropic`         |
| Qwen3.7 Plus      | qwen3.7-plus      | `https://railwise.ai/zen/go/v1/messages`         | `@ai-sdk/anthropic`         |
| Qwen3.6 Plus      | qwen3.6-plus      | `https://railwise.ai/zen/go/v1/messages`         | `@ai-sdk/anthropic`         |

L'[ID de modèle](/docs/config/#models) dans votre configuration RAILWISE utilise le format `railwise-go/<model-id>`. Par exemple, pour Kimi K2.6, vous utiliseriez `railwise-go/kimi-k2.6` dans votre configuration.
...
```
https://railwise.ai/zen/go/v1/models
```
...
Nous avons créé RAILWISE Go pour :
...
4. N'avoir **aucun verrouillage exclusif** en vous permettant d'utiliser n'importe quel autre fournisseur avec RAILWISE également.
