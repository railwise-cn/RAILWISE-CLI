...
Create `.railwise/commands/test.md`:
...
You can add custom commands through the RAILWISE (甬算) config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your RAILWISE (甬算) [config](/docs/config):
...
railwise includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
