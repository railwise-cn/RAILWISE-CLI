...
Replace runtime Bun shell template-tag usage in `packages/railwise/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/railwise/src` is removed except:
