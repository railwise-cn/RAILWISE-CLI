...
const repo = process.env.GH_REPO ?? "anomalyco/railwise"
const bot = ["actions-user", "github-actions[bot]", "railwise", "railwise-agent[bot]"]
const team = [
...
async function commits(from: string, to: string) {
  ...
  const log =
    await $`git log ${base}..${head} --format=%H -- packages/railwise packages/sdk packages/plugin packages/desktop packages/app sdks/vscode packages/extensions github`.text()
