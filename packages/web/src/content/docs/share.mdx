---
title: Share
description: Share your RAILWISE (甬算) conversations.
---

RAILWISE (甬算)'s share feature allows you to create public links to your RAILWISE (甬算) conversations, so you can collaborate with teammates or get help from others.
...
When you share a conversation, RAILWISE (甬算):
...
RAILWISE (甬算) supports three sharing modes that control how conversations are shared:

---

### Manual (default)

By default, RAILWISE (甬算) uses manual sharing mode. Sessions are not shared automatically, but you can manually share them using the `/share` command:
...
To enforce this across your team for a given project, add it to the `railwise.json` in your project and check into Git.
...
[Learn more](/docs/enterprise) about using railwise in your organization.
