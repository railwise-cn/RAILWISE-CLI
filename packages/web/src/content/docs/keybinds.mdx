...
YONSOON (甬算) has a list of keybinds that you can customize through the YONSOON (甬算) config.
...
YONSOON (甬算) uses a `leader` key for most keybinds. This avoids conflicts in your terminal.
...
The YONSOON (甬算) desktop app prompt input supports common Readline/Emacs-style shortcuts for editing text. These are built-in and currently not configurable via `yonsoon.json`.
