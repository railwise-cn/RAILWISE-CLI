...
Puoi aggiungere strumenti esterni a RAILWISE (甬算) usando il _Model Context Protocol_, o MCP. RAILWISE (甬算) supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [RAILWISE (甬算) Config](https://railwise.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/railwise`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
RAILWISE (甬算) gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, RAILWISE (甬算):
...
Se il server richiede autenticazione, RAILWISE (甬算) ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `railwise mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, RAILWISE (甬算) memorizzera' i token in modo sicuro in `~/.local/share/railwise/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in RAILWISE (甬算) insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione RAILWISE (甬算) come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare RAILWISE (甬算) al tuo account Sentry.
