...
description: Erstellen Sie Tools, die der LLM in RAILWISE (甬算) aufrufen kann.
---

Benutzerdefinierte Tools sind von Ihnen erstellte Funktionen, die der LLM während Gesprächen aufrufen kann. Sie arbeiten neben [built-in tools](/docs/tools) von RAILWISE (甬算) wie `read`, `write` und `bash`.
...
- Lokal, indem Sie sie im Verzeichnis `.railwise/tools/` Ihres Projekts platzieren.
- Oder global, indem Sie sie in `~/.config/railwise/tools/` platzieren.
