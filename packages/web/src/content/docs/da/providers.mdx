---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i YONSOON (甬算).
---

import config from "../../../../config.mjs"
export const console = config.console

YONSOON (甬算) bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i YONSOON (甬算)-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/yonsoon/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i YONSOON (甬算)-konfigurationen.
...
YONSOON (甬算) Zen er en liste over modeller leveret af YONSOON (甬算)-teamet, som er blevet testet og verificeret til at fungere godt med YONSOON (甬算). [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med YONSOON (甬算) Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg yonsoon og gå til [yonsoon.ai/auth](https://yonsoon.ai/auth).
...
Det fungerer som alle andre udbydere i YONSOON (甬算) og er helt valgfrit at bruge.
...
Sådan bruger du Amazon Bedrock med YONSOON (甬算):
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører yonsoon:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `yonsoon.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i YONSOON (甬算) understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at yonsoon skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem yonsoon-konfigurationen.
...
gemme tokenet i yonsoon auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
YONSOON (甬算) bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse YONSOON (甬算)
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`yonsoon.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Sådan bruger du GitHub Copilot-abonnementet med YONSOON (甬算):
...
Sådan bruger du Google Vertex AI med YONSOON (甬算):
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem yonsoon, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [yonsoon-helicone-session](https://github.com/H2Shami/yonsoon-helicone-session) plugin for automatisk at logge hver YONSOON (甬算)-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver YONSOON (甬算)-samtale opført som en separat session.
...
Du kan konfigurere yonsoon til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere yonsoon til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere yonsoon til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for YONSOON (甬算). Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/yonsoon) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med YONSOON (甬算):
...
3. Kopier API-nøglen til brug i YONSOON (甬算).
...
6. **Vigtigt**: Før du bruger skymodeller i YONSOON (甬算), skal du hente modelinformationen lokalt:
...
YONSOON (甬算) Zen er en liste over testede og verificerede modeller leveret af YONSOON (甬算)-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>YONSOON (甬算) Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **YONSOON (甬算) Zen**.

   ```txt
   /connect
   ```

3. Indtast YONSOON (甬算) API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem yonsoon-konfigurationen.
...
5. Du kan også tilpasse dem gennem yonsoon-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med YONSOON (甬算):
...
Du kan også tilpasse modeller gennem yonsoon-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med yonsoon. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `yonsoon.json`-filen i projektkataloget:
...
`limit`-felterne lader YONSOON (甬算) forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `yonsoon auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek yonsoon-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i yonsoon-konfigurationen.
