...
import { createYonsoon, type ToolPart } from "@yonsoon/sdk"
...
console.log("🚀 Starting yonsoon server...")
const yonsoon = await createYonsoon({
  port: 0,
})
console.log("✅ Yonsoon server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
;(async () => {
  const events = await yonsoon.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new yonsoon session...")
    const { client, server } = yonsoon
    ...
    console.log("✅ Created yonsoon session:", createResult.data.id)
  ...
  console.log("📝 Sending to yonsoon:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Yonsoon response:", JSON.stringify(result, null, 2))
