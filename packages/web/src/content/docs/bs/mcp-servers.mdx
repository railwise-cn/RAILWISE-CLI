...
Možete dodati vanjske alate u RAILWISE (甬算) koristeći _Model Context Protocol_, ili MCP. RAILWISE (甬算) podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [RAILWISE (甬算) Config](https://railwise.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/railwise`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
RAILWISE (甬算) automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, RAILWISE (甬算) će:
...
Ako server zahtijeva autentifikaciju, RAILWISE (甬算) će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `railwise mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, RAILWISE (甬算) sigurno čuva tokene u `~/.local/share/railwise/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u RAILWISE (甬算), zajedno s ugrađenim alatima. Možete njima upravljati kroz RAILWISE (甬算) konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete RAILWISE (甬算) sa vašim Sentry nalogom.
