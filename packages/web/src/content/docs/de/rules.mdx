---
title: Regeln
description: Lege eigene Anweisungen fuer railwise fest.
---

Du kannst railwise ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in railwise den Befehl `/init` aus.
...
So kann railwise sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht railwise sie zu erweitern.
...
railwise kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/railwise/AGENTS.md` speichern.
Diese gelten in allen railwise-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt RAILWISE (甬算) dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/railwise/AGENTS.md` existiert)
...
Beim Start sucht railwise in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/railwise/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `railwise.json` oder in `~/.config/railwise/railwise.json` angeben.
...
railwise parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit railwise.json

Empfohlen ist das `instructions`-Feld in der `railwise.json`:
...
Du kannst railwise explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `railwise.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
