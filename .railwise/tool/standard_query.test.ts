...
describe("query_standard", () => {
  test("finds clauses by keyword '报警值'", async () => {
    const r = parse(
      await query_standard.execute({ keywords: ["报警值"], standardCode: "all", mandatoryOnly: false }, ctx),
    )
  ...
  test("filters by specific standard code", async () => {
    const r = parse(
      await query_standard.execute({ keywords: ["监测频率"], standardCode: "GB 50911", mandatoryOnly: false }, ctx),
    )
  ...
  test("returns empty for unrelated keywords", async () => {
    const r = parse(
      await query_standard.execute(
        { keywords: ["量子计算", "火星探测"], standardCode: "all", mandatoryOnly: false },
        ctx,
      ),
    )
  ...
  test("multiple keywords improve relevance", async () => {
    const r = parse(
      await query_standard.execute(
        { keywords: ["基坑", "围护墙", "水平位移"], standardCode: "all", mandatoryOnly: false },
        ctx,
      ),
    )
    expect(r.total_matches).toBeGreaterThan(0)
  ...
  test("section number search returns exact match", async () => {
    const r = parse(
      await query_standard.execute({ keywords: ["5.0.1"], standardCode: "GB 50497", mandatoryOnly: false }, ctx),
    )
  ...
  test("returns at most 8 results", async () => {
    const r = parse(
      await query_standard.execute({ keywords: ["监测"], standardCode: "all", mandatoryOnly: false }, ctx),
    )
  ...
  test("results are sorted by relevance (descending)", async () => {
    const r = parse(
      await query_standard.execute({ keywords: ["水准", "闭合差"], standardCode: "all", mandatoryOnly: false }, ctx),
    )
  ...
  test("JGJ 8 clauses are searchable", async () => {
    const r = parse(
      await query_standard.execute(
        { keywords: ["倾斜", "建筑高度"], standardCode: "JGJ 8", mandatoryOnly: false },
        ctx,
      ),
    )
    expect(r.total_matches).toBeGreaterThan(0)
  ...
  test("GB 50026 导线 clauses", async () => {
    const r = parse(
      await query_standard.execute(
        { keywords: ["导线", "角度闭合差"], standardCode: "GB 50026", mandatoryOnly: false },
        ctx,
      ),
    )
    expect(r.total_matches).toBeGreaterThan(0)
