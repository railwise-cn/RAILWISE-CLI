---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i RAILWISE.
---

import config from "../../../../config.mjs"
export const console = config.console

RAILWISE bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i RAILWISE-konfigurasjonen.
...
i `~/.local/share/railwise/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i RAILWISE
konfig.
...
RAILWISE Zen er en liste over modeller levert av RAILWISE-teamet som har vært
testet og verifisert for å fungere godt med RAILWISE. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med RAILWISE Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `RAILWISE Zen` og gå til [railwise.ai/auth](https://railwise.ai/zen).
...
Det fungerer som alle andre leverandører i RAILWISE og er helt valgfritt å bruke.

---

## RAILWISE Go

RAILWISE Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av RAILWISE-teamet som har vært
testet og verifisert for å fungere godt med RAILWISE.

1. Kjør kommandoen `/connect` i TUI, velg `RAILWISE Go`, og gå til [railwise.ai/auth](https://railwise.ai/zen).
...
Slik bruker du Amazon Bedrock med RAILWISE:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører RAILWISE:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `railwise.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i railwise støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere railwise til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at RAILWISE skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom RAILWISE-konfigurasjonen.
...
for å lagre token i RAILWISE auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
RAILWISE bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse RAILWISE
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`railwise.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [railwise-gitlab-auth](https://www.npmjs.com/package/railwise-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `railwise.json`:
...
Slik bruker du GitHub Copilot-abonnementet med RAILWISE:
...
Slik bruker du Google Vertex AI med RAILWISE:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører RAILWISE.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom RAILWISE, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [railwise-helicone-session](https://github.com/H2Shami/railwise-helicone-session) plugin for automatisk å logge hver RAILWISE-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver RAILWISE-samtale oppført som en separat økt.
...
Du kan konfigurere RAILWISE for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere RAILWISE for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere RAILWISE for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for RAILWISE. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/railwise) for detaljer.
:::
...
Slik bruker du Ollama Cloud med RAILWISE:
...
3. Kopier API-nøkkelen for bruk i RAILWISE.
...
6. **Viktig**: Før du bruker skymodeller i RAILWISE, må du hente modellinformasjonen lokalt:
...
RAILWISE Zen er en liste over testede og verifiserte modeller levert av RAILWISE-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>RAILWISE Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **RAILWISE Zen**.

   ```txt
   /connect
   ```

3. Skriv inn RAILWISE API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom RAILWISE-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom RAILWISE-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med RAILWISE:
...
Du kan også tilpasse modeller gjennom RAILWISE-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med RAILWISE. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `railwise.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar RAILWISE forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `railwise auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk RAILWISE-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i RAILWISE-konfigurasjonen.
