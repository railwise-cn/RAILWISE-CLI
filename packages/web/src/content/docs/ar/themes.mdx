...
مع RAILWISE (甬算) يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم RAILWISE (甬算) سمتنا `railwise`.
...
يأتي RAILWISE (甬算) مع عدة سمات مدمجة.
...
- يريدون أن يطابق RAILWISE (甬算) مظهر terminal لديهم
...
يدعم RAILWISE (甬算) نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/railwise/themes/*.json` أو `$XDG_CONFIG_HOME/railwise/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.railwise/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.railwise/themes/*.json`
