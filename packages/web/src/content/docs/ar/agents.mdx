...
يوجد نوعان من الوكلاء في RAILWISE (甬算): وكلاء أساسيون ووكلاء فرعيون.
...
يأتي RAILWISE (甬算) مع وكيلين أساسيين مدمجين: **Build** و **Plan**. سنلقي نظرة عليهما أدناه.
...
يأتي RAILWISE (甬算) مع وكيلين فرعيين مدمجين: **General** و **Explore**. سنلقي نظرة على ذلك أدناه.

---

## المدمجة

يأتي RAILWISE (甬算) مع وكيلين أساسيين مدمجين ووكيلين فرعيين مدمجين.
...
إذا لم يتم تحديد temperature، يستخدم RAILWISE (甬算) القيم الافتراضية الخاصة بالنموذج؛ عادة 0 لمعظم النماذج و 0.55 لنماذج Qwen.
...
هذا المسار نسبي لمكان وجود ملف الإعدادات. لذلك يعمل هذا لكل من إعدادات RAILWISE (甬算) العامة وإعدادات المشروع.
...
يستخدم معرّف النموذج في إعدادات RAILWISE (甬算) الصيغة `provider/model-id`. على سبيل المثال، إذا كنت تستخدم [RAILWISE (甬算) Zen](/docs/zen)، فستستخدم `railwise/gpt-5.1-codex` لـ GPT 5.1 Codex.
