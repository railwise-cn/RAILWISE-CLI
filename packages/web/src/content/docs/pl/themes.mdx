...
Dzięki yonsoon możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie yonsoon używa naszego własnego motywu `yonsoon`.
...
yonsoon ma kilka wbudowanych motywów.
...
- Want yonsoon to match their terminal's appearance
...
yonsoon obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/yonsoon/themes/*.json` lub `$XDG_CONFIG_HOME/yonsoon/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.yonsoon/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.yonsoon/themes/*.json`
