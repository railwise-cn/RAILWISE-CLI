---
title: الخادم
description: تفاعل مع خادم yonsoon عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `yonsoon serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل yonsoon استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `yonsoon.local` |
...
عيّن `YONSOON_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `yonsoon`، أو عيّن `YONSOON_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `yonsoon serve` و `yonsoon web`.
...
عند تشغيل `yonsoon` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم yonsoon للتفاعل مع yonsoon برمجيا.
:::

تتيح هذه البنية لـ yonsoon دعم عدة عملاء وتمكّنك من التفاعل مع yonsoon برمجيا.

يمكنك تشغيل `yonsoon serve` لبدء خادم مستقل. إذا كانت واجهة yonsoon في terminal (TUI)
قيد التشغيل، فسيبدأ `yonsoon serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات YONSOON (甬算) لـ [IDE](/docs/ide).
...
يعرض خادم yonsoon واجهات API التالية.
