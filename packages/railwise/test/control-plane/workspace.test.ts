...
import { EventSequenceTable } from "@railwise/core/event/sql"
...
import { WorkspaceTable } from "@railwise/core/control-plane/workspace.sql"
...
const originalEnv = {
  RAILWISE_AUTH_CONTENT: process.env.RAILWISE_AUTH_CONTENT,
  RAILWISE_EXPERIMENTAL_WORKSPACES: process.env.RAILWISE_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.RAILWISE_EXPERIMENTAL_WORKSPACES = "true"
})
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.RAILWISE_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=railwise-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.RAILWISE_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.RAILWISE_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.RAILWISE_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=railwise-test")
