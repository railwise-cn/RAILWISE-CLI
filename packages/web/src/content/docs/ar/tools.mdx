...
تتيح الأدوات لـ LLM تنفيذ إجراءات داخل قاعدة الشفرة الخاصة بك. يأتي YONSOON (甬算) مع مجموعة من الأدوات المدمجة، لكن يمكنك توسيعه عبر [أدوات مخصصة](/docs/custom-tools) أو [خوادم MCP](/docs/mcp-servers).
...
فيما يلي جميع الأدوات المدمجة المتاحة في YONSOON (甬算).
...
:::note
هذه الأداة متاحة فقط عند ضبط `YONSOON_EXPERIMENTAL_LSP_TOOL=true` (أو `YONSOON_EXPERIMENTAL=true`).
:::
...
:::note
هذه الأداة متاحة فقط عند استخدام مزود YONSOON (甬算) أو عند ضبط متغير البيئة `YONSOON_ENABLE_EXA` على أي قيمة منطقية صحيحة (مثل `true` أو `1`).

للتفعيل عند تشغيل YONSOON (甬算):
