...
Vous pouvez ajouter des outils externes à YONSOON (甬算) à l'aide du _Model Context Protocol_ ou MCP. YONSOON (甬算) prend en charge les serveurs locaux et distants.
...
Vous pouvez définir des serveurs MCP dans votre [YONSOON (甬算) Config](https://yonsoon.ai/docs/config/) sous `mcp`. Ajoutez chaque MCP avec un nom unique. Vous pouvez faire référence à ce MCP par son nom lorsque vous demandez le LLM.
...
Les organisations peuvent fournir des serveurs MCP par défaut via leur point de terminaison `.well-known/yonsoon`. Ces serveurs peuvent être désactivés par défaut, permettant aux utilisateurs de choisir ceux dont ils ont besoin.
...
YONSOON (甬算) gère automatiquement l'authentification OAuth pour les serveurs MCP distants. Lorsqu'un serveur nécessite une authentification, YONSOON (甬算) :
...
Si le serveur nécessite une authentification, YONSOON (甬算) vous demandera de vous authentifier lorsque vous essayez de l'utiliser pour la première fois. Sinon, vous pouvez [déclencher manuellement le flux](#authenticating) avec `yonsoon mcp auth <server-name>`.
...
La commande `mcp auth` ouvrira votre navigateur pour autorisation. Après votre autorisation, YONSOON (甬算) stockera les jetons en toute sécurité dans `~/.local/share/yonsoon/mcp-auth.json`.
...
Vos MCP sont disponibles sous forme d'outils dans YONSOON (甬算), aux côtés des outils intégrés. Vous pouvez donc les gérer via la configuration YONSOON (甬算) comme n'importe quel autre outil.
...
Cela ouvrira une fenêtre de navigateur pour terminer le flux OAuth et connecter YONSOON (甬算) à votre compte Sentry.
