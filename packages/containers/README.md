...
```
REGISTRY=ghcr.io/railwise-cn TAG=24.04 bun ./packages/containers/script/build.ts
REGISTRY=ghcr.io/railwise-cn TAG=24.04 bun ./packages/containers/script/build.ts --push
```
...
jobs:
  build-cli:
    ...
    container:
      image: ghcr.io/railwise-cn/build/bun-node:24.04
```
