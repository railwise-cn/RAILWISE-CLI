...
yonsoon ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak yonsoon kendi `yonsoon` temasını kullanır.
...
yonsoon birden fazla yerleşik temayla gelir.
...
- yonsoon'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
yonsoon, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/yonsoon/themes/*.json` veya `$XDG_CONFIG_HOME/yonsoon/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.yonsoon/themes/*.json`
4. **Geçerli çalışma dizini** - `./.yonsoon/themes/*.json`
