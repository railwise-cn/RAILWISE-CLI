---
title: Servidor
description: Interaja com o servidor railwise via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `railwise serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente railwise pode usar.
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `railwise.local` |
...
Defina `RAILWISE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `railwise`, ou defina `RAILWISE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `railwise serve` quanto ao `railwise web`.

```bash
RAILWISE_SERVER_PASSWORD=your-password railwise serve
```
...
Quando você executa `railwise`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor railwise para interagir com o railwise programaticamente.
:::

Essa arquitetura permite que o railwise suporte múltiplos clientes e permite que você interaja com o railwise programaticamente.

Você pode executar `railwise serve` para iniciar um servidor autônomo. Se você tiver o TUI do railwise em execução, `railwise serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do railwise [IDE](/docs/ide).
...
O servidor railwise expõe as seguintes APIs.
