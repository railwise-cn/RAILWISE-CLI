---
title: Regeln
description: Lege eigene Anweisungen fuer yonsoon fest.
---

Du kannst yonsoon ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in yonsoon den Befehl `/init` aus.
...
So kann yonsoon sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht yonsoon sie zu erweitern.
...
yonsoon kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/yonsoon/AGENTS.md` speichern.
Diese gelten in allen yonsoon-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt YONSOON (甬算) dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/yonsoon/AGENTS.md` existiert)
...
Beim Start sucht yonsoon in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/yonsoon/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `yonsoon.json` oder in `~/.config/yonsoon/yonsoon.json` angeben.
...
yonsoon parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit yonsoon.json

Empfohlen ist das `instructions`-Feld in der `yonsoon.json`:
...
Du kannst yonsoon explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `yonsoon.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
