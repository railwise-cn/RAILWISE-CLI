---
title: Server
description: Nutze den yonsoon-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `yonsoon serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein yonsoon-Client nutzen kann.
...
Setze `YONSOON_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `yonsoon`, kann aber mit `YONSOON_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `yonsoon serve` und `yonsoon web`.
...
Wenn du `yonsoon` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den yonsoon-Server, um yonsoon programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `yonsoon serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `yonsoon serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die YONSOON (甬算)-[IDE](/docs/ide)-Plugins.
...
Der yonsoon-Server stellt folgende APIs bereit.
