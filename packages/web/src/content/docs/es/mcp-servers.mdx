...
Puede agregar herramientas externas a YONSOON (甬算) utilizando el _Model Context Protocol_ o MCP. YONSOON (甬算) admite servidores locales y remotos.
...
Puede definir servidores MCP en su [YONSOON (甬算) Config](https://yonsoon.ai/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/yonsoon`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
YONSOON (甬算) maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, YONSOON (甬算):
...
Si el servidor requiere autenticación, YONSOON (甬算) le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `yonsoon mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, YONSOON (甬算) almacenará los tokens de forma segura en `~/.local/share/yonsoon/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en YONSOON (甬算), junto con herramientas integradas. Para que puedas administrarlos a través de la configuración YONSOON (甬算) como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar YONSOON (甬算) a su cuenta Sentry.
