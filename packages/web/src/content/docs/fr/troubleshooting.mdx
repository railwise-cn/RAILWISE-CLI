...
Pour déboguer les problèmes avec RAILWISE (甬算), commencez par vérifier les journaux et les données locales qu'il stocke sur le disque.
...
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\railwise\log`
...
Vous pouvez définir le niveau de journalisation avec l'option de ligne de commande `--log-level` pour obtenir des informations de débogage plus détaillées. Par exemple, `railwise --log-level DEBUG`.

---

## Stockage

railwise stocke les données de session et autres données d'application sur le disque à l'emplacement :

- **macOS/Linux** : `~/.local/share/railwise/`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\railwise`
...
RAILWISE (甬算) Desktop exécute un serveur RAILWISE (甬算) local (le side-car `railwise-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- macOS uniquement : menu `RAILWISE (甬算)` -> **Recharger la vue Web** (aide si l'interface utilisateur est vide/gelée).
...
- **macOS/Linux** : `~/.config/railwise/railwise.jsonc` (ou `~/.config/railwise/railwise.json`)
- **macOS/Linux** (anciennes installations) : `~/.local/share/railwise/railwise.jsonc`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\railwise\railwise.jsonc`
...
RAILWISE (甬算) peut également charger des plugins locaux à partir du disque. Écartez-les temporairement (ou renommez le dossier) et redémarrez l'application de bureau :

- **Plugins mondiaux**
  - **macOS/Linux** : `~/.config/railwise/plugins/`
  - **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\railwise\plugins`
...
Si la désactivation des plugins ne résout pas le problème (ou si l'installation d'un plugin est bloquée), videz le cache afin que RAILWISE (甬算) puisse le reconstruire.

1. Quittez complètement RAILWISE (甬算) Desktop.
2. Supprimez le répertoire cache :

- **macOS** : Finder -> `Cmd+Shift+G` -> coller `~/.cache/railwise`
- **Linux** : supprimez `~/.cache/railwise` (ou exécutez `rm -rf ~/.cache/railwise`)
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.cache\railwise`

3. Redémarrez le bureau RAILWISE (甬算).

---

### Résoudre les problèmes de connexion au serveur

RAILWISE (甬算) Desktop peut soit démarrer son propre serveur local (par défaut), soit se connecter à un serveur URL que vous avez configuré.
...
Si votre `railwise.json(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.

#### Vérifier les variables d'environnement

Si `RAILWISE_PORT` est défini dans votre environnement, l'application de bureau tentera d'utiliser ce port pour le serveur local.

- Désactivez `RAILWISE_PORT` (ou choisissez un port libre) et redémarrez.
...
Sur Windows, RAILWISE (甬算) Desktop nécessite Microsoft Edge **WebView2 Runtime**. Si l'application s'ouvre sur une fenêtre vide ou ne démarre pas, installez/mettez à jour WebView2 et réessayez.

---

### Windows : problèmes de performances généraux

Si vous rencontrez des performances lentes, des problèmes d'accès aux fichiers ou des problèmes de terminal sur Windows, essayez d'utiliser [WSL (Windows Sous-système pour Linux)](/docs/windows-wsl). WSL fournit un environnement Linux qui fonctionne de manière plus transparente avec les fonctionnalités de RAILWISE (甬算).

---

### Les notifications ne s'affichent pas

RAILWISE (甬算) Desktop affiche uniquement les notifications système lorsque :

- les notifications sont activées pour RAILWISE (甬算) dans les paramètres de votre système d'exploitation, et
...
1. Quittez le bureau RAILWISE (甬算).
2. Recherchez et supprimez ces fichiers (ils se trouvent dans le répertoire de données de l'application RAILWISE (甬算) Desktop) :

- `railwise.settings.dat` (serveur de bureau par défaut URL)
- `railwise.global.dat` et `railwise.workspace.*.dat` (état de l'interface utilisateur comme les serveurs/projets récents)
...
Si vous rencontrez des problèmes avec RAILWISE (甬算) :
...
3. Assurez-vous d'avoir la dernière version avec `railwise upgrade`
...
Pour déterminer à quels modèles vous avez accès, exécutez `railwise models`
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.local\share\railwise`
...
Si vous rencontrez des erreurs d’appel API, cela peut être dû à des packages de fournisseurs obsolètes. railwise installe dynamiquement les packages du fournisseur (OpenAI, Anthropic, Google, etc.) selon les besoins et les met en cache localement.
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.cache\railwise`

2. Redémarrez railwise pour réinstaller les derniers packages du fournisseur

Cela forcera railwise à télécharger les versions les plus récentes des packages du fournisseur, ce qui résout souvent les problèmes de compatibilité avec les paramètres du modèle et les modifications de API.
...
railwise détectera si vous utilisez Wayland et préférez `wl-clipboard`, sinon il essaiera de trouver les outils du presse-papiers dans l'ordre : `xclip` et `xsel`.
