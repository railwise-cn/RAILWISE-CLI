...
Es gibt zwei Arten von Agenten in YONSOON (甬算); Primäragenten und Subagenten.
...
YONSOON (甬算) verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Konfigurieren Sie Agenten in Ihrer `yonsoon.json`-Konfigurationsdatei:
...
Wenn keine Temperatur angegeben ist, verwendet YONSOON (甬算) modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale YONSOON (甬算)-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer YONSOON (甬算)-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [YONSOON (甬算) Zen](/docs/zen) verwenden, würden Sie `yonsoon/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Führen Sie `yonsoon models` aus, um eine Liste der verfügbaren Modelle anzubieten.
:::
...
:::tip
Haben Sie einen Agenten, den Sie gerne teilen möchten? [Reichen Sie einen PR ein](https://github.com/anomalyco/yonsoon).
:::
