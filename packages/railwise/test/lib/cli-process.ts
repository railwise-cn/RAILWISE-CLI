// Subprocess test harness for the railwise CLI. Spawns the real binary against
...
//
// Configuration flows through railwise's built-in test affordances:
//   - RAILWISE_CONFIG_CONTENT      : provider config inline, no files to find
//   - RAILWISE_TEST_HOME           : pins os.homedir() → tmpdir
//   - RAILWISE_DISABLE_PROJECT_CONFIG : skip walking up for railwise.json
//   - RAILWISE_PURE                : skip external plugin discovery + install
//   - RAILWISE_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `railwise.run` is fully wired. The shape supports adding more
// builders (`railwise.serve(opts)`, `railwise.acp(opts)`, `railwise.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of OpencodeCli.
import { test, type TestOptions } from "bun:test"
import { AppFileSystem } from "@railwise/core/filesystem"
import { AppProcess } from "@railwise/core/process"
...
const railwiseRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(railwiseRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    RAILWISE_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    RAILWISE_CONFIG_CONTENT: configJson,
    RAILWISE_DISABLE_PROJECT_CONFIG: "1",
    RAILWISE_PURE: "1",
    RAILWISE_DISABLE_AUTOUPDATE: "1",
    RAILWISE_DISABLE_AUTOCOMPACT: "1",
    RAILWISE_DISABLE_MODELS_FETCH: "1",
    RAILWISE_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `railwise run`. New flags should
...
// `railwise serve` is a long-lived process — it never exits on its own.
...
// `railwise acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type OpencodeCli = {
  ...
  // Spawn `railwise serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `railwise acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("railwise.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const serve = Effect.fn("railwise.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "railwise server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `railwise serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("railwise.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const railwise: OpencodeCli = { run, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, railwise })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `railwise.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
...
function expectExit(result: RunResult, expected: number, label = "railwise") {
...
// every call site. Use this for any test that needs the railwise CLI fixture.
//
...
// resources (e.g. `railwise.serve`) without an extra `Effect.scoped` wrapper —
