...
Les compétences d'agent permettent à YONSOON (甬算) de découvrir des instructions réutilisables à partir de votre dépôt ou de votre répertoire personnel.
...
YONSOON (甬算) recherche ces emplacements :

- Configuration du projet : `.yonsoon/skills/<name>/SKILL.md`
- Configuration globale : `~/.config/yonsoon/skills/<name>/SKILL.md`
...
Pour les chemins locaux du projet, YONSOON (甬算) remonte de votre répertoire de travail actuel jusqu'à ce qu'il atteigne l'arbre de travail git.
Il charge tout `skills/*/SKILL.md` correspondant dans `.yonsoon/` et tout `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` correspondant en cours de route.

Les définitions globales sont également chargées à partir de `~/.config/yonsoon/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` et `~/.agents/skills/*/SKILL.md`.
...
Créez `.yonsoon/skills/git-release/SKILL.md` comme ceci :
...
YONSOON (甬算) répertorie les compétences disponibles dans la description de l'outil `skill`.
...
Contrôlez les compétences auxquelles les agents peuvent accéder à l'aide d'autorisations basées sur des modèles dans `yonsoon.json` :
...
**Pour les agents intégrés** (dans `yonsoon.json`) :
