...
Med RAILWISE (甬算) kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger RAILWISE (甬算) vårt eget `railwise`-tema.
...
RAILWISE (甬算) kommer med flere innebygde temaer.
...
- Vil at RAILWISE (甬算) skal matche terminalens utseende
...
RAILWISE (甬算) støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/railwise/themes/*.json` eller `$XDG_CONFIG_HOME/railwise/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.railwise/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.railwise/themes/*.json`
