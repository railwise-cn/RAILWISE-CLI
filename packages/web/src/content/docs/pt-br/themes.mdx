...
Com o railwise, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o railwise usa nosso próprio tema `railwise`.
...
O railwise vem com vários temas embutidos.
...
- Querem que o railwise corresponda à aparência do seu terminal
...
O railwise suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/railwise/themes/*.json` ou `$XDG_CONFIG_HOME/railwise/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.railwise/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.railwise/themes/*.json`
