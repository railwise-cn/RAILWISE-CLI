...
async fn initialize(app: AppHandle) {
    ...
    let sqlite_done = needs_sqlite_migration.then(|| {
        tracing::info!(
            path = %yonsoon_db_path().expect("failed to get db path").display(),
                    ...
                    Some(
                        async move {
                            ...
                            if let Some(err) = err {
                                ...
                                return Err(format!(
                                    "Failed to spawn YONSOON (甬算) Server ({err}). Logs:\n{}",
                                    get_logs()
                                ));
                            }
...
fn get_sidecar_port() -> u32 {
    option_env!("YONSOON_PORT")
        .map(|s| s.to_string())
        .or_else(|| std::env::var("YONSOON_PORT").ok())
...
fn sqlite_file_exists() -> bool {
    let Ok(path) = yonsoon_db_path() else {
        return true;
    };
...
fn yonsoon_db_path() -> Result<PathBuf, &'static str> {
    ...
    Ok(data_home.join("yonsoon").join("yonsoon.db"))
}
