...
Umiejętności agenta pozwalają yonsoon odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
yonsoon przeszukuje te lokalizacje:

- Project config: `.yonsoon/skills/<name>/SKILL.md`
- Global config: `~/.config/yonsoon/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu yonsoon przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.yonsoon/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/yonsoon/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.yonsoon/skills/git-release/SKILL.md` w ten sposób:
...
yonsoon wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `yonsoon.json`:
...
**Dla agentów wbudowanych** (w `yonsoon.json`):
