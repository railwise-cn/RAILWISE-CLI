---
title: Providers
description: Using any LLM provider in YONSOON (甬算).
---

import config from "../../../config.mjs"
export const console = config.console

YONSOON (甬算) uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your YONSOON (甬算) config.
...
You can customize the providers through the `provider` section in your YONSOON (甬算)
config.
...
YONSOON (甬算) Zen is a list of models provided by the YONSOON (甬算) team that have been
tested and verified to work well with YONSOON (甬算). [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with YONSOON (甬算) Zen.
:::
...
It works like any other provider in YONSOON (甬算) and is completely optional to use.
...
To use Amazon Bedrock with YONSOON (甬算):
...
2. **Configure authentication** using one of the following methods:
   ...
   Set one of these environment variables while running yonsoon:
   ...
   For project-specific or persistent configuration, use `yonsoon.json`:
...
:::info
Using your Claude Pro/Max subscription in YONSOON (甬算) is not officially supported by [Anthropic](https://anthropic.com).
:::
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for yonsoon to work properly.
   :::
...
5. Run the `/models` command to select a model.
   ...
   You can also add models through your yonsoon config.
...
YONSOON (甬算) uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock YONSOON (甬算)
to only use your own GitLab-hosted instance, add the following to your
`yonsoon.json` file. It is also recommended to disable session sharing.
...
To use your GitHub Copilot subscription with yonsoon:
...
To use Google Vertex AI with YONSOON (甬算):
...
In the event you see a feature or model from Helicone that isn't configured automatically through yonsoon, you can always configure it yourself.
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [yonsoon-helicone-session](https://github.com/H2Shami/yonsoon-helicone-session) plugin to automatically log each YONSOON (甬算) conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each YONSOON (甬算) conversation listed as a separate session.
...
You can configure yonsoon to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility
...
You can configure yonsoon to use local models through LM Studio.
...
You can configure yonsoon to use local models through Ollama.

:::tip
Ollama can automatically configure itself for YONSOON (甬算). See the [Ollama integration docs](https://docs.ollama.com/integrations/yonsoon) for details.
:::
...
To use Ollama Cloud with YONSOON (甬算):
...
3. Copy the API key for use in YONSOON (甬算).
...
6. **Important**: Before using cloud models in YONSOON (甬算), you must pull the model information locally:
...
YONSOON (甬算) Zen is a list of tested and verified models provided by the YONSOON (甬算) team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>YONSOON (甬算) Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **YONSOON (甬算) Zen**.

   ```txt
   /connect
   ```

3. Enter your YONSOON (甬算) API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your yonsoon config.
...
5. You can also customize them through your yonsoon config. Here's an example of specifying a provider
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with Yonsoon:
...
You can also customize models through your yonsoon config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with yonsoon. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
4. Create or update your `yonsoon.json` file in your project directory:
...
The `limit` fields allow YONSOON (甬算) to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `yonsoon auth list` to see if the credentials
...
2. For custom providers, check the yonsoon config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your yonsoon config.
