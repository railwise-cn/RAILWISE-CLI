...
export const OpencodePlugin = PluginV2.define({
  id: PluginV2.ID.make("railwise"),
  effect: Effect.gen(function* () {
    let hasKey = false
    return {
      "provider.update": Effect.fn(function* (evt) {
        if (evt.provider.id !== ProviderV2.ID.railwise) return
        hasKey = Boolean(
          process.env.RAILWISE_API_KEY ||
      ...
      "model.update": Effect.fn(function* (evt) {
        if (evt.model.providerID !== ProviderV2.ID.railwise) return
        if (hasKey) return
