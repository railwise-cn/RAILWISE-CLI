---
title: Proveedores
description: Usando cualquier proveedor LLM en YONSOON (甬算).
---

import config from "../../../../config.mjs"
export const console = config.console

YONSOON (甬算) usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración YONSOON (甬算).
...
Puedes personalizar los proveedores a través de la sección `provider` en tu YONSOON (甬算)
configuración.
...
YONSOON (甬算) Zen es una lista de modelos proporcionados por el equipo YONSOON (甬算) que han sido
probado y verificado para funcionar bien con YONSOON (甬算). [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con YONSOON (甬算) Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione yonsoon y diríjase a [yonsoon.ai/auth](https://yonsoon.ai/auth).
...
Funciona como cualquier otro proveedor en YONSOON (甬算) y su uso es completamente opcional.
...
Para usar Amazon Bedrock con YONSOON (甬算):
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta yonsoon:
   ...
   Para una configuración persistente o específica del proyecto, utilice `yonsoon.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en YONSOON (甬算) no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que yonsoon funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración yonsoon.
...
2. Elija su método de autenticación:
   ...
   3. Nombre: `YONSOON (甬算)`, Alcances: `api`
...
para almacenar el token en el almacenamiento de autenticación yonsoon.
:::

##### Autohospedado GitLab

:::note[nota de cumplimiento]
YONSOON (甬算) utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear YONSOON (甬算)
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `yonsoon.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Personalizar a través de `yonsoon.json`:
...
Para utilizar su suscripción GitHub Copilot con yonsoon:
...
Para utilizar Google Vertex AI con YONSOON (甬算):
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta yonsoon.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de yonsoon, siempre podrá configurarlo usted mismo.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación YONSOON (甬算) enumerada como una sesión separada.
...
Puede configurar yonsoon para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar yonsoon para usar modelos locales a través de LM Studio.
...
Puedes configurar yonsoon para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para YONSOON (甬算). Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/yonsoon) para obtener más detalles.
:::
...
Para usar Ollama Cloud con YONSOON (甬算):
...
3. Copie la clave API para usarla en YONSOON (甬算).
...
6. **Importante**: Antes de usar modelos en la nube en YONSOON (甬算), debe obtener la información del modelo localmente:
...
YONSOON (甬算) Zen es una lista de modelos probados y verificados proporcionada por el equipo YONSOON (甬算). [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>YONSOON (甬算) Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **YONSOON (甬算) Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave YONSOON (甬算) API.
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración yonsoon.
...
5. También puedes personalizarlos a través de tu configuración yonsoon. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con YONSOON (甬算):
...
También puede personalizar modelos a través de su configuración yonsoon. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con yonsoon. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
4. Cree o actualice su archivo `yonsoon.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a YONSOON (甬算) comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `yonsoon auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración yonsoon y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración yonsoon.
