...
export namespace Config {
  ...
  function systemManagedConfigDir(): string {
    switch (process.platform) {
      case "darwin":
        return "/Library/Application Support/railwise"
      case "win32":
        return path.join(process.env.ProgramData || "C:\\ProgramData", "railwise")
      default:
        return "/etc/railwise"
    }
  }
  ...
  export function managedConfigDir() {
    return process.env.RAILWISE_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
  }

  const managedDir = managedConfigDir()

  const MANAGED_PLIST_DOMAIN = "ai.railwise.managed"

  // Keys injected by macOS/MDM into the managed plist that are not RAILWISE config
  ...
  /**
   * Parse raw JSON (from plutil conversion of a managed plist) into RAILWISE config.
  ...
  async function loadCommand(dir: string) {
    ...
    for (const item of await Glob.scan("{command,commands}/**/*.md", {
      ...
      const patterns = ["/.railwise/command/", "/.railwise/commands/", "/command/", "/commands/"]
  ...
  async function loadAgent(dir: string) {
    ...
    for (const item of await Glob.scan("{agent,agents}/**/*.md", {
      ...
      const patterns = ["/.railwise/agent/", "/.railwise/agents/", "/agent/", "/agents/"]
      const file = rel(item, patterns) ?? path.basename(item)
  ...
  export const Server = z
    .object({
      ...
      mdnsDomain: z.string().optional().describe("Custom domain name for mDNS service (default: railwise.local)"),
  ...
  export const Info = z
    .object({
      ...
      server: Server.optional().describe("Server configuration for railwise serve and web commands"),
      command: z
        .record(z.string(), Command)
        .optional()
        .describe("Command configuration, see https://railwise.ai/docs/commands"),
      ...
      agent: z
        ...
        .catchall(Agent)
        .optional()
        .describe("Agent configuration, see https://railwise.ai/docs/agents"),
      provider: z
  ...
  export class Service extends ServiceMap.Service<Service, Interface>()("@railwise/Config") {}
      ...
      Effect.gen(function* () {
        ...
        const loadGlobal = Effect.fnUntraced(function* () {
          let result: Info = pipe(
            ...
            mergeDeep(yield* loadFile(path.join(Global.Path.config, "railwise.json"))),
            mergeDeep(yield* loadFile(path.join(Global.Path.config, "railwise.jsonc"))),
          )
        ...
        const loadInstanceState = Effect.fnUntraced(function* (ctx: InstanceContext) {
          ...
          if (Flag.RAILWISE_CONFIG) {
            merge(Flag.RAILWISE_CONFIG, yield* loadFile(Flag.RAILWISE_CONFIG))
            log.debug("loaded custom config", { path: Flag.RAILWISE_CONFIG })
          }

          if (!Flag.RAILWISE_DISABLE_PROJECT_CONFIG) {
            for (const file of yield* Effect.promise(() =>
              ConfigPaths.projectFiles("railwise", ctx.directory, ctx.worktree),
            )) {
          ...
          if (activeOrg) {
            yield* Effect.gen(function* () {
              ...
              if (Option.isSome(tokenOpt)) {
                ...
                Env.set("RAILWISE_CONSOLE_TOKEN", tokenOpt.value)
              }
