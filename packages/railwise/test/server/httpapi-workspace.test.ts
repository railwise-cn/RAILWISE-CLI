...
function requestServer(path: string, directory: string, init: RequestInit = {}) {
  ...
  headers.set("x-railwise-directory", directory)
      ...
      try {
        const response = yield* requestDefault(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-railwise-workspace": "internal",
          },
          body: JSON.stringify({ $schema: "https://railwise.ai/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-railwise-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-railwise-workspace")
