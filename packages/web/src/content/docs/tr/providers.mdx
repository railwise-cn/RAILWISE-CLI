---
title: Sağlayıcılar
description: railwise'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

railwise, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı railwise yapılandırmanızda yapılandırın.
...
`~/.local/share/railwise/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları railwise'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
RAILWISE Zen, railwise ekibi tarafından sağlanan modellerin bir listesidir.
railwise ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz RAILWISE Zen ile başlamanızı öneririz.
:::
...
railwise'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## RAILWISE Go

RAILWISE Go, railwise ile iyi çalıştığı test edilmiş ve doğrulanmış, railwise ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `RAILWISE Go`'yu seçin ve [railwise.ai/auth](https://railwise.ai/zen) adresine gidin.
...
Amazon Bedrock'u railwise ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   railwise'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `railwise.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi railwise'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
railwise'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   railwise'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
:::note[uyumluluk notu]
railwise, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. railwise'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`railwise.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [railwise-gitlab-auth](https://www.npmjs.com/package/railwise-gitlab-auth) ana sayfasında.

##### Yapılandırma

`railwise.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi railwise'la kullanmak için:
...
Google Vertex AI'yi railwise ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları railwise çalıştırılırken ayarlayın.
...
Helicone'un railwise aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her railwise görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [railwise-helicone-session](https://github.com/H2Shami/railwise-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her railwise görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
railwise'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla railwise'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla railwise'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini railwise için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/railwise)'a bakın.
:::
...
Ollama Cloud'u railwise ile kullanmak için:
...
3. railwise'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: railwise'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
RAILWISE Zen, railwise ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>RAILWISE Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **RAILWISE Zen**'i arayın.

   ```txt
   /connect
   ```

3. railwise API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   railwise yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca railwise yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
Modelleri railwise yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı railwise ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `railwise.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları railwise'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `railwise auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için railwise yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, railwise yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
