...
import type { AssistantMessage, Event, RailwiseClient, SessionMessageResponse } from "@railwise/sdk/v2"
...
export namespace ACP {
  ...
  export async function init({ sdk: _sdk }: { sdk: RailwiseClient }) {
    return {
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          // RAILWISE stores files internally as { type: "file", url, filename, mime }.
  ...
  async function defaultModel(config: ACPConfig, cwd?: string) {
    ...
    const railwiseProvider = providers.find((p) => p.id === "railwise")
    if (railwiseProvider) {
      if (railwiseProvider.models["big-pickle"]) {
        return { providerID: "railwise", modelID: "big-pickle" }
      }
      const [best] = Provider.sort(Object.values(railwiseProvider.models))
      if (best) {
        return {
