...
Aby debugować problemy z railwise, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\railwise\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `railwise --log-level DEBUG`.

---

## Przechowywanie danych

railwise przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/railwise/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\railwise`
...
railwise Desktop uruchamia w tle lokalny serwer railwise (sidecar `railwise-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `RAILWISE` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/railwise/railwise.jsonc` (lub `~/.config/railwise/railwise.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/railwise/railwise.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\railwise\railwise.jsonc`
...
railwise może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/railwise/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\railwise\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby railwise mógł ją odbudować.

1. Całkowicie zamknij railwise Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/railwise`
- **Linux**: usuń `~/.cache/railwise` (lub uruchom `rm -rf ~/.cache/railwise`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\railwise`

3. Uruchom ponownie railwise Desktop.

---

### Napraw problemy z połączeniem z serwerem

railwise Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `railwise.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `RAILWISE_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `RAILWISE_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows railwise Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami railwise.

---

### Brak powiadomień

railwise Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla railwise w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie railwise Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji railwise Desktop):

- `railwise.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `railwise.global.dat` i `railwise.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
Jeśli masz problemy z railwise:
...
3. Upewnij się, że masz najnowszą wersję z `railwise upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `railwise models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\railwise`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. railwise dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\railwise`

2. Uruchom ponownie kod railwise, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to railwise do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
railwise wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
