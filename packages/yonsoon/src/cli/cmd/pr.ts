...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run yonsoon",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for yonsoon session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found yonsoon session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`yonsoon import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting yonsoon...")
        UI.println()

        const { spawn } = await import("child_process")
        const yonsoonArgs = sessionId ? ["-s", sessionId] : []
        const yonsoonProcess = spawn("yonsoon", yonsoonArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          yonsoonProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`yonsoon exited with code ${code}`))
          })
          yonsoonProcess.on("error", reject)
        })
      },
    })
  },
