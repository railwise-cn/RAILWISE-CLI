...
test("旧项目驾驶舱入口进入极简工作台与全局项目栏", async ({ launchApp }) => {
  ...
  await expect(page.getByRole("button", { name: /^能力市场$/ })).toHaveCount(0)
  await expect(page.locator("[data-testid=home-harness-panel]")).toContainText("执行环境")
  await expect(page.locator("[data-testid=home-harness-panel]")).toContainText("模型：")
  await expect(page.locator("[data-testid=home-harness-inspector]")).toContainText("环境")
