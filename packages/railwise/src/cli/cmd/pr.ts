  ...
  async handler(args) {
    await Instance.provide({
      ...
      async fn() {
          ...
          if (prInfoText.trim()) {
            ...
            if (prInfo && prInfo.body) {
              ...
              if (sessionMatch) {
                ...
                const importResult = await Process.text(["railwise", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        const railwiseProcess = Process.spawn(["railwise", ...railwiseArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        })
        const code = await railwiseProcess.exited
        if (code !== 0) throw new Error(`railwise exited with code ${code}`)
      },
    })
  },
})
