...
مع YONSOON (甬算) يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم YONSOON (甬算) سمتنا `yonsoon`.
...
يأتي YONSOON (甬算) مع عدة سمات مدمجة.
...
- يريدون أن يطابق YONSOON (甬算) مظهر terminal لديهم
...
يدعم YONSOON (甬算) نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/yonsoon/themes/*.json` أو `$XDG_CONFIG_HOME/yonsoon/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.yonsoon/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.yonsoon/themes/*.json`
