...
- `not_applicable`: belongs to upstream infrastructure or App/Desktop client code maintained outside RAILWISE-CLI.

| Release  | Core / CLI decision                                                                         | Client decision                             |
| -------- | ------------------------------------------------------------------------------------------- | ------------------------------------------- |
| v1.18.0  | `not_applicable` (Desktop-only release)                                                     | `not_applicable` (WorkWise owns the client) |
...
| v1.18.16 | `partial` (tolerant top-level config parsing ported)                                        | `not_applicable` (client project discovery) |
...
| Copilot PDF input                         | `deferred`       | It needs an explicit CLI attachment and transport decision; client behavior belongs to WorkWise.                               |
| Desktop v2                                | `not_applicable` | App/Desktop code is outside RAILWISE-CLI and is not an upstream migration target.                                              |
...
A row can move to `ported` only after focused tests, `packages/railwise` tests and typecheck, SDK typecheck/regeneration, and cross-platform CLI build checks pass. Recording `v1.18.18` as reviewed is a separate action from marking individual rows as ported.

The 2026-08-14 upstream batch passed Railwise unit tests, Core/CLI typecheck, SDK regeneration, all CLI build targets, and the native `railwise --version` smoke test. App/Desktop results are not part of the CLI validation record.
