...
// terminal sizes. The default railwise tui command is excluded —
// `railwise --help` includes an ASCII banner that pulls in the install
...
// Top-level commands. Order matches what `railwise --help` prints today;
...
// top-level help on `--help` and exits 1; not a real railwise command.
const TOP_LEVEL = [
  "acp",
  "mcp",
  "attach",
...
describe("railwise CLI help-text snapshots", () => {
  ...
  cliIt.live(
    ...
    ({ railwise }) =>
      Effect.gen(function* () {
        const topLevel = yield* railwise.spawn(["--help"], { env: SNAPSHOT_ENV })
        ...
        const [failures, results] = yield* Effect.partition(
          ...
          (argv) =>
            Effect.gen(function* () {
              const result = yield* railwise.spawn([...argv, "--help"], { env: SNAPSHOT_ENV })
              if (result.exitCode !== 0) {
                return yield* Effect.fail(`railwise ${argv.join(" ")}: exit ${result.exitCode}`)
              }
              return { argv, result }
            }),
          { concurrency: 8 },
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`railwise ${argv.join(" ")} --help`)
        }
