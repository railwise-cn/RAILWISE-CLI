...
describe("railwise acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ railwise }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ railwise }))
        ...
        expect(initialized.agentInfo?.name).toBe("RAILWISE")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ railwise }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ railwise })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("railwise-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "railwise-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.RAILWISE_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ railwise }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("railwise-login")
