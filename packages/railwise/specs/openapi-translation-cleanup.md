...
Trim `packages/railwise/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/railwise/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/railwise`.
- `[x]` `bun typecheck` from `packages/railwise`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/railwise/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/railwise`.
- `[x]` `bun dev generate > /tmp/railwise-openapi.json` from `packages/railwise`.
...
- Edit `packages/railwise/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/railwise-openapi.json` from `packages/railwise`.
...
- Check source schemas in files like `packages/railwise/src/session/schema.ts`, `packages/railwise/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/railwise/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/railwise/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
