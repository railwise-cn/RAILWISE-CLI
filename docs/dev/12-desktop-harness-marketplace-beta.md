...
本文档用于 RAILWISE Desktop v1.3.0 Beta 发布前的人工验收和回归记录。目标不是证明页面能打开，而是证明桌面端已经像一个可用的本地 AI 工作台：用户进入工作台、选择项目文件夹、通过对话发起任务，并能看到执行层、权限和能力市场的真实状态。
...
7. 点击开始协作。
8. 确认进入会话页面，协作面板显示 `chief_manager` 或等价主控智能体。
...
15. 确认内置能力可见：项目总控、本地文件读取、复测资料检查、DeepSeek、本地安全模式。
