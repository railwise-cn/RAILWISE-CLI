...
- **macOS/Linux**: `~/.config/opencode/railwise.jsonc`（或 `~/.config/opencode/railwise.json`）
- **macOS/Linux**（舊版安裝）: `~/.local/share/opencode/railwise.jsonc`
- **Windows**: 按 `WIN+R` 並貼上 `%USERPROFILE%\.config\opencode\railwise.jsonc`
