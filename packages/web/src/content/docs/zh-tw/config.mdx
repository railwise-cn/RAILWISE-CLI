...
2. **全域設定**（`~/.config/opencode/yonsoon.json`）- 使用者偏好
...
將全域 OpenCode 設定放在 `~/.config/opencode/yonsoon.json` 中。使用全域設定來設定使用者層級的偏好，例如主題、供應商或快捷鍵。
...
使用 `YONSOON_CONFIG_DIR` 環境變數指定自訂設定目錄。該目錄會像標準 `.opencode` 目錄一樣被搜尋代理、指令、模式和外掛程式，並且應遵循相同的結構。
...
您還可以使用 `~/.config/opencode/agents/` 或 `.yonsoon/agents/` 中的 Markdown 檔案定義代理。[在此了解更多](/docs/agents)。
...
您還可以使用 `~/.config/opencode/commands/` 或 `.yonsoon/commands/` 中的 Markdown 檔案定義指令。[在此了解更多](/docs/commands)。
...
將外掛程式檔案放置在 `.yonsoon/plugins/` 或 `~/.config/opencode/plugins/` 中。您還可以透過 `plugin` 選項從 npm 載入外掛程式。
