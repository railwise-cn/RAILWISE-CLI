...
_Model Bağlam Protokolü_ veya MCP kullanarak railwise'a harici araçlar ekleyebilirsiniz. railwise hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [railwise Config](https://railwise.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/railwise` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
railwise, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde railwise şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa railwise, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `railwise mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından railwise, belirteçleri `~/.local/share/railwise/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra railwise'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi railwise yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve railwise'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
