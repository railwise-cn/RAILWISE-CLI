...
Mit RAILWISE kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet RAILWISE das `railwise`-Theme.
...
RAILWISE liefert mehrere eingebaute Themes mit.
...
- RAILWISE optisch ans Terminal anpassen wollen
...
RAILWISE unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/railwise/themes/*.json` oder `$XDG_CONFIG_HOME/railwise/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.railwise/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.railwise/themes/*.json`
