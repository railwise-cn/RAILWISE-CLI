...
import { FrontmatterError } from "@railwise/core/v1/config/error"
import { ConfigMarkdown } from "@/config/markdown"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Glob } from "@railwise/core/util/glob"
...
// Built-in skill that ships with railwise. The model's intuition for what an
// railwise.json should look like is often wrong, and railwise hard-fails on
// invalid config, so users hit cryptic startup errors. Loading this skill
// when the model is asked to touch railwise's own config files gives it the
// actual schemas instead of guesses.
const CUSTOMIZE_RAILWISE_SKILL_NAME = "customize-railwise"
const CUSTOMIZE_RAILWISE_SKILL_DESCRIPTION =
...
const CUSTOMIZE_RAILWISE_SKILL_BODY = SkillPlugin.CustomizeOpencodeContent
...
const discoverSkills = Effect.fnUntraced(function* (
  ...
  for (const dir of configDirs) {
    yield* scan(state, dir, RAILWISE_SKILL_PATTERN)
  }
...
export class Service extends Context.Service<Service, Interface>()("@railwise/Skill") {}
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make(
      Effect.fn("Skill.state")(function* () {
        ...
        s.skills[CUSTOMIZE_RAILWISE_SKILL_NAME] = {
          name: CUSTOMIZE_RAILWISE_SKILL_NAME,
          description: CUSTOMIZE_RAILWISE_SKILL_DESCRIPTION,
          location: "<built-in>",
          content: CUSTOMIZE_RAILWISE_SKILL_BODY,
        }
