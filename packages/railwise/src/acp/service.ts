...
import type { Message, OpencodeClient, SessionMessageResponse } from "@railwise/sdk/v2"
...
export const AuthMethodID = "railwise-login"
...
export class Service extends Context.Service<Service, Interface>()("@railwise/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `railwise auth login` in the terminal",
      name: "Login with railwise",
      id: AuthMethodID,
    }
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, railwise provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const railwiseProvider = providers[ProviderV2.ID.make("railwise")]
  const railwiseModel = railwiseProvider ? Provider.sort(Object.values(railwiseProvider.models))[0] : undefined
  if (railwiseProvider && railwiseModel) return { providerID: railwiseProvider.id, modelID: railwiseModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "RAILWISE service failure", service })
}
