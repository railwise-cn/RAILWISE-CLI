...
description: RAILWISE, LSP sunucularınızla bütünleşir.
---

RAILWISE, tanılamayı agent için geri bildirim olarak kullanmak üzere Language Server Protocol (LSP) sunucularıyla bütünleşebilir.

---

## Yerleşik

RAILWISE, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`RAILWISE_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
LSP etkinleştirildiğinde ve railwise bir dosyayı açtığında:
...
LSP sunucularını, railwise yapılandırmanızdaki `lsp` bölümü aracılığıyla etkinleştirebilir ve özelleştirebilirsiniz.
