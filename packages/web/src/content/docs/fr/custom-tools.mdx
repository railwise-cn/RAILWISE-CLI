...
description: Créez des outils que le LLM peut appeler dans yonsoon.
---

Les outils personnalisés sont des fonctions que vous créez et que le LLM peut appeler pendant les conversations. Ils fonctionnent avec les [outils intégrés](/docs/tools) de yonsoon comme `read`, `write` et `bash`.
...
- Localement en les plaçant dans le répertoire `.yonsoon/tools/` de votre projet.
- Ou globalement, en les plaçant dans `~/.config/yonsoon/tools/`.
...
```ts title=".yonsoon/tools/database.ts" {1}
...
```ts title=".yonsoon/tools/math.ts"
import { tool } from "@yonsoon/plugin"
...
```ts title=".yonsoon/tools/project.ts" {8}
import { tool } from "@yonsoon/plugin"
...
```python title=".yonsoon/tools/add.py"
import sys
...
```ts title=".yonsoon/tools/python-add.ts" {10}
import { tool } from "@yonsoon/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".yonsoon/tools/add.py")
