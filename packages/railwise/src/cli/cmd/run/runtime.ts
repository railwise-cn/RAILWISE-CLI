...
import { createOpencodeClient } from "@railwise/sdk/v2"
...
async function runInteractiveRuntime(input: RunRuntimeInput): Promise<void> {
  return withRunSpan(
    ...
    {
      "railwise.mode": input.resolveSession ? "local" : "attach",
      "railwise.initial_input": !!input.initialInput,
      "railwise.demo": input.demo,
    },
    async (span) => {
      ...
      setRunSpanAttributes(span, {
        "railwise.directory": ctx.directory,
        "railwise.resume": ctx.resume === true,
        "railwise.agent.name": state.agent,
        "railwise.model.provider": state.model?.providerID,
        "railwise.model.id": state.model?.modelID,
        "railwise.model.variant": state.activeVariant,
      ...
      const shell = await createRuntimeLifecycle({
        ...
        onModelSelect: async (model) => {
          ...
          setRunSpanAttributes(span, {
            "railwise.model.provider": model.providerID,
            "railwise.model.id": model.modelID,
            "railwise.model.variant": state.activeVariant,
          })
          return {
          ...
          run: async (prompt, signal) => {
            ...
            return withRunSpan(
              "RunInteractive.turn",
              {
                ...
                "railwise.model.id": state.model?.modelID,
                "railwise.model.variant": state.activeVariant,
                "railwise.prompt.chars": prompt.text.length,
                "railwise.prompt.parts": prompt.parts.length,
                "railwise.prompt.include_files": includeFiles,
                "railwise.prompt.file_parts": includeFiles ? input.files.length : 0,
                "session.id": state.sessionID || undefined,
              },
              async (span) => {
                try {
...
export async function runInteractiveLocalMode(input: RunLocalInput): Promise<void> {
  return withRunSpan(
    ...
    {
      "railwise.directory": input.directory,
      "railwise.initial_input": !!input.initialInput,
      "railwise.demo": input.demo,
    },
    async () => {
      const sdk = createOpencodeClient({
        baseUrl: "http://railwise.internal",
        fetch: input.fetch,
