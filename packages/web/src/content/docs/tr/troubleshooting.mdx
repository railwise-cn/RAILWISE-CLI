...
railwise ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\railwise\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `railwise --log-level DEBUG`.

---

## Depolama

railwise, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:

- **macOS/Linux**: `~/.local/share/railwise/`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\railwise` yapistirin
...
railwise Desktop arka planda yerel bir railwise sunucusu (`railwise-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `railwise` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/railwise/railwise.jsonc` (veya `~/.config/railwise/railwise.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/railwise/railwise.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\railwise\railwise.jsonc` yapistirin
...
railwise diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/railwise/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\railwise\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), railwise'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. railwise Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/railwise`
- **Linux**: `~/.cache/railwise` dizinini silin (veya `rm -rf ~/.cache/railwise` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\railwise` yapistirin

3. railwise Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

railwise Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`railwise.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.

#### Ortam degiskenlerini kontrol edin

Ortamda `RAILWISE_PORT` ayarliysa desktop uygulamasi yerel sunucu icin o portu kullanmaya calisir.

- `RAILWISE_PORT` degerini kaldirin (veya bos bir port secin) ve yeniden baslatin
...
Windows'ta railwise Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, railwise ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

railwise Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda railwise icin bildirimler etkinse
...
1. railwise Desktop'u kapatin
2. Su dosyalari bulun ve silin (railwise Desktop uygulama veri dizininde yer alirlar):

- `railwise.settings.dat` (desktop varsayilan sunucu URL'si)
- `railwise.global.dat` ve `railwise.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
railwise ile ilgili bir sorun yasiyorsaniz:
...
3. `railwise upgrade` ile en guncel surumu kullandiginizdan emin olun
...
Erisiminiz olan modelleri gormek icin `railwise models` calistirin.
...
2. Sorun surerse kayitli konfigurasyonu temizlemeyi deneyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.local\share\railwise`
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. railwise, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
1. Provider paket cache'ini temizleyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.cache\railwise`

2. En guncel provider paketlerini yeniden kurmak icin railwise'u yeniden baslatin

Bu, railwise'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
railwise, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
