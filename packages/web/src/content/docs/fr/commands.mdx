...
Créez `.yonsoon/commands/test.md` :
...
Vous pouvez ajouter des commandes personnalisées via la configuration YONSOON (甬算) ou en créant des fichiers markdown dans le répertoire `commands/`.

---

### JSON

Utilisez l'option `command` dans votre YONSOON (甬算) [config](/docs/config) :
...
yonsoon comprend plusieurs commandes intégrées comme `/init`, `/undo`, `/redo`, `/share`, `/help` ; [en savoir plus](/docs/tui#commands).
