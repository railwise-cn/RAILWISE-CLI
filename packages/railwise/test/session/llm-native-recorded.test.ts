...
import { SessionV1 } from "@railwise/core/v1/session"
import { FSUtil } from "@railwise/core/fs-util"
import { ModelsDev } from "@railwise/core/models-dev"
import { HttpRecorder } from "@railwise/http-recorder"
import { HttpRecorderInternal } from "@railwise/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@railwise/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor } from "@railwise/llm/route"
import { Env } from "@/env"
...
const zenURL = (connection: string) => `https://console.railwise.ai/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.RAILWISE_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["railwise", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("RAILWISE_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("RAILWISE_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["railwise", "native", "oauth", "tool-loop"],
  ...
  },
  {
    id: "railwise-proxy",
    name: "RAILWISE proxy",
    providerID: ProviderV2.ID.railwise,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["railwise", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.RAILWISE_RECORD_CONSOLE_TOKEN && process.env.RAILWISE_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.railwise,
        name: "RAILWISE Zen",
        env: ["RAILWISE_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.RAILWISE_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.RAILWISE_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.RAILWISE_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["railwise", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("RAILWISE_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("RAILWISE_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("RAILWISE_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set RAILWISE_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the railwise LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      ...
      JSON.stringify({ $schema: "https://railwise.ai/config.json", ...scenario.config(model) }),
    ),
  )
