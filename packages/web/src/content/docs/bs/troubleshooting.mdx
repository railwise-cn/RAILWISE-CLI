...
Da biste otklonili probleme s YONSOON (甬算), počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\yonsoon\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `yonsoon --log-level DEBUG`.

---

## Pohrana

YONSOON (甬算) pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/yonsoon/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\yonsoon`
...
YONSOON (甬算) Desktop pokreće lokalni YONSOON (甬算) server (`yonsoon-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `YONSOON (甬算)` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/yonsoon/yonsoon.jsonc` (ili `~/.config/yonsoon/yonsoon.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/yonsoon/yonsoon.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\yonsoon\yonsoon.jsonc`
...
YONSOON (甬算) također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/yonsoon/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\yonsoon\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga YONSOON (甬算) mogao ponovo izgraditi.

1. Potpuno zatvorite YONSOON (甬算) Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/yonsoon`
- **Linux**: obrišite `~/.cache/yonsoon` (ili pokrenite `rm -rf ~/.cache/yonsoon`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\yonsoon`

3. Ponovo pokrenite YONSOON (甬算) Desktop.

---

### Rješavanje problema sa vezom na serveru

YONSOON (甬算) Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `yonsoon.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `YONSOON_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `YONSOON_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, YONSOON (甬算) Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa YONSOON (甬算) karakteristikama.

---

### Obavještenja se ne prikazuju

YONSOON (甬算) Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za YONSOON (甬算) u postavkama vašeg OS-a, i
...
1. Zatvorite YONSOON (甬算) Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka YONSOON (甬算) Desktop aplikacije):

- `yonsoon.settings.dat` (URL zadanog servera za desktop)
- `yonsoon.global.dat` i `yonsoon.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Ako imate problema s YONSOON (甬算):
...
Da saznate kojim modelima imate pristup, pokrenite `yonsoon models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\yonsoon`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. YONSOON (甬算) dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\yonsoon`

2. Ponovo pokrenite YONSOON (甬算) da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti YONSOON (甬算) da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
YONSOON (甬算) će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
