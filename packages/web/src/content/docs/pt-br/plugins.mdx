---
title: Plugins
description: Escreva seus próprios plugins para estender o railwise.
---

Plugins permitem que você estenda o railwise conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do railwise.
...
- `.railwise/plugins/` - Plugins em nível de projeto
- `~/.config/railwise/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/railwise/node_modules/`.
...
1. Configuração global (`~/.config/railwise/railwise.json`)
2. Configuração do projeto (`railwise.json`)
3. Diretório de plugins global (`~/.config/railwise/plugins/`)
4. Diretório de plugins do projeto (`.railwise/plugins/`)
...
O railwise executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do railwise para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o railwise.
...
:::note
Se você estiver usando o aplicativo desktop railwise, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o railwise de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao railwise:
...
O helper `tool` cria uma ferramenta personalizada que o railwise pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o railwise junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://railwise.ai/docs/sdk) para detalhes.
