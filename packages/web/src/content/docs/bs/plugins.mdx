---
title: Dodaci
description: Napišite vlastite dodatke za proširenje RAILWISE (甬算).
---

Dodaci vam omogućavaju da proširite RAILWISE (甬算) spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja RAILWISE (甬算).
...
- `.railwise/plugins/` - Dodaci na nivou projekta
- `~/.config/railwise/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/railwise/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/railwise/railwise.json`)
2. Konfiguracija projekta (`railwise.json`)
3. Globalni direktorij dodataka (`~/.config/railwise/plugins/`)
4. Direktorij dodataka projekta (`.railwise/plugins/`)
...
RAILWISE (甬算) pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Railwise SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje RAILWISE (甬算).
...
:::note
Ako koristite desktop aplikaciju RAILWISE (甬算), ona može automatski slati sistemske obavijesti kada je odgovor spreman ili kada dođe do greške u sesiji.
:::
...
Spriječite railwise da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u railwise:
...
Pomoćnik `tool` kreira prilagođeni alat koji railwise može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za railwise zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://railwise.ai/docs/sdk) za detalje.
