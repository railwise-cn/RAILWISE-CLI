...
Создайте `.yonsoon/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию yonsoon или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем yonsoon [config](/docs/config):
...
- Глобальный: `~/.config/yonsoon/commands/`
- Для каждого проекта: `.yonsoon/commands/`
...
yonsoon включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
