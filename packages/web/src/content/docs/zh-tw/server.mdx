...
設定 `RAILWISE_SERVER_PASSWORD` 以使用 HTTP 基本認證保護伺服器。使用者名稱預設為 `opencode`，也可以設定 `RAILWISE_SERVER_USERNAME` 來覆蓋它。這適用於 `opencode serve` 和 `opencode web`。

```bash
RAILWISE_SERVER_PASSWORD=your-password opencode serve
```
