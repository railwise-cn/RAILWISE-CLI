...
你可能想不到，一台闲置的安卓手机，也能跑起一个拥有 7 个 AI 智能体、24 个专业计算工具和 11 个领域技能包的测绘内业系统。
...
RAILWISE-CLI 是一个基于 [Bun](https://bun.sh)（JavaScript 运行时）的命令行程序。它的核心依赖就两样东西：**Bun 运行时**和**网络连接**（用来调用大模型 API）。所有 24 个专业计算工具（平差、坐标转换、轴力计算等）都是纯 TypeScript 代码，在本地执行，不依赖任何 GPU 或特殊硬件。
