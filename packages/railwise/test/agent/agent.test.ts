...
it.instance("plan agent denies edits except .railwise/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".railwise/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".railwise", "skill", "perm-skill")
...
`,
      ...
      process.env.RAILWISE_TEST_HOME = test.directory
