...
Les fournisseurs les plus populaires sont préchargés par défaut. Si vous avez ajouté les informations d'identification d'un fournisseur via la commande `/connect`, elles seront disponibles lorsque vous démarrerez YONSOON (甬算).
...
Voici plusieurs modèles qui fonctionnent bien avec YONSOON (甬算), sans ordre particulier. (Cette liste n’est pas exhaustive et n’est pas nécessairement à jour) :
...
Pour définir l'un d'entre eux comme modèle par défaut, vous pouvez définir la clé `model` dans votre configuration YONSOON (甬算).

```json title="yonsoon.json" {3}
{
...
Ici, l'identifiant complet est `provider_id/model_id`. Par exemple, si vous utilisez [YONSOON (甬算) Zen](/docs/zen), vous utiliserez `yonsoon/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```jsonc title="yonsoon.jsonc" {7-12,19-24}
{
  "$schema": "https://yonsoon.ai/config.json",
  "provider": {
    "openai": {
...
```jsonc title="yonsoon.jsonc" {6-21}
{
  "$schema": "https://yonsoon.ai/config.json",
  "provider": {
    "yonsoon": {
...
De nombreux modèles prennent en charge plusieurs variantes avec différentes configurations. YONSOON (甬算) est livré avec des variantes par défaut intégrées pour les fournisseurs populaires.

### Variantes intégrées

YONSOON (甬算) est livré avec des variantes par défaut pour de nombreux fournisseurs :
...
```jsonc title="yonsoon.jsonc" {7-18}
{
  "$schema": "https://yonsoon.ai/config.json",
  "provider": {
    "openai": {
...
Lorsque YONSOON (甬算) démarre, il recherche les modèles dans l'ordre de priorité suivant :
...
2. La liste des modèles dans la configuration YONSOON (甬算).
