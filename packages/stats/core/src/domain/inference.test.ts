...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve railwise route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "railwise")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "railwise")).toBe("openai")
    expect(statProvider("big-pickle", "", "railwise")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "railwise"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep railwise as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "railwise"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "railwise"))).toMatchObject([{ provider: "unknown" }])
  })

  test("geo aggregates never keep railwise or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "railwise"), country: "US" })).toMatchObject([
