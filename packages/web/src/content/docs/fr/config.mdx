---
title: Configuration
description: Utilisation de la configuration JSON RAILWISE (甬算).
---

Vous pouvez configurer RAILWISE (甬算) à l'aide d'un fichier de configuration JSON.

---

## Format

RAILWISE (甬算) prend en charge les formats **JSON** et **JSONC** (JSON avec commentaires).
...
Par exemple, si votre configuration globale définit `theme: "railwise"` et `autoupdate: true` et que la configuration de votre projet définit `model: "anthropic/claude-sonnet-4-5"`, la configuration finale inclura les trois paramètres.
...
Placez votre configuration globale RAILWISE (甬算) dans `~/.config/railwise/railwise.json`. Utilisez la configuration globale pour les préférences de l'utilisateur telles que les thèmes, les fournisseurs ou les raccourcis clavier.
...
Lorsque RAILWISE (甬算) démarre, il recherche un fichier de configuration dans le répertoire actuel ou remonte jusqu'au répertoire Git le plus proche.
...
- `mdns` - Activer la découverte du service mDNS. Cela permet à d'autres appareils du réseau de découvrir votre serveur RAILWISE (甬算).
...
Vous pouvez configurer les fournisseurs et les modèles que vous souhaitez utiliser dans votre configuration RAILWISE (甬算) via les options `provider`, `model` et `small_model`.
...
L'option `small_model` configure un modèle distinct pour les tâches légères comme la génération de titres. Par défaut, RAILWISE (甬算) essaie d'utiliser un modèle moins cher s'il est disponible auprès de votre fournisseur, sinon il revient à votre modèle principal.
...
Vous pouvez configurer le thème que vous souhaitez utiliser dans votre configuration RAILWISE (甬算) via l'option `theme`.
...
L'agent par défaut doit être un agent principal (et non un sous-agent). Il peut s'agir d'un agent intégré tel que `"build"` ou `"plan"`, ou d'un [agent personnalisé](/docs/agents) que vous avez défini. Si l'agent spécifié n'existe pas ou est un sous-agent, RAILWISE (甬算) reviendra à `"build"` avec un avertissement.
...
RAILWISE (甬算) téléchargera automatiquement toutes les nouvelles mises à jour au démarrage. Vous pouvez désactiver cela avec l'option `autoupdate`.
...
[Plugins](/docs/plugins) étendent RAILWISE (甬算) avec des outils, des hooks et des intégrations personnalisés.
...
Ceci est utile lorsque vous souhaitez restreindre RAILWISE (甬算) à l'utilisation de fournisseurs spécifiques plutôt que de les désactiver un par un.
