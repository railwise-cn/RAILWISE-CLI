...
- `railwise web` command
- The desktop app itself

**Only** add if it's specifically about the Desktop application or `railwise web` view. **Do not** add for terminal, TUI, or general railwise issues.
...
**Only** add if the issue mentions "zen" or "railwise zen" or "railwise black".

If the issue doesn't have "zen" or "railwise black" in it then don't add zen label

#### core

Use for core server issues in `packages/railwise/`, excluding `packages/railwise/src/cli/cmd/tui/`.
...
TUI (`packages/railwise/src/cli/cmd/tui/...`):
...
Core (`packages/railwise/...`, excluding TUI subtree):

- thdxr for sqlite/snapshot/memory bugs and larger architectural core features
- jlongster for railwise server + API feature work (tool currently remaps jlongster -> thdxr until assignable)
