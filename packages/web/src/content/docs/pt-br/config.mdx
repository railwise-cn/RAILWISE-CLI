...
Por exemplo, se sua configuração global define `theme: "railwise"` e `autoupdate: true`, e sua configuração de projeto define `model: "anthropic/claude-sonnet-4-5"`, a configuração final incluirá as três configurações.
...
Coloque sua configuração global do railwise em `~/.config/railwise/railwise.json`. Use a configuração global para preferências de usuário, como temas, provedores ou atalhos de teclado.
...
O arquivo de configuração tem um esquema que está definido em [**`railwise.ai/config.json`**](https://railwise.ai/config.json).
...
Você pode configurar o tema que deseja usar em sua configuração do railwise através da opção `theme`.
