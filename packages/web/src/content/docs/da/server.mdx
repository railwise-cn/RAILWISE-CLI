---
title: Server
description: Interager med yonsoon-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `yonsoon serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en yonsoon-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `yonsoon.local` |
...
Indstil `YONSOON_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `yonsoon`, eller indstil `YONSOON_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `yonsoon serve` og `yonsoon web`.
...
Når du kører `yonsoon` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug yonsoon-serveren til at interagere med yonsoon programmatisk.
:::

Denne arkitektur lader yonsoon understøtte flere klienter og lader dig interagere med yonsoon programmatisk.

Du kan køre `yonsoon serve` for at starte en selvstændig server. Hvis du har
yonsoon TUI kørende, vil `yonsoon serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af YONSOON (甬算) [IDE](/docs/ide) plugins.
...
YONSOON (甬算)-serveren viser følgende API'er.
