...
Puede agregar herramientas externas a RAILWISE (甬算) utilizando el _Model Context Protocol_ o MCP. RAILWISE (甬算) admite servidores locales y remotos.
...
Puede definir servidores MCP en su [RAILWISE (甬算) Config](https://railwise.ai/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/railwise`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
RAILWISE (甬算) maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, RAILWISE (甬算):
...
Si el servidor requiere autenticación, RAILWISE (甬算) le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `railwise mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, RAILWISE (甬算) almacenará los tokens de forma segura en `~/.local/share/railwise/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en RAILWISE (甬算), junto con herramientas integradas. Para que puedas administrarlos a través de la configuración RAILWISE (甬算) como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar RAILWISE (甬算) a su cuenta Sentry.
