...
Sie können externe Tools zu RAILWISE hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. RAILWISE unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [RAILWISE Config](https://railwise.ai/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
RAILWISE übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird RAILWISE Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von RAILWISE beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `railwise mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert RAILWISE die Token sicher in `~/.local/share/railwise/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in RAILWISE verfügbar. Sie können sie also wie jedes andere Tool über die RAILWISE-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und RAILWISE mit Ihrem Sentry-Konto zu verbinden.
