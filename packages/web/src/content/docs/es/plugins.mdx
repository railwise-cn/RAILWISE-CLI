---
title: Complementos
description: Escriba sus propios complementos para extender YONSOON (甬算).
---

Los complementos le permiten extender YONSOON (甬算) al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de YONSOON (甬算).
...
- `.yonsoon/plugins/` - Complementos a nivel de proyecto
- `~/.config/yonsoon/plugins/` - Complementos globales
...
```json title="yonsoon.json"
{
  ...
  "plugin": ["yonsoon-helicone-session", "yonsoon-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/yonsoon/node_modules/`.
...
1. Configuración global (`~/.config/yonsoon/yonsoon.json`)
2. Configuración del proyecto (`yonsoon.json`)
3. Directorio global de complementos (`~/.config/yonsoon/plugins/`)
4. Directorio de complementos del proyecto (`.yonsoon/plugins/`)
...
```json title=".yonsoon/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
YONSOON (甬算) ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".yonsoon/plugins/my-plugin.ts"
...
```js title=".yonsoon/plugins/example.js"
...
- `client`: Un cliente SDK yonsoon para interactuar con la IA.
...
import type { Plugin } from "@yonsoon/plugin"
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar yonsoon.
...
```js title=".yonsoon/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "yonsoon"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio YONSOON (甬算), puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que yonsoon lea archivos `.env`:

```javascript title=".yonsoon/plugins/env-protection.js"
...
```javascript title=".yonsoon/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a yonsoon:

```ts title=".yonsoon/plugins/custom-tools.ts"
import { type Plugin, tool } from "@yonsoon/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que yonsoon puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para yonsoon junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://yonsoon.ai/docs/sdk) para obtener más detalles.
...
```ts title=".yonsoon/plugins/compaction.ts"
import type { Plugin } from "@yonsoon/plugin"
...
```ts title=".yonsoon/plugins/custom-compaction.ts"
import type { Plugin } from "@yonsoon/plugin"
