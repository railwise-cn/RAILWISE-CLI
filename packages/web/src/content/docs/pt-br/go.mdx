...
O RAILWISE Go é uma assinatura de baixo custo — **US$ 5 no seu primeiro mês**, depois **US$ 10/mês** — que oferece acesso confiável a modelos abertos de programação populares.

O Go funciona como qualquer outro provedor no RAILWISE. Você assina o RAILWISE Go e
...
O RAILWISE Go lhe dá acesso a esses modelos por **US$ 5 no seu primeiro mês**, depois **US$ 10/mês**.

---

## Como funciona

O RAILWISE Go funciona como qualquer outro provedor no RAILWISE.

1. Você entra no **<a href={console}>RAILWISE Zen</a>**, assina o Go e
   copia a sua chave de API.
2. Você executa o comando `/connect` na TUI, seleciona `RAILWISE Go` e cola
   a sua chave de API.
3. Execute `/models` na TUI para ver a lista de modelos disponíveis através do Go.

:::note
Apenas um membro por workspace pode assinar o RAILWISE Go.
:::
...
O RAILWISE Go inclui os seguintes limites:
...
- MiniMax M3 — 810 tokens de entrada, 62.000 em cache, 225 tokens de saída por requisição
...
O [ID do modelo](/docs/config/#models) na sua configuração do RAILWISE
usa o formato `railwise-go/<model-id>`. Por exemplo, para o Kimi K2.6, você usaria
`railwise-go/kimi-k2.6` na sua configuração.
...
4. Não ter **nenhuma restrição de fornecedor (lock-in)**, permitindo que você use qualquer outro provedor com o RAILWISE também.
