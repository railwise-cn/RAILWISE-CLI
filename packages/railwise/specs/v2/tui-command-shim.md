...
Delete `packages/railwise/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/railwise/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/railwise/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/railwise`
- TUI plugin loader tests in `packages/railwise` if runtime plugin API wiring changed
