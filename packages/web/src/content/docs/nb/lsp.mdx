---
title: LSP-servere
description: yonsoon integreres med LSP-serverne dine.
---

YONSOON (甬算) integreres med din Language Server Protocol (LSP) for å hjelpe LLM med å samhandle med kodebasen din. Den bruker diagnostikk for å gi tilbakemelding til LLM.

---

## Innebygde servere

YONSOON (甬算) kommer med flere innebygde LSP-servere for populære språk:
...
:::note
Du kan deaktivere automatiske LSP-servernedlastinger ved å sette miljøvariabelen `YONSOON_DISABLE_LSP_DOWNLOAD` til `true`.
:::
...
Når yonsoon åpner en fil, gjør den:
...
Du kan tilpasse LSP-servere gjennom `lsp`-delen i YONSOON (甬算)-konfigurasjonen.
