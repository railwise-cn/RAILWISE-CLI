...
Agent Skills erlauben YONSOON (甬算), wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
YONSOON (甬算) sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft YONSOON (甬算) vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.yonsoon/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/yonsoon/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.yonsoon/skills/git-release/SKILL.md` zum Beispiel so:
...
YONSOON (甬算) listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `yonsoon.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `yonsoon.json`):
