...
pub fn spawn_command(
    ...
    let mut cmd = if cfg!(windows) {
        if is_wsl_enabled(app) {
            ...
            let mut script = vec![
                "set -e".to_string(),
                "BIN=\"$HOME/.railwise/bin/railwise\"".to_string(),
...
pub fn serve(
    ...
    let envs = [
        ("RAILWISE_SERVER_USERNAME", "railwise".to_string()),
    ...
    .expect("Failed to spawn railwise");
...
mod tests {
    ...
    fn merge_shell_env_keeps_explicit_overrides() {
        ...
        assert_eq!(merged.get("RAILWISE_CLIENT"), Some(&"desktop".to_string()));
    }
