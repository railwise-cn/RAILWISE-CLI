...
Med YONSOON (甬算) kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger YONSOON (甬算) vårt eget `yonsoon`-tema.
...
YONSOON (甬算) kommer med flere innebygde temaer.
...
- Vil at YONSOON (甬算) skal matche terminalens utseende
...
YONSOON (甬算) støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/yonsoon/themes/*.json` eller `$XDG_CONFIG_HOME/yonsoon/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.yonsoon/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.yonsoon/themes/*.json`
