...
import { errorData, errorMessage } from "@railwise/tui/util/error"
...
import { hasTheme, upsertTheme } from "@railwise/tui/context/theme"
import { Global } from "@railwise/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@railwise/core/util/flock"
import { Flag } from "@railwise/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@railwise/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@railwise/core/v1/config/plugin"
import { createCommandShim } from "@railwise/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@railwise/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".railwise"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".railwise", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".railwise", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.RAILWISE_PURE ? [] : pluginOrigins
    if (Flag.RAILWISE_PURE && pluginOrigins.length) {
    }
