---
title: TUI
description: Using the RAILWISE terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

RAILWISE provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running RAILWISE starts the TUI for the current directory.
...
When using the RAILWISE TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to RAILWISE. Allows you to select from available providers and add their API keys.
...
Exit RAILWISE. _Aliases_: `/quit`, `/q`
...
This is separate from `railwise.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long RAILWISE waits after the leader key. Defaults to `2000`.
...
Use `RAILWISE_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `railwise.default`.
