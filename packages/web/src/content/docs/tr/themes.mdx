...
railwise ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak railwise kendi `railwise` temasını kullanır.
...
railwise birden fazla yerleşik temayla gelir.
...
- railwise'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
railwise, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/railwise/themes/*.json` veya `$XDG_CONFIG_HOME/railwise/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.railwise/themes/*.json`
4. **Geçerli çalışma dizini** - `./.railwise/themes/*.json`
