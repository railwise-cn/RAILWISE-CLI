...
Det er to typer agenter i RAILWISE (甬算); primære agenter og underagenter.
...
RAILWISE (甬算) kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
RAILWISE (甬算) kommer med to innebygde underagenter, **General** og **Explore**. Vi skal se på dette nedenfor.

---

## Innebygd

RAILWISE (甬算) kommer med to innebygde primære agenter og to innebygde underagenter.
...
Hvis ingen temperatur er spesifisert, bruker RAILWISE (甬算) modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale RAILWISE (甬算)-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i RAILWISE (甬算)-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [RAILWISE (甬算) Zen](/docs/zen), vil du bruke `railwise/gpt-5.1-codex` for GPT 5.1 Codex.
