---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in YONSOON (甬算).
---

import config from "../../../../config.mjs"
export const console = config.console

YONSOON (甬算) verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer YONSOON (甬算)-Konfiguration.
...
in `~/.local/share/yonsoon/auth.json`.

---

### Konfiguration

Sie können die Anbieter über den Abschnitt `provider` in Ihrem YONSOON (甬算) anpassen
config.
...
YONSOON (甬算) Zen ist eine Liste von Modellen, die vom YONSOON (甬算)-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit YONSOON (甬算) funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit YONSOON (甬算) Zen zu beginnen.
:::
...
Es funktioniert wie jeder andere Anbieter in YONSOON (甬算) und ist völlig optional.
...
So verwenden Sie Amazon Bedrock mit YONSOON (甬算):
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie YONSOON (甬算) ausführen:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `yonsoon.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in YONSOON (甬算) wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit YONSOON (甬算) ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre YONSOON (甬算)-Konfiguration hinzufügen.
...
2. Wählen Sie Ihre Authentifizierungsmethode:
   ...
   3. Name: `YONSOON (甬算)`, Bereiche: `api`
...
um das Token im YONSOON (甬算)-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
YONSOON (甬算) verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von YONSOON (甬算)
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`yonsoon.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit YONSOON (甬算):
...
So verwenden Sie Google Vertex AI mit YONSOON (甬算):
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie YONSOON (甬算) ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über YONSOON (甬算) konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [yonsoon-helicone-session](https://github.com/H2Shami/yonsoon-helicone-session)-Plugin, um jede YONSOON (甬算)-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede YONSOON (甬算)-Konversation als separate Sitzung aufgeführt.
...
Sie können YONSOON (甬算) für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können YONSOON (甬算) über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können YONSOON (甬算) über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für YONSOON (甬算) konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/yonsoon).
:::
...
So verwenden Sie Ollama Cloud mit YONSOON (甬算):
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in YONSOON (甬算).
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in YONSOON (甬算) verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
YONSOON (甬算) Zen ist eine Liste getesteter und verifizierter Modelle, die vom YONSOON (甬算)-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>YONSOON (甬算) Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **YONSOON (甬算) Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren YONSOON (甬算) API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre YONSOON (甬算)-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre YONSOON (甬算)-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit YONSOON (甬算):
...
Sie können Modelle auch über Ihre YONSOON (甬算)-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit YONSOON (甬算) verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
4. Erstellen oder aktualisieren Sie Ihre `yonsoon.json`-Datei in Ihrem Projektverzeichnis:
...
Mithilfe der `limit`-Felder kann YONSOON (甬算) erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `yonsoon auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die YONSOON (甬算)-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer YONSOON (甬算)-Konfiguration übereinstimmt.
