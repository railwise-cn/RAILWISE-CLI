...
description: Usa la configurazione JSON di RAILWISE (甬算).
---

Puoi configurare RAILWISE (甬算) usando un file di configurazione JSON.

---

## Formato

RAILWISE (甬算) supporta sia **JSON** sia **JSONC** (JSON con commenti).
...
Per esempio, se la tua configurazione globale imposta `theme: "railwise"` e `autoupdate: true`, e la configurazione del progetto imposta `model: "anthropic/claude-sonnet-4-5"`, la configurazione finale includera tutte e tre le impostazioni.
...
Metti la configurazione globale di RAILWISE (甬算) in `~/.config/railwise/railwise.json`. Usa la configurazione globale per preferenze valide per l'utente (ad es. temi, provider o keybind).
...
Quando RAILWISE (甬算) si avvia, cerca un file di configurazione nella directory corrente oppure risale fino alla directory Git piu vicina.
...
- `mdns` - Abilita la scoperta del servizio mDNS. Questo permette ad altri dispositivi sulla rete di scoprire il tuo server RAILWISE (甬算).
...
Puoi configurare provider e modelli da usare in RAILWISE (甬算) tramite le opzioni `provider`, `model` e `small_model`.
...
L'opzione `small_model` configura un modello separato per task leggeri come la generazione dei titoli. Per impostazione predefinita, RAILWISE (甬算) prova a usare un modello più economico se disponibile dal tuo provider, altrimenti fa fallback sul tuo modello principale.
...
Puoi configurare il tema da usare in RAILWISE (甬算) tramite l'opzione `theme`.
...
L'agente predefinito deve essere un agente primary (non un subagent). Puo essere un agente integrato come `"build"` o `"plan"`, oppure un [agente personalizzato](/docs/agents) che hai definito. Se l'agente specificato non esiste o e un subagent, RAILWISE (甬算) fara fallback su `"build"` mostrando un avviso.
...
RAILWISE (甬算) scarichera automaticamente eventuali aggiornamenti quando si avvia. Puoi disabilitare questa funzione con l'opzione `autoupdate`.
...
[I plugin](/docs/plugins) estendono RAILWISE (甬算) con strumenti personalizzati, hook e integrazioni.
...
E utile quando vuoi limitare RAILWISE (甬算) a usare solo alcuni provider invece di disabilitarli uno per uno.
