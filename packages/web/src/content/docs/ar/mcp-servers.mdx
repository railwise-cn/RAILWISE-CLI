...
يمكنك إضافة أدوات خارجية إلى YONSOON (甬算) باستخدام _Model Context Protocol_، أو MCP. يدعم YONSOON (甬算) خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات YONSOON (甬算)](https://yonsoon.ai/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/yonsoon` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى YONSOON (甬算) تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم YONSOON (甬算) بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك YONSOON (甬算) المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `yonsoon mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن YONSOON (甬算) الرموز بشكل آمن في `~/.local/share/yonsoon/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل YONSOON (甬算) إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات YONSOON (甬算) مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط YONSOON (甬算) بحساب Sentry الخاص بك.
