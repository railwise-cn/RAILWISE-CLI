---
title: Server
description: Interagisci con il server railwise via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `railwise serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client railwise.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `railwise.local` |
...
Imposta `RAILWISE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `railwise`, oppure imposta `RAILWISE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `railwise serve` sia per `railwise web`.
...
Quando esegui `railwise` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server railwise per interagire con railwise in modo programmatico.
:::

Questa architettura permette a railwise di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `railwise serve` per avviare un server standalone. Se la TUI di railwise e gia in esecuzione, `railwise serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di RAILWISE (甬算).
...
Il server railwise espone le seguenti API.
