...
Create `.yonsoon/commands/test.md`:
...
You can add custom commands through the YONSOON (甬算) config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your YONSOON (甬算) [config](/docs/config):
...
yonsoon includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
