...
Kreiraj `.yonsoon/commands/test.md`:
...
Možete dodati prilagođene komande kroz YONSOON (甬算) konfiguraciju ili kreiranjem markdown datoteka u direktoriju `commands/`.

---

### JSON

Koristite opciju `command` u svom YONSOON (甬算) [config](/docs/config):
...
- Globalno: `~/.config/yonsoon/commands/`
- Po projektu: `.yonsoon/commands/`
...
YONSOON (甬算) uključuje nekoliko ugrađenih naredbi kao što su `/init`, `/undo`, `/redo`, `/share`, `/help`; [saznaj više](/docs/tui#commands).
:::note
