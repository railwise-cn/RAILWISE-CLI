...
YONSOON (甬算) har en liste over tastebindinger som du kan tilpasse gjennom YONSOON (甬算)-konfigurasjonen.
...
YONSOON (甬算) bruker en `leader`-tast for de fleste tastebindinger. Dette unngår konflikter i terminalen din.
...
Prompt-feltet for YONSOON (甬算)-skrivebordsappen støtter vanlige snarveier i Readline/Emacs-stil for redigering av tekst. Disse er innebygd og foreløpig ikke konfigurerbare via `yonsoon.json`.
