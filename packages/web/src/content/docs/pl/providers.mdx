---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w railwise.
---

import config from "../../../../config.mjs"
export const console = config.console

railwise używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji railwise.
...
w `~/.local/share/railwise/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim railwise
config.
...
RAILWISE Zen to lista modeli dostarczonych przez zespół railwise, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z railwise. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od RAILWISE Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `RAILWISE Zen` i przejdź do [railwise.ai/auth](https://railwise.ai/zen).
...
Działa jak każdy inny dostawca w railwise i jest całkowicie opcjonalny w użyciu.

---

## RAILWISE Go

RAILWISE Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół railwise, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z railwise.

1. Uruchom polecenie `/connect` w TUI, wybierz `RAILWISE Go` i przejdź do [railwise.ai/auth](https://railwise.ai/zen).
...
Aby używać Amazon Bedrock z railwise:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania railwise:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `railwise.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w railwise nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować railwise do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod railwise działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji railwise.
...
railwise używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować railwise
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`railwise.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [railwise-gitlab-auth](https://www.npmjs.com/package/railwise-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem railwise:
...
Aby używać Google Vertex AI z railwise:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania railwise.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą railwise, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [railwise-helicone-session](https://github.com/H2Shami/railwise-helicone-session), aby automatycznie rejestrować każdą konwersację railwise jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja railwise będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować railwise tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować railwise tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować railwise tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem railwise. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/railwise).
:::
...
Aby korzystać z Ollama Cloud z railwise:
...
3. Skopiuj klucz API do użycia w railwise.
...
6. **Ważne**: Przed użyciem modeli chmurowych w railwise musisz pobrać informacje o modelu lokalnie:
...
RAILWISE Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół railwise. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>RAILWISE Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **RAILWISE Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API railwise.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji railwise.
...
5. Możesz je także dostosować za pomocą konfiguracji railwise. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z railwise:
...
Możesz także dostosować modele za pomocą konfiguracji railwise. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z railwise. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `railwise.json` w katalogu projektu:
...
Pola `limit` pozwalają railwise zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `railwise auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację railwise i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji railwise.
