...
test("已配置模型时首页任务可以创建会话并发送给 chief_manager", async ({ launchApp }) => {
  ...
  await expect(page.locator("[data-testid=session-turn-execution-capabilities]").first()).toContainText("轨道交通监测方案")
  await expect(page.locator("[data-testid=session-turn-execution-capabilities]").first()).toContainText("规范条文速查")
  ...
  await expect(page.locator("[data-testid=session-runtime-chain]")).toContainText("5 项能力")
  ...
  await expect(page.locator("[data-testid=session-runtime-capabilities]")).toHaveText("5 项能力")
  ...
  await expect(page.locator("[data-testid=session-runtime-capability-list]")).toContainText("轨道交通监测方案")
