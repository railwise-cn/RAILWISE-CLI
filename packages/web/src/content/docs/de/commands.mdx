...
Erstellen Sie `.yonsoon/commands/test.md`:

```md title=".yonsoon/commands/test.md"
---
...
Sie können benutzerdefinierte Befehle über die YONSOON (甬算)-Konfiguration hinzufügen oder indem Sie Markdown-Dateien im Verzeichnis `commands/` erstellen.

---

### JSON

Verwenden Sie die Option `command` in Ihrem YONSOON (甬算) [config](/docs/config):

```json title="yonsoon.jsonc" {4-12}
{
...
- Global: `~/.config/yonsoon/commands/`
- Pro Projekt: `.yonsoon/commands/`

```markdown title="~/.config/yonsoon/commands/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".yonsoon/commands/component.md"
---
...
```md title=".yonsoon/commands/create-file.md"
---
...
```md title=".yonsoon/commands/analyze-coverage.md"
---
...
```md title=".yonsoon/commands/review-changes.md"
---
...
```md title=".yonsoon/commands/review-component.md"
---
...
YONSOON (甬算) enthält mehrere integrierte Befehle wie `/init`, `/undo`, `/redo`, `/share`, `/help`; [mehr erfahren](/docs/tui#commands).
