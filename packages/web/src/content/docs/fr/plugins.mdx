---
title: Plugins
description: Écrivez vos propres plugins pour étendre RAILWISE (甬算).
---

Les plugins vous permettent d'étendre RAILWISE (甬算) en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de RAILWISE (甬算).
...
- `.railwise/plugins/` - Plugins au niveau du projet
- `~/.config/railwise/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/railwise/node_modules/`.
...
1. Configuration globale (`~/.config/railwise/railwise.json`)
2. Configuration du projet (`railwise.json`)
3. Répertoire global des plugins (`~/.config/railwise/plugins/`)
4. Répertoire des plugins du projet (`.railwise/plugins/`)
...
RAILWISE (甬算) exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK railwise pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre railwise.
...
:::note
Si vous utilisez l'application de bureau RAILWISE (甬算), elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez railwise de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à railwise :
...
L'assistant `tool` crée un outil personnalisé que railwise peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour railwise aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://railwise.ai/docs/sdk) pour plus de détails.
