...
test("resolves scoped npm plugins in config", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "railwise.json")).href
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("nb-railwise@2.4.3")).toBe("nb-railwise")
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@railwise/plugin@2.0.0")).toBe("@railwise/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["nb-railwise@2.4.3", "file:///project/.railwise/plugin/nb-railwise.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.railwise/plugin/nb-railwise.js")
  })
  ...
  test("local plugin directory overrides global railwise.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const railwiseDir = path.join(projectDir, ".railwise")
