---
title: Serveur
description: Interagissez avec le serveur railwise via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `railwise serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client railwise peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `railwise.local` |
...
Définissez `RAILWISE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `railwise`, ou définissez `RAILWISE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `railwise serve` et à `railwise web`.
...
Lorsque vous exécutez `railwise`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur railwise pour interagir avec railwise par programmation.
:::

Cette architecture permet à railwise de prendre en charge plusieurs clients et vous permet d'interagir avec railwise par programme.

Vous pouvez exécuter `railwise serve` pour démarrer un serveur autonome. Si vous avez le
railwise TUI en cours d'exécution, `railwise serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins RAILWISE (甬算) [IDE](/docs/ide).
...
Le serveur railwise expose les API suivantes.
