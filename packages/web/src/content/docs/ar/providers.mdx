---
title: المزوّدون
description: استخدام أي موفّر LLM في RAILWISE.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم RAILWISE ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات RAILWISE.
...
في `~/.local/share/railwise/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات RAILWISE.
...
RAILWISE Zen هي قائمة نماذج يوفّرها فريق RAILWISE وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع RAILWISE. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع RAILWISE Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر railwise، ثم انتقل إلى [railwise.ai/auth](https://railwise.ai/auth).
...
يعمل مثل أي مزوّد آخر في RAILWISE واستخدامه اختياري بالكامل.

---

## RAILWISE Go

RAILWISE Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق RAILWISE، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع RAILWISE.

1. شغّل الأمر `/connect` في TUI، واختر `RAILWISE Go`، ثم انتقل إلى [railwise.ai/auth](https://railwise.ai/zen).
...
لاستخدام Amazon Bedrock مع RAILWISE:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل railwise:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `railwise.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل RAILWISE غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين railwise لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل railwise بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات railwise.
...
تخزين الرمز في مساحة تخزين مصادقة railwise.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم RAILWISE نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر RAILWISE
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`railwise.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [railwise-gitlab-auth](https://www.npmjs.com/package/railwise-gitlab-auth).

##### التهيئة

خصّص عبر `railwise.json`:
...
لاستخدام اشتراك GitHub Copilot مع railwise:
...
لاستخدام Google Vertex AI مع RAILWISE:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر railwise، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [railwise-helicone-session](https://github.com/H2Shami/railwise-helicone-session) لتسجيل كل محادثة RAILWISE تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة RAILWISE مدرجة كجلسة مستقلة.
...
يمكنك ضبط railwise لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط railwise لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط railwise لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع RAILWISE. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/railwise) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع RAILWISE:
...
3. انسخ مفتاح API لاستخدامه في RAILWISE.
...
6. **مهم**: قبل استخدام نماذج السحابة في RAILWISE، يجب سحب معلومات النموذج محليا:
...
RAILWISE Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق RAILWISE. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>RAILWISE Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **RAILWISE Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ RAILWISE.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات railwise.
...
5. يمكنك أيضا تخصيصها عبر إعدادات railwise. إليك مثالا على تحديد مزوّد
...
يمكنك أيضا تخصيص النماذج عبر إعدادات railwise. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع railwise. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `railwise.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ RAILWISE بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `railwise auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات railwise وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات railwise.
