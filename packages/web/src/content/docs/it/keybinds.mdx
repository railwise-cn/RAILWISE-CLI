...
RAILWISE (甬算) ha una lista di scorciatoie che puoi personalizzare tramite la configurazione di RAILWISE (甬算).
...
RAILWISE (甬算) usa un tasto `leader` per la maggior parte delle scorciatoie. Questo evita conflitti nel terminale.
...
Il campo di input del prompt nell'app desktop di RAILWISE (甬算) supporta scorciatoie comuni in stile Readline/Emacs per modificare il testo. Sono integrate e al momento non configurabili tramite `railwise.json`.
