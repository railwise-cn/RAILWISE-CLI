...
async function railwiseFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "railwise"),
    ...(await Filesystem.findUp(["railwise.json", "railwise.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
