---
title: Server
description: Interact with yonsoon server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `yonsoon serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an yonsoon client can use.
...
Set `YONSOON_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `yonsoon`, or set `YONSOON_SERVER_USERNAME` to override it. This applies to both `yonsoon serve` and `yonsoon web`.
...
When you run `yonsoon` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the yonsoon server to interact with yonsoon programmatically.
:::

This architecture lets yonsoon support multiple clients and allows you to interact with yonsoon programmatically.

You can run `yonsoon serve` to start a standalone server. If you have the
yonsoon TUI running, `yonsoon serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the YONSOON (甬算) [IDE](/docs/ide) plugins.
...
The yonsoon server exposes the following APIs.
