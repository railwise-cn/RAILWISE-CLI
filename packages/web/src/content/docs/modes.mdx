...
:::caution
Modes are now configured through the `agent` option in the railwise config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in railwise allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the railwise config.
...
railwise comes with two built-in modes.
...
- `edit` - Cannot modify existing files, except for files located at `.railwise/plans/*.md` to detail the plan itself
...
Configure modes in your `railwise.json` config file:
...
both the global railwise config and the project specific config.
...
Create mode files in `.railwise/modes/` for project-specific modes or `~/.config/railwise/modes/` for global modes:
