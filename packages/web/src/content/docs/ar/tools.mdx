...
تتيح الأدوات لـ LLM تنفيذ إجراءات داخل قاعدة الشفرة الخاصة بك. يأتي RAILWISE (甬算) مع مجموعة من الأدوات المدمجة، لكن يمكنك توسيعه عبر [أدوات مخصصة](/docs/custom-tools) أو [خوادم MCP](/docs/mcp-servers).
...
فيما يلي جميع الأدوات المدمجة المتاحة في RAILWISE (甬算).
...
:::note
هذه الأداة متاحة فقط عند ضبط `RAILWISE_EXPERIMENTAL_LSP_TOOL=true` (أو `RAILWISE_EXPERIMENTAL=true`).
:::
...
:::note
هذه الأداة متاحة فقط عند استخدام مزود RAILWISE (甬算) أو عند ضبط متغير البيئة `RAILWISE_ENABLE_EXA` على أي قيمة منطقية صحيحة (مثل `true` أو `1`).

للتفعيل عند تشغيل RAILWISE (甬算):
