...
U RAILWISE (甬算) mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, RAILWISE (甬算) koristi nasu `railwise` temu.
...
RAILWISE (甬算) dolazi sa vise ugradenih tema.
...
- Zele da RAILWISE (甬算) odgovara izgledu njihovog terminala
...
RAILWISE (甬算) podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/railwise/themes/*.json` ili `$XDG_CONFIG_HOME/railwise/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.railwise/themes/*.json`
4. **Trenutni radni direktorij** - `./.railwise/themes/*.json`
