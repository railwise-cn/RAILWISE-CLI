---
title: Proveedores
description: Usando cualquier proveedor LLM en RAILWISE (甬算).
---

import config from "../../../../config.mjs"
export const console = config.console

RAILWISE (甬算) usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración RAILWISE (甬算).
...
Puedes personalizar los proveedores a través de la sección `provider` en tu RAILWISE (甬算)
configuración.
...
RAILWISE (甬算) Zen es una lista de modelos proporcionados por el equipo RAILWISE (甬算) que han sido
probado y verificado para funcionar bien con RAILWISE (甬算). [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con RAILWISE (甬算) Zen.
:::
...
Funciona como cualquier otro proveedor en RAILWISE (甬算) y su uso es completamente opcional.
...
Para usar Amazon Bedrock con RAILWISE (甬算):
...
:::info
El uso de su suscripción Claude Pro/Max en RAILWISE (甬算) no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
2. Elija su método de autenticación:
   ...
   3. Nombre: `RAILWISE (甬算)`, Alcances: `api`
...
RAILWISE (甬算) utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear RAILWISE (甬算)
...
Para utilizar Google Vertex AI con RAILWISE (甬算):
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [railwise-helicone-session](https://github.com/H2Shami/railwise-helicone-session) para registrar automáticamente cada conversación RAILWISE (甬算) como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación RAILWISE (甬算) enumerada como una sesión separada.
...
:::tip
Ollama puede configurarse automáticamente para RAILWISE (甬算). Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/railwise) para obtener más detalles.
:::
...
Para usar Ollama Cloud con RAILWISE (甬算):
...
3. Copie la clave API para usarla en RAILWISE (甬算).
...
6. **Importante**: Antes de usar modelos en la nube en RAILWISE (甬算), debe obtener la información del modelo localmente:
...
RAILWISE (甬算) Zen es una lista de modelos probados y verificados proporcionada por el equipo RAILWISE (甬算). [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>RAILWISE (甬算) Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **RAILWISE (甬算) Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave RAILWISE (甬算) API.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con RAILWISE (甬算):
...
Los campos `limit` le permiten a RAILWISE (甬算) comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
