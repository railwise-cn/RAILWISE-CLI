---
title: Dodaci
description: Napišite vlastite dodatke za proširenje YONSOON (甬算).
---

Dodaci vam omogućavaju da proširite YONSOON (甬算) spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja YONSOON (甬算).
...
- `.yonsoon/plugins/` - Dodaci na nivou projekta
- `~/.config/yonsoon/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/yonsoon/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/yonsoon/yonsoon.json`)
2. Konfiguracija projekta (`yonsoon.json`)
3. Globalni direktorij dodataka (`~/.config/yonsoon/plugins/`)
4. Direktorij dodataka projekta (`.yonsoon/plugins/`)
...
YONSOON (甬算) pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Yonsoon SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje YONSOON (甬算).
...
:::note
Ako koristite desktop aplikaciju YONSOON (甬算), ona može automatski slati sistemske obavijesti kada je odgovor spreman ili kada dođe do greške u sesiji.
:::
...
Spriječite yonsoon da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u yonsoon:
...
Pomoćnik `tool` kreira prilagođeni alat koji yonsoon može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za yonsoon zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://yonsoon.ai/docs/sdk) za detalje.
