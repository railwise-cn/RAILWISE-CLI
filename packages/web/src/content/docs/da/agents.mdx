...
Der er to typer agenter i RAILWISE (甬算); primære agenter og subagenter.
...
RAILWISE (甬算) leveres med indbyggede primære agenter, **Build** og **Plan**. Vi vil
...
RAILWISE (甬算) leveres med to indbyggede underagenter, **Generelt** og **Udforsk**. Vi vil se på dette nedenfor.

---

## Indbyggede

RAILWISE (甬算) leveres med to indbyggede primære agenter og to indbyggede subagenter.
...
Hvis ingen temperatur er angivet, bruger RAILWISE (甬算) modelspecifikke standardindstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne sti er i forhold til, hvor konfigurationsfilen er placeret. Så dette virker både for den globale RAILWISE (甬算)-konfiguration og den projektspecifikke konfiguration.
...
Model-id'et i din RAILWISE (甬算)-konfiguration brugerformatet `provider/model-id`. For eksempel, hvis du bruger [RAILWISE (甬算) Zen](/docs/zen), vil du bruge `railwise/gpt-5.1-codex` til GPT 5.1 Codex.
