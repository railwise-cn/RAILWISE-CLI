---
title: Servidor
description: Interactuar con el servidor yonsoon a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `yonsoon serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente yonsoon puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `yonsoon.local` |
...
Configure `YONSOON_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `yonsoon`, o configure `YONSOON_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `yonsoon serve` como a `yonsoon web`.
...
Cuando ejecuta `yonsoon`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor yonsoon para interactuar con yonsoon mediante programación.
:::

Esta arquitectura permite que yonsoon admita múltiples clientes y le permite interactuar con yonsoon mediante programación.

Puede ejecutar `yonsoon serve` para iniciar un servidor independiente. Si tienes el
yonsoon TUI ejecutándose, `yonsoon serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos YONSOON (甬算) [IDE](/docs/ide).
...
El servidor yonsoon expone las siguientes API.
