...
Puoi aggiungere strumenti esterni a RAILWISE usando il _Model Context Protocol_, o MCP. RAILWISE supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [RAILWISE Config](https://railwise.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
RAILWISE gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, RAILWISE:
...
Se il server richiede autenticazione, RAILWISE ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `railwise mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, RAILWISE memorizzera' i token in modo sicuro in `~/.local/share/railwise/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in RAILWISE insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione RAILWISE come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare RAILWISE al tuo account Sentry.
