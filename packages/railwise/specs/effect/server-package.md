...
Practical reference for a future `packages/server` split after the railwise
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/railwise`.
...
Until enough shared service code lives outside `packages/railwise`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/railwise`

It should not import `packages/railwise` services while `packages/railwise`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on railwise-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/railwise` implementation details.
