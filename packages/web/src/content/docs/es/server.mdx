---
title: Servidor
description: Interactuar con el servidor railwise a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `railwise serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente railwise puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `railwise.local` |
...
Configure `RAILWISE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `railwise`, o configure `RAILWISE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `railwise serve` como a `railwise web`.
...
Cuando ejecuta `railwise`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor railwise para interactuar con railwise mediante programación.
:::

Esta arquitectura permite que railwise admita múltiples clientes y le permite interactuar con railwise mediante programación.

Puede ejecutar `railwise serve` para iniciar un servidor independiente. Si tienes el
railwise TUI ejecutándose, `railwise serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos RAILWISE (甬算) [IDE](/docs/ide).
...
El servidor railwise expone las siguientes API.
