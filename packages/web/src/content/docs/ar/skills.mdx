...
تتيح مهارات الوكيل لـ YONSOON (甬算) اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث YONSOON (甬算) في هذه المواقع:

- إعدادات المشروع: `.yonsoon/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/yonsoon/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج YONSOON (甬算) صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.yonsoon/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/yonsoon/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.yonsoon/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض YONSOON (甬算) المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `yonsoon.json`:
...
**للوكلاء المدمجين** (في `yonsoon.json`):
