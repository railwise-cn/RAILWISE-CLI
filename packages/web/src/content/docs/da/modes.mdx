...
:::caution
Tilstande er nu konfigureret gennem indstillingen `agent` i yonsoon-konfigurationen. De
`mode` mulighed er nu forældet. [Learn more](/docs/agents).
:::

Tilstande i yonsoon giver dig mulighed for at tilpasse adfærd, værktøjer og prompter til forskellige brugstilfælde.

Den kommer med to indbyggede tilstande: **build** og **plan**. Du kan tilpasse
disse eller konfigurer dine egne gennem yonsoon-konfigurationen.
...
yonsoon leveres med to indbyggede tilstande.
...
- `edit` - Kan ikke ændre eksisterende filer, undtagen filer placeret på `.yonsoon/plans/*.md` for at detaljere selve planen
...
Konfigurer tilstande i din `yonsoon.json`-konfigurationsfil:
...
Hvis der ikke er angivet nogen temperatur, bruger yonsoon modelspecifikke standarder (typisk 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
både den globale yonsoon-konfiguration og den projektspecifikke konfiguration.
...
Opret tilstandsfiler i `.yonsoon/modes/` for projektspecifikke tilstande eller `~/.config/yonsoon/modes/` for globale tilstande:
