...
export async function spawnWslSidecar(
  ...
  const railwise = await resolveWslOpencode(distro)
  if (!railwise) throw new Error(`RAILWISE is not installed in ${distro}`)

  const port = await allocatePort()
  const password = randomUUID()
  const username = "railwise"
  const script = [
    "set -euo pipefail",
    'cd "$HOME" || cd /',
    ...
    "export PATH",
    "export WSLENV=",
    "export RAILWISE_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export RAILWISE_CLIENT=desktop",
    `export RAILWISE_SERVER_USERNAME=${shellEscape(username)}`,
    `export RAILWISE_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(railwise)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
