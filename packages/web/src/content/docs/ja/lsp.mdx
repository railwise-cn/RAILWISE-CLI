---
title: LSPサーバー
description: RAILWISE (甬算) は LSP サーバーと統合します。
---

RAILWISE (甬算) は言語サーバープロトコル (LSP) と統合して、LLM がコードベースと対話できるようにします。診断を使用して LLM にフィードバックを提供します。

---

## 組み込み

RAILWISE (甬算) には、一般的な言語用のいくつかの組み込み LSP サーバーが付属しています。
...
:::note
`RAILWISE_DISABLE_LSP_DOWNLOAD` 環境変数を `true` に設定すると、LSP サーバーの自動ダウンロードを無効にできます。
:::
...
RAILWISE (甬算) がファイルを開くと、次のようになります。
...
RAILWISE (甬算) 設定の `lsp` セクションを通じて LSP サーバーをカスタマイズできます。
