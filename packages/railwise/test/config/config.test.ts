...
async function writeManagedSettings(settings: object, filename = "railwise.json") {
...
test("creates global jsonc config with schema when no global configs exist", async () => {
  ...
  try {
    ...
    const content = await Filesystem.readText(path.join(tmp.path, "railwise.jsonc"))
...
test("does not create global config when RAILWISE_CONFIG_DIR is set", async () => {
  await using tmp = await tmpdir()
  await using custom = await tmpdir()
  const prevConfig = Global.Path.config
  const prevEnv = process.env.RAILWISE_CONFIG_DIR
  ;(Global.Path as { config: string }).config = tmp.path
  process.env.RAILWISE_CONFIG_DIR = custom.path
  await clear(true)
  ...
  try {
    ...
    expect(await Filesystem.exists(path.join(tmp.path, "railwise.jsonc"))).toBe(false)
  } finally {
    ;(Global.Path as { config: string }).config = prevConfig
    if (prevEnv === undefined) delete process.env.RAILWISE_CONFIG_DIR
    else process.env.RAILWISE_CONFIG_DIR = prevEnv
    await clear(true)
  }
})
...
test("updates global config and omits empty shell key in json", async () => {
  ...
  try {
    ...
    const writtenConfig = await Filesystem.readJson<{ shell?: string }>(path.join(tmp.path, "railwise.json"))
...
test("updates global config and omits empty shell key in jsonc", async () => {
  ...
  try {
    ...
    const file = path.join(tmp.path, "railwise.jsonc")
...
test("ignores legacy tui keys in railwise config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://railwise.ai/config.json",
  ...
  try {
    ...
    await WithInstance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "railwise.json"))
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["RAILWISE_CONSOLE_TOKEN"]
  ...
  try {
    ...
    if (originalControlToken !== undefined) {
      process.env["RAILWISE_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["RAILWISE_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "railwise.json"), "{ invalid json }")
    },
  })
  await provideTestInstance({
    directory: tmp.path,
...
test("agent markdown permission config preserves user key order", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const agentDir = path.join(dir, ".railwise", "agent")
      await fs.mkdir(agentDir, { recursive: true })
...
test("does not try to install dependencies in read-only RAILWISE_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.RAILWISE_CONFIG_DIR
  process.env.RAILWISE_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.RAILWISE_CONFIG_DIR
    else process.env.RAILWISE_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable RAILWISE_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "railwise.json"),
        JSON.stringify({ $schema: "https://railwise.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".railwise")
...
test("project config overrides remote well-known config", async () => {
  ...
  globalThis.fetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/railwise")) {
      fetchedUrl = urlStr
    ...
    await provideTmpdirInstance(
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            const config = yield* svc.get()
            expect(fetchedUrl).toBe("https://example.com/.well-known/railwise")
    ...
    await provideTmpdirInstance(
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            const config = yield* svc.get()
            expect(wellknownFetchedUrl).toBe("https://example.com/.well-known/railwise")
            expect(remoteFetchedUrl).toBe("https://config.example.com/railwise.json")
...
describe("RAILWISE_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["RAILWISE_DISABLE_PROJECT_CONFIG"]
    process.env["RAILWISE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["RAILWISE_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["RAILWISE_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .railwise/ directories when flag is set", async () => {
    const originalEnv = process.env["RAILWISE_DISABLE_PROJECT_CONFIG"]
    process.env["RAILWISE_DISABLE_PROJECT_CONFIG"] = "true"
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["RAILWISE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["RAILWISE_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["RAILWISE_CONFIG_DIR"]
      process.env["RAILWISE_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["RAILWISE_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["RAILWISE_CONFIG_DIR"]
      } else {
        process.env["RAILWISE_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("RAILWISE_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["RAILWISE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["RAILWISE_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["RAILWISE_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["RAILWISE_CONFIG_DIR"] = configDirTmp.path

      await WithInstance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await load()
          // Should load from RAILWISE_CONFIG_DIR, not project
...
describe("RAILWISE_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in RAILWISE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["RAILWISE_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["RAILWISE_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://railwise.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["RAILWISE_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["RAILWISE_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in RAILWISE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["RAILWISE_CONFIG_CONTENT"]
