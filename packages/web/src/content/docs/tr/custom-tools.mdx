---
title: Özel Araçlar
description: LLM'un railwise'da çağırabileceği araçları birleştirebilir.
---

Özel araçlar, LLM'un konuşmalar sırasında arayabileceği, oluşturduğunuz işlevlerdir. railwise'un [built-in tools](/docs/tools) ile birlikte `read`, `write` ve `bash` gibi çalışırlar.
...
- Yerel olarak bunları projenizin `.railwise/tools/` dizinine yerleştirerek.
- Veya küresel olarak bunları `~/.config/railwise/tools/` içine yerleştirerek.
