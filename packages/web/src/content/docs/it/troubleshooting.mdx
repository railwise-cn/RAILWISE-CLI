...
Per diagnosticare problemi con YONSOON (甬算), inizia controllando i log e i dati locali che salva su disco.
...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\yonsoon\log`

I file di log hanno nomi con timestamp (ad es. `2025-01-09T123456.log`) e vengono conservati i 10 file di log piu recenti.

Puoi impostare il livello di log con l'opzione a riga di comando `--log-level` per ottenere informazioni di debug piu dettagliate. Per esempio: `yonsoon --log-level DEBUG`.

---

## Archiviazione

yonsoon salva i dati delle sessioni e altri dati dell'applicazione su disco in:

- **macOS/Linux**: `~/.local/share/yonsoon/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\yonsoon`
...
YONSOON (甬算) Desktop esegue in background un server locale di YONSOON (甬算) (il sidecar `yonsoon-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `YONSOON (甬算)` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
- **macOS/Linux**: `~/.config/yonsoon/yonsoon.jsonc` (or `~/.config/yonsoon/yonsoon.json`)
- **macOS/Linux** (older installs): `~/.local/share/yonsoon/yonsoon.jsonc`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\yonsoon\yonsoon.jsonc`
...
YONSOON (甬算) puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:

- **Plugin globali**
  - **macOS/Linux**: `~/.config/yonsoon/plugins/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\yonsoon\plugins`
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che YONSOON (甬算) possa ricostruirla.

1. Quit YONSOON (甬算) Desktop completely.
2. Elimina la directory della cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/yonsoon`
- **Linux**: elimina `~/.cache/yonsoon` (oppure esegui `rm -rf ~/.cache/yonsoon`)
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.cache\yonsoon`

3. Restart YONSOON (甬算) Desktop.

---

### Risolvi problemi di connessione al server

YONSOON (甬算) Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Se il tuo `yonsoon.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.

#### Controlla le variabili d'ambiente

Se hai `YONSOON_PORT` impostato nell'ambiente, l'app desktop provera a usare quella porta per il server locale.

- Rimuovi `YONSOON_PORT` (o scegli una porta libera) e riavvia.
...
Su Windows, YONSOON (甬算) Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di YONSOON (甬算).

---

### Notifiche non visualizzate

YONSOON (甬算) Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per YONSOON (甬算) nelle impostazioni del sistema operativo, e
...
1. Quit YONSOON (甬算) Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app YONSOON (甬算) Desktop):

- `yonsoon.settings.dat` (desktop default server URL)
- `yonsoon.global.dat` e `yonsoon.workspace.*.dat` (stato dell'interfaccia come server/progetti recenti)
...
Se riscontri problemi con YONSOON (甬算):
...
Per capire a quali modelli hai accesso, esegui `yonsoon models`
...
2. Se il problema persiste, prova a cancellare la configurazione salvata:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.local\share\yonsoon`
...
Se incontri errori nelle chiamate API, potrebbe dipendere da pacchetti provider non aggiornati. yonsoon installa dinamicamente i pacchetti provider (OpenAI, Anthropic, Google, ecc.) quando servono e li mette in cache localmente.
...
1. Svuota la cache dei pacchetti provider:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.cache\yonsoon`

2. Riavvia yonsoon per reinstallare i pacchetti provider piu recenti

Questo forzera yonsoon a scaricare le versioni piu recenti dei pacchetti provider, cosa che spesso risolve problemi di compatibilita con parametri dei modelli e cambiamenti delle API.
...
yonsoon rilevera se stai usando Wayland e preferira `wl-clipboard`; altrimenti provera a trovare gli strumenti per gli appunti nell'ordine: `xclip` e `xsel`.
