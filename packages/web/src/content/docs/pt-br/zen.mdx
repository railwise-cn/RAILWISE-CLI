...
O RAILWISE (甬算) Zen é uma lista de modelos testados e verificados fornecidos pela equipe do railwise.

:::note
O RAILWISE (甬算) Zen está atualmente em beta.
:::

O Zen funciona como qualquer outro provedor no railwise. Você faz login no RAILWISE (甬算) Zen e obtém sua chave de API. É **completamente opcional** e você não precisa usá-lo para utilizar o railwise.
...
O RAILWISE (甬算) Zen é um gateway de IA que lhe dá acesso a esses modelos.

---

## Como funciona

O RAILWISE (甬算) Zen funciona como qualquer outro provedor no railwise.

1. Você faz login no **<a href={console}>RAILWISE (甬算) Zen</a>**, adiciona seus dados de cobrança e copia sua chave de API.
2. Você executa o comando `/connect` no TUI, seleciona RAILWISE (甬算) Zen e cola sua chave de API.
...
O [id do modelo](/docs/config/#models) na sua configuração do railwise usa o formato `railwise/<model-id>`. Por exemplo, para GPT 5.2 Codex, você usaria `railwise/gpt-5.2-codex` na sua configuração.
...
- GLM 4.7 Free está disponível no railwise por tempo limitado. A equipe está usando esse tempo para coletar feedback e melhorar o modelo.
- Kimi K2.5 Free está disponível no railwise por tempo limitado. A equipe está usando esse tempo para coletar feedback e melhorar o modelo.
- MiniMax M2.1 Free está disponível no railwise por tempo limitado. A equipe está usando esse tempo para coletar feedback e melhorar o modelo.
- Big Pickle é um modelo oculto que está gratuito no railwise por tempo limitado. A equipe está usando esse tempo para coletar feedback e melhorar o modelo.
...
Criamos o RAILWISE (甬算) Zen para:
...
4. Não ter **vinculação** permitindo que você o use com qualquer outro agente de codificação. E sempre permitir que você use qualquer outro provedor com o railwise também.
