---
title: Konfiguracija
description: Korištenje YONSOON (甬算) JSON konfiguracije.
---

Možete konfigurirati YONSOON (甬算) koristeći JSON konfiguracijski fajl.

---

## Format

YONSOON (甬算) podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
Na primjer, ako vaša globalna konfiguracija postavlja `theme: "yonsoon"` i `autoupdate: true`, a vaša projektna konfiguracija postavlja `model: "anthropic/claude-sonnet-4-5"`, konačna konfiguracija će uključivati ​​sve tri postavke.
...
1. **Udaljena konfiguracija** (od `.well-known/yonsoon`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/yonsoon/yonsoon.json`) - korisničke postavke
3. **Prilagođena konfiguracija** (`YONSOON_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`yonsoon.json` u projektu) - postavke specifične za projekat
5. **`.yonsoon` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`YONSOON_CONFIG_CONTENT` env var) - runtime preinačenja
   ...
   :::note
   Direktoriji `.yonsoon` i `~/.config/yonsoon` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
   :::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/yonsoon` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu YONSOON (甬算) konfiguraciju u `~/.config/yonsoon/yonsoon.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su teme, provajderi ili veze tipki.
...
Dodajte `yonsoon.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
:::tip
Postavite specifičnu konfiguraciju projekta u korijen vašeg projekta.
:::
Kada se YONSOON (甬算) pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `YONSOON_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `YONSOON_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.yonsoon` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.yonsoon`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`yonsoon.ai/config.json`**](https://yonsoon.ai/config.json).
...
Možete konfigurirati postavke servera za naredbe `yonsoon serve` i `yonsoon web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš YONSOON (甬算) server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `yonsoon.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj YONSOON (甬算) konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, YONSOON (甬算) pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Možete konfigurirati temu koju želite koristiti u svojoj YONSOON (甬算) konfiguraciji putem opcije `theme`.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/yonsoon/agents/` ili `.yonsoon/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, YONSOON (甬算) će se vratiti na `"build"` s upozorenjem.
Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`yonsoon run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/yonsoon/commands/` ili `.yonsoon/commands/`. [Saznajte više ovdje](/docs/commands).
...
YONSOON (甬算) će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, YONSOON (甬算) **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju YONSOON (甬算) sa prilagođenim alatima, kukicama i integracijama.
Postavite datoteke dodataka u `.yonsoon/plugins/` ili `~/.config/yonsoon/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite YONSOON (甬算) da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
:::note
`disabled_providers` ima prioritet nad `enabled_providers`.
:::
