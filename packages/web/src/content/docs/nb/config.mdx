---
title: Konfigurasjon
description: Ved å bruke railwise JSON-konfigurasjonen.
---

Du kan konfigurere RAILWISE (甬算) ved å bruke en JSON konfigurasjonsfil.

---

## Format

RAILWISE (甬算) støtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
For eksempel, hvis din globale konfigurasjon setter `theme: "railwise"` og `autoupdate: true`, og prosjektkonfigurasjonen setter `model: "anthropic/claude-sonnet-4-5"`, vil den endelige konfigurasjonen inkludere alle tre innstillingene.
...
1. **Ekstern konfigurasjon** (fra `.well-known/railwise`) - organisasjonsstandarder
2. **Global konfigurasjon** (`~/.config/railwise/railwise.json`) - brukerinnstillinger
3. **Egendefinert konfigurasjon** (`RAILWISE_CONFIG` env var) - egendefinerte overstyringer
4. **Prosjektkonfigurasjon** (`railwise.json` i prosjekt) - prosjektspesifikke innstillinger
5. **`.railwise` kataloger** - agenter, kommandoer, plugins
6. **Inline config** (`RAILWISE_CONFIG_CONTENT` env var) - kjøretidsoverstyringer
...
:::note
`.railwise` og `~/.config/railwise` katalogene bruker **flertallsnavn** på underkataloger: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkeltnavn (f.eks. `agent/`) støttes også for bakoverkompatibilitet.
:::
...
Organisasjoner kan tilby standardkonfigurasjon via `.well-known/railwise`-endepunktet. Dette hentes automatisk når du autentiserer med en leverandør som støtter det.
...
Plasser din globale RAILWISE (甬算)-konfigurasjon i `~/.config/railwise/railwise.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Legg til `railwise.json` i prosjektroten din. Prosjektkonfigurasjon har den høyeste prioritet blant standard config-filer - den overstyrer både globale og eksterne konfigurasjoner.

:::tip
Plasser prosjektspesifikk konfigurasjon i roten til prosjektet ditt.
:::

Når RAILWISE (甬算) starter opp, ser den etter en konfigurasjonsfil i gjeldende katalog eller går opp til nærmeste Git-katalog.
...
Spesifiser en tilpasset konfigurasjonsfilbane ved å bruke miljøvariabelen `RAILWISE_CONFIG`.
...
Spesifiser en tilpasset konfigurasjonskatalog ved å bruke `RAILWISE_CONFIG_DIR`
miljøvariabel. Denne katalogen vil bli søkt etter agenter, kommandoer,
moduser og plugins akkurat som standard `.railwise` katalog, og bør
...
Den egendefinerte katalogen lastes inn etter den globale konfigurasjonen og `.railwise`-katalogene, slik at den **kan overstyre** innstillingene deres.

---

## Skjema

Konfigurasjonsfilen har et skjema som er definert i [**`railwise.ai/config.json`**](https://railwise.ai/config.json).
...
Du kan konfigurere serverinnstillinger for kommandoene `railwise serve` og `railwise web` gjennom alternativet `server`.
...
- `mdns` - Aktiver mDNS-tjenesteoppdagelse. Dette lar andre enheter på nettverket oppdage RAILWISE (甬算)-serveren din.
- `mdnsDomain` - Egendefinert domenenavn for mDNS-tjeneste. Standard er `railwise.local`. Nyttig for å kjøre flere forekomster på samme nettverk.
...
Du kan konfigurere leverandørene og modellene du vil bruke i RAILWISE (甬算)-konfigurasjonen gjennom alternativene `provider`, `model` og `small_model`.
...
Alternativet `small_model` konfigurerer en egen modell for lette oppgaver som tittelgenerering. Som standard prøver RAILWISE (甬算) å bruke en rimeligere modell hvis en er tilgjengelig fra leverandøren din, ellers faller den tilbake til hovedmodellen din.
...
Du kan konfigurere temaet du vil bruke i RAILWISE (甬算)-konfigurasjonen gjennom alternativet `theme`.
...
Du kan også definere agenter ved å bruke markdown-filer i `~/.config/railwise/agents/` eller `.railwise/agents/`. [Les mer her](/docs/agents).
...
Standardagenten må være en primæragent (ikke en underagent). Dette kan være en innebygd agent som `"build"` eller `"plan"`, eller en [egendefinert agent](/docs/agents) du har definert. Hvis den angitte agenten ikke eksisterer eller er en subagent, vil RAILWISE (甬算) falle tilbake til `"build"` med en advarsel.

Denne innstillingen gjelder for alle grensesnitt: TUI, CLI (`railwise run`), skrivebordsapp og GitHub Action.
...
Du kan også definere kommandoer ved å bruke markdown-filer i `~/.config/railwise/commands/` eller `.railwise/commands/`. [Les mer her](/docs/commands).
...
RAILWISE (甬算) vil automatisk laste ned eventuelle nye oppdateringer når den starter opp. Du kan deaktivere dette med alternativet `autoupdate`.
...
Som standard **tillater RAILWISE (甬算) alle operasjoner** uten å kreve eksplisitt godkjenning. Du kan endre dette ved å bruke alternativet `permission`.
...
[Plugins](/docs/plugins) utvider RAILWISE (甬算) med tilpassede verktøy, kroker og integrasjoner.

Plasser plugin-filer i `.railwise/plugins/` eller `~/.config/railwise/plugins/`. Du kan også laste inn plugins fra npm gjennom alternativet `plugin`.
...
Dette er nyttig når du vil begrense RAILWISE (甬算) til kun å bruke spesifikke leverandører i stedet for å deaktivere dem én etter én.
