---
title: Formattere
description: YONSOON (甬算) bruker språkspesifikke formattere.
---

YONSOON (甬算) formaterer automatisk filer etter at de er skrevet eller redigert ved hjelp av språkspesifikke formattere. Dette sikrer at koden som genereres følger kodestilene til prosjektet ditt.

---

## Innebygde formattere

YONSOON (甬算) kommer med flere innebygde formattere for populære språk og rammeverk. Nedenfor er en liste over formattere, støttede filendelser og kommandoer eller konfigurasjonsalternativer den krever.
...
Så hvis prosjektet ditt har `prettier` i `package.json`, vil YONSOON (甬算) automatisk bruke det.

---

## Slik fungerer det

Når YONSOON (甬算) skriver eller redigerer en fil, gjør den:
...
Du kan tilpasse formattere gjennom `formatter`-delen i YONSOON (甬算)-konfigurasjonen.
