---
title: Konfiguracja
description: Ustawienia YONSOON (甬算) w pliku JSON.
---

Możesz dostosować YONSOON (甬算) za pomocą pliku konfiguracyjnego JSON.

---

## Format

YONSOON (甬算) obsługuje formaty **JSON** i **JSONC** (JSON z komentarzami).
...
Na przykład, jeśli konfiguracja globalna ustawi `theme: "yonsoon"` i `autoupdate: true`, a konfiguracja projektu ustawi `model: "anthropic/claude-sonnet-4-5"`, ostateczna konfiguracja będzie zawierać wszystkie trzy ustawienia.
...
1. **Konfiguracja zdalna** (z `.well-known/yonsoon`) – ustawienia wymuszane przez organizację
2. **Konfiguracja globalna** (`~/.config/yonsoon/yonsoon.json`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `YONSOON_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`yonsoon.json` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.yonsoon`** - agenci, polecenia, umiejętności
6. **Konfiguracja wbudowana** (zmienna `YONSOON_CONFIG_CONTENT`) — nadpisanie środowiska uruchomieniowego
...
:::note
W katalogach `.yonsoon` i `~/.config/yonsoon` używane są **nazwy w liczbie mnogiej** dla podkatalogów: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Nazwy w liczbie pojedynczej (np. `agent/`) są również obsługiwane dla kompatybilności wstecznej.
:::
...
Organizacje mogą publikować konfigurację pod adresem `.well-known/yonsoon`. Jest ona pobierana automatycznie po uwierzytelnieniu u dostawcy, który ją obsługuje.
...
Umieść swoją globalną konfigurację YONSOON (甬算) w `~/.config/yonsoon/yonsoon.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak motywy, domyślny dostawca lub skróty klawiszowe.
...
Dodaj `yonsoon.json` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.

:::tip
Dodaj ten plik do kontroli wersji, aby udostępniać konfigurację zespołowi.
:::

Kiedy YONSOON (甬算) się uruchamia, szuka pliku konfiguracyjnego w katalogu głównym repozytorium Git.
...
Możesz załadować niestandardowy plik konfiguracyjny, ustawiając zmienną środowiskową `YONSOON_CONFIG`.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `YONSOON_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.yonsoon` i powinien zachować tę samą strukturę.
...
Katalog niestandardowy jest ładowany po globalnej konfiguracji i katalogach `.yonsoon`, więc **może być łączony** z ich zawartością.

---

## Schemat

Plik konfiguracyjny ma schemat JSON dostępny pod adresem [**`yonsoon.ai/config.json`**](https://yonsoon.ai/config.json).
...
Możesz skonfigurować serwer dla `yonsoon serve` i `yonsoon web` za pomocą opcji `server`.
...
- `mdns` - Włącz wykrywanie usług mDNS. Dzięki temu inne urządzenia w sieci mogą łatwo znaleźć Twój serwer YONSOON (甬算).
- `mdnsDomain` - Niestandardowa nazwa domeny dla usług mDNS. Domyślnie `yonsoon.local`. Przydatne do uruchamiania wielu instancji w tej samej sieci.
...
Opcja `small_model` konfiguruje oddzielny model do lżejszych zadań, takich jak generowanie tytułów. Domyślnie YONSOON (甬算) wybiera tańszy model od tego samego dostawcy, jeśli jest dostępny, w przeciwnym razie powraca do modelu głównego.
...
Skonfiguruj motyw interfejsu YONSOON (甬算) za pomocą opcji `theme`.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/yonsoon/agents/` lub `.yonsoon/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Agent domyślny musi być agentem typu `primary` (nie `subagent`). Może to być agent wbudowany, taki jak `"build"` lub `"plan"`, albo zdefiniowany przez Ciebie [agent niestandardowy](/docs/agents). Jeśli agent nie istnieje lub jest subagentem, YONSOON (甬算) powróci do `"build"` z ostrzeżeniem.

Dotyczy to wszystkich interfejsów: TUI, CLI (`yonsoon run`), aplikacji desktopowej i GitHub Actions.
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/yonsoon/commands/` lub `.yonsoon/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
YONSOON (甬算) może automatycznie pobierać nowe wersje. Możesz to kontrolować za pomocą opcji `autoupdate`.
...
Działa to tylko wtedy, gdy YONSOON (甬算) nie został zainstalowany przez menedżera pakietów, takiego jak Homebrew.
...
Domyślnie YONSOON (甬算) **zezwala na wszystkie działania** bez konieczności zatwierdzania. Możesz to zmienić za pomocą opcji `permission`.
...
[Wtyczki](/docs/plugins) rozszerzają YONSOON (甬算) o niestandardowe narzędzia, hooki i integracje.

Umieść pliki wtyczek w `.yonsoon/plugins/` lub `~/.config/yonsoon/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
...
Jest to przydatne, gdy chcesz ograniczyć użycie YONSOON (甬算) tylko do określonych dostawców.
