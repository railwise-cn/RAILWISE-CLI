...
Opprett `.yonsoon/commands/test.md`:
...
Du kan legge til egendefinerte kommandoer gjennom YONSOON (甬算)-konfigurasjonen eller ved å lage markdown-filer i katalogen `commands/`.

---

### JSON

Bruk alternativet `command` i YONSOON (甬算) [config](/docs/config):
...
YONSOON (甬算) inkluderer flere innebygde kommandoer som `/init`, `/undo`, `/redo`, `/share`, `/help`; [finn ut mer](/docs/tui#commands).
