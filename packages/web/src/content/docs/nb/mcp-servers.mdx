...
Du kan legge til eksterne verktøy i RAILWISE ved å bruke _Model Context Protocol_ eller MCP. RAILWISE støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [RAILWISE Config](https://railwise.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.
...
RAILWISE håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil RAILWISE:
...
Hvis serveren krever autentisering, vil RAILWISE be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `railwise mcp auth <server-name>`.
...
Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil RAILWISE lagre tokens sikkert i `~/.local/share/railwise/mcp-auth.json`.
...
MCP-ene dine er tilgjengelige som verktøy i RAILWISE, sammen med innebygde verktøy. Så du kan administrere dem gjennom RAILWISE-konfigurasjonen som alle andre verktøy.
...
Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble RAILWISE til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver RAILWISE-samtale oppført som en separat økt.
