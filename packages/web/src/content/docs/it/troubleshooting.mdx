...
Per diagnosticare problemi con RAILWISE (甬算), inizia controllando i log e i dati locali che salva su disco.
...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\railwise\log`

I file di log hanno nomi con timestamp (ad es. `2025-01-09T123456.log`) e vengono conservati i 10 file di log piu recenti.

Puoi impostare il livello di log con l'opzione a riga di comando `--log-level` per ottenere informazioni di debug piu dettagliate. Per esempio: `railwise --log-level DEBUG`.

---

## Archiviazione

railwise salva i dati delle sessioni e altri dati dell'applicazione su disco in:

- **macOS/Linux**: `~/.local/share/railwise/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\railwise`
...
RAILWISE (甬算) Desktop esegue in background un server locale di RAILWISE (甬算) (il sidecar `railwise-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `RAILWISE (甬算)` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\railwise\railwise.jsonc`
...
RAILWISE (甬算) puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:

- **Plugin globali**
  - **macOS/Linux**: `~/.config/railwise/plugins/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\railwise\plugins`
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che RAILWISE (甬算) possa ricostruirla.

1. Quit RAILWISE (甬算) Desktop completely.
2. Elimina la directory della cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/railwise`
- **Linux**: elimina `~/.cache/railwise` (oppure esegui `rm -rf ~/.cache/railwise`)
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.cache\railwise`
...
RAILWISE (甬算) Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Se il tuo `railwise.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.

#### Controlla le variabili d'ambiente

Se hai `RAILWISE_PORT` impostato nell'ambiente, l'app desktop provera a usare quella porta per il server locale.

- Rimuovi `RAILWISE_PORT` (o scegli una porta libera) e riavvia.
...
Su Windows, RAILWISE (甬算) Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di RAILWISE (甬算).

---

### Notifiche non visualizzate

RAILWISE (甬算) Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per RAILWISE (甬算) nelle impostazioni del sistema operativo, e
...
2. Trova ed elimina questi file (si trovano nella directory dati dell'app RAILWISE (甬算) Desktop):

- `railwise.settings.dat` (desktop default server URL)
- `railwise.global.dat` e `railwise.workspace.*.dat` (stato dell'interfaccia come server/progetti recenti)
...
Se riscontri problemi con RAILWISE (甬算):
...
Per capire a quali modelli hai accesso, esegui `railwise models`
...
2. Se il problema persiste, prova a cancellare la configurazione salvata:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.local\share\railwise`
...
Se incontri errori nelle chiamate API, potrebbe dipendere da pacchetti provider non aggiornati. railwise installa dinamicamente i pacchetti provider (OpenAI, Anthropic, Google, ecc.) quando servono e li mette in cache localmente.
...
1. Svuota la cache dei pacchetti provider:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.cache\railwise`

2. Riavvia railwise per reinstallare i pacchetti provider piu recenti

Questo forzera railwise a scaricare le versioni piu recenti dei pacchetti provider, cosa che spesso risolve problemi di compatibilita con parametri dei modelli e cambiamenti delle API.
...
railwise rilevera se stai usando Wayland e preferira `wl-clipboard`; altrimenti provera a trovare gli strumenti per gli appunti nell'ordine: `xclip` e `xsel`.
