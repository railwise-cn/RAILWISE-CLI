---
title: TUI
description: Korištenje korisničkog interfejsa YONSOON (甬算) terminala.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

YONSOON (甬算) pruža interaktivni terminalski interfejs ili TUI za rad na vašim projektima sa LLM.

Pokretanje YONSOON (甬算) pokreće TUI za trenutni direktorij.
...
Kada koristite YONSOON (甬算) TUI, možete upisati `/` nakon čega slijedi ime komande da biste brzo izvršili radnje. na primjer:
...
Dodajte provajdera u YONSOON (甬算). Omogućava vam da odaberete između dostupnih provajdera i dodate njihove API ključeve.
...
Izađite iz YONSOON (甬算). _Aliases_: `/quit`, `/q`
...
Možete prilagoditi TUI ponašanje putem vašeg YONSOON (甬算) konfiguracionog fajla.
