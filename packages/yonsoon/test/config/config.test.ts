...
const managedConfigDir = process.env.YONSOON_TEST_MANAGED_CONFIG_DIR!

afterEach(async () => {
  await fs.rm(managedConfigDir, { force: true, recursive: true }).catch(() => {})
})

async function writeManagedSettings(settings: object, filename = "yonsoon.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Bun.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "yonsoon.json") {
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Bun.file(path.join(tmp.path, "yonsoon.json")).text()
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "yonsoon.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .yonsoon directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const yonsoonDir = path.join(dir, ".yonsoon")
      await fs.mkdir(yonsoonDir, { recursive: true })
      const agentDir = path.join(yonsoonDir, "agent")
...
test("loads agents from .yonsoon/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const yonsoonDir = path.join(dir, ".yonsoon")
      await fs.mkdir(yonsoonDir, { recursive: true })

      const agentsDir = path.join(yonsoonDir, "agents")
...
test("loads commands from .yonsoon/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const yonsoonDir = path.join(dir, ".yonsoon")
      await fs.mkdir(yonsoonDir, { recursive: true })

      const commandDir = path.join(yonsoonDir, "command")
...
test("loads commands from .yonsoon/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const yonsoonDir = path.join(dir, ".yonsoon")
      await fs.mkdir(yonsoonDir, { recursive: true })

      const commandsDir = path.join(yonsoonDir, "commands")
...
test("does not try to install dependencies in read-only YONSOON_CONFIG_DIR", async () => {
  ...
  const prev = process.env.YONSOON_CONFIG_DIR
  process.env.YONSOON_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.YONSOON_CONFIG_DIR
    else process.env.YONSOON_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable YONSOON_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Bun.write(
        path.join(dir, "yonsoon.json"),
        JSON.stringify({ $schema: "https://yonsoon.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "yonsoon.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .yonsoon config
      const projectDir = path.join(dir, "project")
      const yonsoonDir = path.join(projectDir, ".yonsoon")
      await fs.mkdir(yonsoonDir, { recursive: true })
      ...
      // Local .yonsoon config with different plugins
      await Bun.write(
        path.join(yonsoonDir, "yonsoon.json"),
        JSON.stringify({
          $schema: "https://yonsoon.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .yonsoon config with some overlapping plugins
      await Bun.write(
        path.join(yonsoonDir, "yonsoon.json"),
        JSON.stringify({
          $schema: "https://yonsoon.ai/config.json",
...
// Note: preload.ts sets YONSOON_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .yonsoon config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .yonsoon directory config enables it
      const yonsoonDir = path.join(dir, ".yonsoon")
      await fs.mkdir(yonsoonDir, { recursive: true })
      await Bun.write(
        path.join(yonsoonDir, "yonsoon.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  const mockFetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/yonsoon")) {
      fetchedUrl = urlStr
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(fetchedUrl).toBe("https://example.com/.well-known/yonsoon")
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("oh-my-yonsoon@2.4.3")).toBe("oh-my-yonsoon")
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@yonsoon/plugin@2.0.0")).toBe("@yonsoon/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-yonsoon@2.4.3", "file:///project/.yonsoon/plugin/oh-my-yonsoon.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.yonsoon/plugin/oh-my-yonsoon.js")
  })
  ...
  test("local plugin directory overrides global yonsoon.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const yonsoonDir = path.join(projectDir, ".yonsoon")
        const pluginDir = path.join(yonsoonDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("YONSOON_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["YONSOON_DISABLE_PROJECT_CONFIG"]
    process.env["YONSOON_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["YONSOON_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["YONSOON_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .yonsoon/ directories when flag is set", async () => {
    const originalEnv = process.env["YONSOON_DISABLE_PROJECT_CONFIG"]
    process.env["YONSOON_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .yonsoon directory with a command
          const yonsoonDir = path.join(dir, ".yonsoon", "command")
          await fs.mkdir(yonsoonDir, { recursive: true })
          await Bun.write(path.join(yonsoonDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .yonsoon should NOT be in directories list
          const hasProjectYonsoon = directories.some((d) => d.startsWith(tmp.path))
          expect(hasProjectYonsoon).toBe(false)
        },
      })
    } finally {
      if (originalEnv === undefined) {
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["YONSOON_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["YONSOON_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["YONSOON_CONFIG_DIR"]
      process.env["YONSOON_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["YONSOON_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["YONSOON_CONFIG_DIR"]
      } else {
        process.env["YONSOON_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("YONSOON_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["YONSOON_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["YONSOON_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["YONSOON_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["YONSOON_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from YONSOON_CONFIG_DIR, not project
...
describe("YONSOON_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in YONSOON_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["YONSOON_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["YONSOON_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://yonsoon.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["YONSOON_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["YONSOON_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in YONSOON_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["YONSOON_CONFIG_CONTENT"]
