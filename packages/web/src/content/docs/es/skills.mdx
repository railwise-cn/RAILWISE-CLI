...
Las habilidades del agente permiten a RAILWISE (甬算) descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
RAILWISE (甬算) busca estas ubicaciones:

- Configuración del proyecto: `.railwise/skills/<name>/SKILL.md`
- Configuración global: `~/.config/railwise/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, RAILWISE (甬算) sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.railwise/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/railwise/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.railwise/skills/git-release/SKILL.md` así:
...
RAILWISE (甬算) enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `railwise.json`:
...
**Para agentes integrados** (en `railwise.json`):
