---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u YONSOON (甬算).
---

import config from "../../../../config.mjs"
export const console = config.console

YONSOON (甬算) koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj YONSOON (甬算) konfiguraciji.
...
u `~/.local/share/yonsoon/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem YONSOON (甬算)
config.
...
YONSOON (甬算) Zen je lista modela koje je obezbedio YONSOON (甬算) tim koji su bili
testirano i potvrđeno da dobro radi sa YONSOON (甬算). [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa YONSOON (甬算) Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite yonsoon i idite na [yonsoon.ai/auth](https://yonsoon.ai/auth).
...
Radi kao i svaki drugi provajder u YONSOON (甬算) i potpuno je opcionalan za korištenje.
...
Da biste koristili Amazon Bedrock s YONSOON (甬算):
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete yonsoon:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `yonsoon.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u YONSOON (甬算).
:::
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi yonsoon ispravno radio.
   :::
...
Također možete dodati modele kroz svoju yonsoon konfiguraciju.
...
da pohrani token u yonsoon auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
YONSOON (甬算) koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate YONSOON (甬算)
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`yonsoon.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Prilagodite putem `yonsoon.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s yonsoon:
...
Za korištenje Google Vertex AI s YONSOON (甬算):
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće yonsoon.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem yonsoona, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [yonsoon-helicone-session](https://github.com/H2Shami/yonsoon-helicone-session) da automatski prijavite svaki YONSOON (甬算) razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki YONSOON (甬算) razgovor naveden kao zasebna sesija.
...
Možete konfigurirati yonsoon za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati yonsoon za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati yonsoon za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za YONSOON (甬算). Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/yonsoon) za detalje.
:::
...
Da biste koristili Ollama Cloud s YONSOON (甬算):
...
3. Kopirajte API ključ za korištenje u YONSOON (甬算).
...
6. **Važno**: Prije upotrebe modela oblaka u YONSOON (甬算), morate lokalno povući informacije o modelu:
...
YONSOON (甬算) Zen je lista testiranih i verifikovanih modela koju je obezbedio YONSOON (甬算) tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>YONSOON (甬算) Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **YONSOON (甬算) Zen**.

```txt
   /connect
```

3. Unesite svoj YONSOON (甬算) API ključ.
...
Također možete dodati dodatne modele putem vaše yonsoon konfiguracije.
...
5. Također ih možete prilagoditi putem vaše yonsoon konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Yonsoonom:
...
Također možete prilagoditi modele kroz svoju yonsoon konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s yonsoon-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `yonsoon.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju YONSOON (甬算) da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `yonsoon auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite YONSOON (甬算) konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj yonsoon konfiguraciji.
