# Remove `packages/railwise/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/railwise/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the railwise legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/railwise/src/storage/db.ts` are concentrated in 22 source files:

- `packages/railwise/src/account/repo.ts`
- `packages/railwise/src/cli/cmd/db.ts`
- `packages/railwise/src/cli/cmd/import.ts`
- `packages/railwise/src/cli/cmd/stats.ts`
- `packages/railwise/src/control-plane/workspace.ts`
- `packages/railwise/src/index.ts`
- `packages/railwise/src/node.ts`
- `packages/railwise/src/permission/index.ts`
- `packages/railwise/src/project/project.ts`
- `packages/railwise/src/server/projectors.ts`
- `packages/railwise/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/railwise/src/server/shared/fence.ts`
- `packages/railwise/src/session/message-v2.ts`
- `packages/railwise/src/session/projectors.ts`
- `packages/railwise/src/session/prompt.ts`
- `packages/railwise/src/session/session.ts`
- `packages/railwise/src/session/todo.ts`
- `packages/railwise/src/share/share-next.ts`
- `packages/railwise/src/storage/db.ts`
- `packages/railwise/src/sync/index.ts`
- `packages/railwise/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy railwise database wrapper; `packages/railwise/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the railwise wrapper.
...
Status: Completed. `SyncEvent` and the railwise projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy railwise database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy railwise database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy railwise database wrapper.
...
All migration groups are complete or superseded. `packages/railwise/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No railwise data-migration group remains.

The previous railwise `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate railwise data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/railwise`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/railwise/src`
- `bun typecheck` from `packages/railwise`
- Relevant package tests from `packages/railwise`, not the repo root
