...
Il existe deux types d'agents dans RAILWISE (甬算) ; agents primaires et sous-agents.
...
RAILWISE (甬算) est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
RAILWISE (甬算) est livré avec deux sous-agents intégrés, **General** et **Explore**. Nous verrons cela ci-dessous.

---

## Agents intégrés

RAILWISE (甬算) est livré avec deux agents primaires intégrés et deux sous-agents intégrés.
...
Si aucune température n'est spécifiée, RAILWISE (甬算) utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale RAILWISE (甬算) et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration RAILWISE (甬算) utilise le format `provider/model-id`. Par exemple, si vous utilisez [RAILWISE (甬算) Zen](/docs/zen), vous utiliserez `railwise/gpt-5.1-codex` pour GPT 5.1 Codex.
