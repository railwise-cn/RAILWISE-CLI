---
title: LSP Server
description: YONSOON (甬算) lässt sich in Ihren LSP-Server integrieren.
---

YONSOON (甬算) lässt sich in Ihr Language Server Protocol (LSP) integrieren, um die Interaktion von LLM mit Ihrer Codebasis zu unterstützen. Es verwendet Diagnosefunktionen, um dem LLM Feedback zu geben.

---

## Integriert

YONSOON (甬算) verfügt über mehrere integrierte LSP-Server für gängige Sprachen:
...
:::note
Sie können LSP-Server-Downloads automatisch deaktivieren, indem Sie die Umgebungsvariable `YONSOON_DISABLE_LSP_DOWNLOAD` auf `true` setzen.
:::
...
Wenn YONSOON (甬算) eine Datei öffnet, geschieht Folgendes:
...
Sie können LSP-Server über den Abschnitt `lsp` in Ihrer YONSOON (甬算)-Konfiguration anpassen.
