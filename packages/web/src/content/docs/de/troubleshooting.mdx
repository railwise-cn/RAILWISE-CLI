...
Wenn YONSOON (甬算) Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `yonsoon --log-level DEBUG`.

---

## Speicher

yonsoon speichert Sitzungs- und App-Daten auf der Festplatte unter:
...
YONSOON (甬算) Desktop startet im Hintergrund einen lokalen YONSOON (甬算)-Server (`yonsoon-cli`-Sidecar).
...
- Nur macOS: `YONSOON (甬算)`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
Wenn du Probleme mit YONSOON (甬算) hast:
