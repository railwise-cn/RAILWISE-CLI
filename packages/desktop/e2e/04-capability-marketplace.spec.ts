...
test("执行层可以通过统一外壳打开", async ({ launchApp }) => {
  ...
  await expect(page.locator("[data-testid=harness-timeline]")).toContainText(/还没有会话执行记录|复测资料检查/)
  await expect(page.locator("[data-testid=harness-session-detail]")).toContainText(/还没有可查看的会话|复测资料检查/)
