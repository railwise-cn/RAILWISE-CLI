...
import railwise from "./theme/railwise.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@railwise/plugin/tui"
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "railwise")
        draft.active = typeof active === "string" ? active : "railwise"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.railwise, store.mode)
    })
