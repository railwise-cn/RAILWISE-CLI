...
describe("railwise read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // RAILWISE_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ railwise }) =>
      Effect.gen(function* () {
        const r = yield* railwise.spawn(["mcp", "list"])
        railwise.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ railwise }) =>
      Effect.gen(function* () {
        const r = yield* railwise.spawn(["providers", "list"])
        railwise.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ railwise }) =>
      Effect.gen(function* () {
        const r = yield* railwise.spawn(["models"])
        railwise.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ railwise }) =>
      Effect.gen(function* () {
        const r = yield* railwise.spawn(["agent", "list"])
        railwise.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh RAILWISE_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ railwise }) =>
      Effect.gen(function* () {
        const r = yield* railwise.spawn(["session", "list"])
        railwise.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ railwise }) =>
      Effect.gen(function* () {
        const r = yield* railwise.spawn(["stats"])
        railwise.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under RAILWISE_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ railwise }) =>
      Effect.gen(function* () {
        const r = yield* railwise.spawn(["db", "path"])
        railwise.expectExit(r, 0, "db path")
