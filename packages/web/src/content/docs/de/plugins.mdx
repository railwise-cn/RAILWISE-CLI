---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um YONSOON (甬算) zu erweitern.
---

Mit Plugins können Sie YONSOON (甬算) erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von YONSOON (甬算) zu ändern.
...
- `.yonsoon/plugins/` – Plugins auf Projektebene
- `~/.config/yonsoon/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/yonsoon/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/yonsoon/yonsoon.json`)
2. Projektkonfiguration (`yonsoon.json`)
3. Globales Plugin-Verzeichnis (`~/.config/yonsoon/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.yonsoon/plugins/`)
...
YONSOON (甬算) führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein YONSOON (甬算)-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie YONSOON (甬算) erweitern können.
...
:::note
Wenn Sie die YONSOON (甬算)-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass YONSOON (甬算) `.env`-Dateien liest:
...
Plugins können YONSOON (甬算) auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das YONSOON (甬算) aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für YONSOON (甬算) zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://yonsoon.ai/docs/sdk).
