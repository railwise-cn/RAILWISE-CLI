...
Možete dodati vanjske alate u RAILWISE koristeći _Model Context Protocol_, ili MCP. RAILWISE podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [RAILWISE Config](https://railwise.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
RAILWISE automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, RAILWISE će:
...
Ako server zahtijeva autentifikaciju, RAILWISE će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `railwise mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, RAILWISE sigurno čuva tokene u `~/.local/share/railwise/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u RAILWISE, zajedno s ugrađenim alatima. Možete njima upravljati kroz RAILWISE konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete RAILWISE sa vašim Sentry nalogom.
