...
RAILWISE Zen ist eine vom RAILWISE-Team getestete und verifizierte Modellliste.

:::note
RAILWISE Zen befindet sich aktuell in der Beta.
:::

Zen funktioniert wie jeder andere Provider in RAILWISE.
Du meldest dich bei RAILWISE Zen an, holst dir deinen API-Key und nutzt ihn optional.
...
:::tip
Wir haben eine Auswahl aus Modellen und Providern getestet, die gut mit RAILWISE funktionieren.
:::
...
RAILWISE Zen ist ein AI-Gateway, das dir Zugriff auf genau diese Modelle gibt.
...
1. Melde dich bei **<a href={console}>RAILWISE Zen</a>** an, hinterlege Zahlungsdaten und kopiere deinen API-Key.
2. Fuehre in der TUI `/connect` aus, waehle RAILWISE Zen und fuege den API-Key ein.
...
Die [Model-ID](/docs/config/#models) in deiner RAILWISE-Konfiguration hat das Format `railwise/<model-id>`.
...
Wir haben RAILWISE Zen entwickelt, um:
...
4. **Kein Lock-in** zu erzwingen, damit du Zen mit anderen Coding-Agents und RAILWISE weiter mit anderen Providern nutzen kannst
