...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run railwise",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for railwise session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found railwise session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`railwise import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting railwise...")
        UI.println()

        const { spawn } = await import("child_process")
        const railwiseArgs = sessionId ? ["-s", sessionId] : []
        const railwiseProcess = spawn("railwise", railwiseArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          railwiseProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`railwise exited with code ${code}`))
          })
          railwiseProcess.on("error", reject)
        })
      },
    })
  },
