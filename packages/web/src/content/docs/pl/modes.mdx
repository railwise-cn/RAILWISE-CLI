...
:::caution
Tryby są teraz konfigurowane za pomocą opcji `agent` w konfiguracji yonsoon.
...
te lub skonfiguruj własne za pomocą konfiguracji yonsoon.
...
- `edit` — Nie można zastosować naruszenia plików, z naruszeniem praw autorskich w `.yonsoon/plans/*.md` w celu uszczegółowienia samego planu
...
Skonfiguruj tryb w pliku konfiguracyjnym `yonsoon.json`:
...
- Globalnie: `~/.config/yonsoon/modes/`
...
Jeśli nie ma wpływu na temperaturę, yonsoon stosuje się narzędzia badawcze dla modelu (zwykle 0 dla największych modeli, 0,55 dla modeli Qwen).
...
zarówno globalna yonsoon, jak i specjalna dla projektu.
...
Utwórz pliki trybów w `.yonsoon/modes/` dla trybów zapisanych dla projektu lub `~/.config/yonsoon/modes/` dla trybów globalnych:
