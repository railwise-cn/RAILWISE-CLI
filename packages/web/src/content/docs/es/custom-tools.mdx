...
description: Cree herramientas que LLM pueda llamar en railwise.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de railwise como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.railwise/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/railwise/tools/`.
...
```ts title=".railwise/tools/bash.ts"
import { tool } from "@railwise/plugin"
