...
RAILWISE Go هو اشتراك منخفض التكلفة — **$5 للشهر الأول**، ثم **$10/شهريًا** — يمنحك وصولًا موثوقًا إلى نماذج البرمجة المفتوحة الشائعة.

يعمل Go مثل أي مزود آخر في RAILWISE. تشترك في RAILWISE Go وتحصل على مفتاح API الخاص بك. وهو **اختياري تمامًا**، ولا تحتاج إلى استخدامه لاستخدام RAILWISE.
...
:::tip
اختبرنا مجموعة مختارة من النماذج والمزودين الذين يعملون جيدًا مع RAILWISE.
:::
...
يمنحك RAILWISE Go الوصول إلى هذه النماذج مقابل **$5 للشهر الأول**، ثم **$10/شهريًا**.

---

## كيف يعمل

يعمل RAILWISE Go مثل أي مزود آخر في RAILWISE.

1. تسجّل الدخول إلى **<a href={console}>RAILWISE Zen</a>**، وتشترك في Go، ثم تنسخ مفتاح API الخاص بك.
2. تشغّل الأمر `/connect` في TUI، وتختار `RAILWISE Go`، ثم تلصق مفتاح API الخاص بك.
3. شغّل `/models` في TUI لرؤية قائمة النماذج المتاحة عبر Go.

:::note
يمكن لعضو واحد فقط في كل workspace الاشتراك في RAILWISE Go.
:::
...
يتضمن RAILWISE Go الحدود التالية:

- **حد 5 ساعات** — استخدام بقيمة $12
- **الحد الأسبوعي** — استخدام بقيمة $30
- **الحد الشهري** — استخدام بقيمة $60

تُحدَّد الحدود بالقيمة بالدولار. وهذا يعني أن عدد طلباتك الفعلي يعتمد على النموذج الذي تستخدمه. تتيح النماذج الأقل تكلفة مثل DeepSeek V4 Flash عددًا أكبر من الطلبات، بينما تتيح النماذج الأعلى تكلفة مثل GLM-5.1 عددًا أقل.
...
- MiniMax M3 — ‏810 input، و62,000 cached، و225 output tokens لكل طلب
...
يستخدم [model id](/docs/config/#models) في إعدادات RAILWISE لديك التنسيق `railwise-go/<model-id>`. على سبيل المثال، بالنسبة إلى Kimi K2.6، ستستخدم `railwise-go/kimi-k2.6` في إعداداتك.
...
4. **عدم فرض قيود ارتباط** من خلال السماح لك أيضًا باستخدام أي مزود آخر مع RAILWISE.
