...
Opprett `.railwise/commands/test.md`:
...
Du kan legge til egendefinerte kommandoer gjennom RAILWISE (甬算)-konfigurasjonen eller ved å lage markdown-filer i katalogen `commands/`.

---

### JSON

Bruk alternativet `command` i RAILWISE (甬算) [config](/docs/config):
...
- Per prosjekt: `.railwise/commands/`
...
RAILWISE (甬算) inkluderer flere innebygde kommandoer som `/init`, `/undo`, `/redo`, `/share`, `/help`; [finn ut mer](/docs/tui#commands).
