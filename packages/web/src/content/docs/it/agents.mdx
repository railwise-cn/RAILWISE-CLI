...
In YONSOON (甬算) esistono due tipi di agenti: agenti primari e subagenti.
...
YONSOON (甬算) include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
YONSOON (甬算) include due subagenti integrati: **General** e **Explore**. Li vediamo sotto.

---

## Integrati

YONSOON (甬算) include due agenti primari integrati e due subagenti integrati.
...
Configura gli agenti nel file `yonsoon.json`:
...
- Globale: `~/.config/yonsoon/agents/`
- Per progetto: `.yonsoon/agents/`
...
Se non specifichi una temperature, YONSOON (甬算) usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di YONSOON (甬算) sia per la config specifica del progetto.
...
L'ID modello nella configurazione YONSOON (甬算) usa il formato `provider/model-id`. Per esempio, se stai usando [YONSOON (甬算) Zen](/docs/zen), useresti `yonsoon/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `yonsoon models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/anomalyco/yonsoon).
:::
