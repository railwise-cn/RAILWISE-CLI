---
title: Complementos
description: Escriba sus propios complementos para extender RAILWISE (甬算).
---

Los complementos le permiten extender RAILWISE (甬算) al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de RAILWISE (甬算).
...
- `.railwise/plugins/` - Complementos a nivel de proyecto
- `~/.config/railwise/plugins/` - Complementos globales
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/railwise/node_modules/`.
...
1. Configuración global (`~/.config/railwise/railwise.json`)
2. Configuración del proyecto (`railwise.json`)
3. Directorio global de complementos (`~/.config/railwise/plugins/`)
4. Directorio de complementos del proyecto (`.railwise/plugins/`)
...
RAILWISE (甬算) ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.
...
- `client`: Un cliente SDK railwise para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar railwise.
...
:::note
Si está utilizando la aplicación de escritorio RAILWISE (甬算), puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que railwise lea archivos `.env`:
...
Los complementos también pueden agregar herramientas personalizadas a railwise:
...
El ayudante `tool` crea una herramienta personalizada a la que railwise puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para railwise junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://railwise.ai/docs/sdk) para obtener más detalles.
