...
test("resolves env templates in account config with account token", async () => {
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { opencode: { options: { apiKey: "{env:RAILWISE_CONSOLE_TOKEN}" } } },
        }),
      ),
...
test("loads config from .railwise directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".railwise")
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const opencodeDir = path.join(projectDir, ".railwise")
      await fs.mkdir(opencodeDir, { recursive: true })
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-opencode@2.4.3", "file:///project/.railwise/plugin/oh-my-opencode.js"]
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          ...
          const opencodeDir = path.join(dir, ".railwise", "command")
          await fs.mkdir(opencodeDir, { recursive: true })
