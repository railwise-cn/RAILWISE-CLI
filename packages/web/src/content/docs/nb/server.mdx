---
title: Tjener
description: Samhandle med RAILWISE (甬算)-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `railwise serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en RAILWISE (甬算)-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `railwise.local` |
...
Sett `RAILWISE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `railwise` som standard, eller sett `RAILWISE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `railwise serve` og `railwise web`.
...
Når du kjører `railwise` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk RAILWISE (甬算)-serveren til å samhandle med RAILWISE (甬算) programmatisk.
:::

Denne arkitekturen lar RAILWISE (甬算) støtte flere klienter og lar deg samhandle med RAILWISE (甬算) programmatisk.

Du kan kjøre `railwise serve` for å starte en frittstående server. Hvis du har
RAILWISE (甬算) TUI kjører, vil `railwise serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av RAILWISE (甬算) [IDE](/docs/ide) plugins.
...
railwise-serveren viser følgende APIer.
