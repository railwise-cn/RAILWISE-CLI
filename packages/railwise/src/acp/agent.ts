...
import type { AssistantMessage, Event, OpencodeClient, SessionMessageResponse, ToolPart } from "@railwise/sdk/v2"
...
async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
  ...
  if (railwiseProvider) {
    if (railwiseProvider.models["big-pickle"]) {
      return { providerID: ProviderID.railwise, modelID: ModelID.make("big-pickle") }
    }
