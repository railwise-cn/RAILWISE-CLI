...
YONSOON (甬算) supporta le variabili d'ambiente standard per i proxy e certificati personalizzati per ambienti di rete enterprise.

---

## Proxy

YONSOON (甬算) rispetta le variabili d'ambiente standard dei proxy.
...
Se la tua organizzazione usa CA personalizzate per connessioni HTTPS, configura YONSOON (甬算) in modo che si fidi di esse.
