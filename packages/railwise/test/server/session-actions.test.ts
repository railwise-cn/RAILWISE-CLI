  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const headers = { "Content-Type": "application/json", "x-railwise-directory": test.directory }
        ...
        const fetched = yield* Effect.promise(() =>
          Promise.resolve(
            app.request(`/session/${session.id}`, { headers: { "x-railwise-directory": test.directory } }),
          ),
        )
