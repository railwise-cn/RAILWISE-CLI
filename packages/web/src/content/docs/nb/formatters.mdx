---
title: Formattere
description: RAILWISE (甬算) bruker språkspesifikke formattere.
---

RAILWISE (甬算) formaterer automatisk filer etter at de er skrevet eller redigert ved hjelp av språkspesifikke formattere. Dette sikrer at koden som genereres følger kodestilene til prosjektet ditt.

---

## Innebygde formattere

RAILWISE (甬算) kommer med flere innebygde formattere for populære språk og rammeverk. Nedenfor er en liste over formattere, støttede filendelser og kommandoer eller konfigurasjonsalternativer den krever.
...
Så hvis prosjektet ditt har `prettier` i `package.json`, vil RAILWISE (甬算) automatisk bruke det.

---

## Slik fungerer det

Når RAILWISE (甬算) skriver eller redigerer en fil, gjør den:
...
Du kan tilpasse formattere gjennom `formatter`-delen i RAILWISE (甬算)-konfigurasjonen.
