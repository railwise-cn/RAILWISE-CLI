...
Com o yonsoon, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o yonsoon usa nosso próprio tema `yonsoon`.
...
O yonsoon vem com vários temas embutidos.
...
- Querem que o yonsoon corresponda à aparência do seu terminal
...
O yonsoon suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/yonsoon/themes/*.json` ou `$XDG_CONFIG_HOME/yonsoon/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.yonsoon/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.yonsoon/themes/*.json`
