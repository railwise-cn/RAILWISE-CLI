...
Move the canonical RAILWISE terminal application from
`packages/railwise/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/railwise`,
`packages/cli`, or `@railwise/core`.

The SDK is the TUI's RAILWISE boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/railwise`.
- Do not replace private `packages/railwise` imports with `@railwise/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@railwise/tui`.
...
- RAILWISE domain data displayed by the TUI
...
Its private dependency on `packages/railwise` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/railwise/src/*`.
...
Status: Completed. The private `@railwise/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@railwise/tui`.
...
- The package has no dependency on `railwise`, `@railwise/cli`, or
  `@railwise/core`.
...
primitives now live in `@railwise/tui`. The legacy host consumes explicit
...
and build inputs through `@railwise/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads RAILWISE globals or process state;
...
Replace process-global RAILWISE state with resolved TUI inputs.
...
`@railwise/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/railwise` initially.
...
canonical in `@railwise/tui`. Configured references resolve through the new
...
adapters rather than RAILWISE domain implementations.

Make the SDK the only RAILWISE domain boundary used by the TUI.
...
- Domain-facing TUI code imports RAILWISE data and operations only from
  `@railwise/sdk`.
...
`@railwise/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/railwise`.
...
- The package has no imports from `packages/railwise`, `packages/cli`, or
  `@railwise/core`.
...
public `@railwise/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/railwise`.
- Keep the legacy embedded worker and server startup in `packages/railwise`.
- Change those adapters to load config, create transport inputs, and call the
  public `@railwise/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/railwise`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@railwise/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@railwise/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for RAILWISE domain state.
...
cd packages/railwise && bun typecheck
...
rg '@railwise/core|packages/railwise|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/railwise/src packages/railwise/test
```
...
- Build the current-platform `packages/railwise` binary.
