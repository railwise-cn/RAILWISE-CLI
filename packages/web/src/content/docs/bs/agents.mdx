...
Postoje dvije vrste agenata u RAILWISE (甬算): primarni agenti i podagenti.
...
RAILWISE (甬算) dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
RAILWISE (甬算) dolazi sa dva ugrađena subagenta, **General** i **Explore**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

RAILWISE (甬算) dolazi sa dva ugrađena primarna agenta i dva ugrađena subagenta.
...
Ako temperatura nije navedena, RAILWISE (甬算) koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu RAILWISE (甬算) konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj RAILWISE (甬算) konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [RAILWISE (甬算) Zen](/docs/zen), koristili biste `railwise/gpt-5.1-codex` za GPT 5.1 Codex.
