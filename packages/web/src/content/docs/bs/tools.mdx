...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. RAILWISE (甬算) dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u RAILWISE (甬算).
...
:::note
Ovaj alat je dostupan samo kada je `RAILWISE_EXPERIMENTAL_LSP_TOOL=true` (ili `RAILWISE_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz RAILWISE (甬算) provajdera ili kada je varijabla `RAILWISE_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju RAILWISE (甬算):
