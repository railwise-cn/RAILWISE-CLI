...
ove ili konfigurirajte svoje putem railwise config.
...
railwise dolazi sa dva ugrađena načina rada.
...
- `edit` - Ne mogu modificirati postojeće fajlove, osim fajlova koji se nalaze na `.railwise/plans/*.md` radi detaljiziranja samog plana
...
Konfigurirajte načine rada u svom `railwise.json` konfiguracijskom fajlu:
...
- Globalno: `~/.config/railwise/modes/`
- Projekat: `.railwise/modes/`
...
Ako temperatura nije navedena, railwise koristi podrazumijevane postavke specifične za model (obično 0 za većinu modela i 0.55 za Qwen modele).
...
Kreirajte fajlove načina u `.railwise/modes/` za specifične načine rada ili `~/.config/railwise/modes/` za globalne načine:
