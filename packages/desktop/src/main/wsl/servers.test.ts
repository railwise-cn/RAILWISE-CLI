...
test("rejects an update that did not install the desktop version", () => {
  ...
  expect(() => expectOpencodeVersion("1.14.35", "1.16.2")).toThrow(
    "RAILWISE update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating RAILWISE", () => {
  expect(
...
test("clears cached distro probes when removing a WSL server", () => {
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.railwise/bin/railwise",
          version: "1.16.2",
  ...
  ).toEqual({ distroProbes: {}, railwiseChecks: {} })
})
...
test("ignores stale background RAILWISE checks after removing a WSL server", async () => {
  persistedServers = []
  ...
  expect(controller.getState().railwiseChecks).toEqual({})
})

test("ignores stale startup RAILWISE checks after removing a WSL server", async () => {
...
function testControllerOptions() {
  return {
    ...
    resolveOpencode: async () => {
      ...
      return "/home/me/.railwise/bin/railwise"
    },
  }
}
