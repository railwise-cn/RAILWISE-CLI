...
As habilidades do agente permitem que o yonsoon descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O yonsoon pesquisa nesses locais:

- Configuração do projeto: `.yonsoon/skills/<name>/SKILL.md`
- Configuração global: `~/.config/yonsoon/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o yonsoon sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.yonsoon/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/yonsoon/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.yonsoon/skills/git-release/SKILL.md` assim:
...
O yonsoon lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `yonsoon.json`:
...
**Para agentes embutidos** (em `yonsoon.json`):
