---
title: LSP-servere
description: railwise integreres med LSP-serverne dine.
---

RAILWISE (甬算) integreres med din Language Server Protocol (LSP) for å hjelpe LLM med å samhandle med kodebasen din. Den bruker diagnostikk for å gi tilbakemelding til LLM.

---

## Innebygde servere

RAILWISE (甬算) kommer med flere innebygde LSP-servere for populære språk:
...
:::note
Du kan deaktivere automatiske LSP-servernedlastinger ved å sette miljøvariabelen `RAILWISE_DISABLE_LSP_DOWNLOAD` til `true`.
:::
...
Når railwise åpner en fil, gjør den:
...
Du kan tilpasse LSP-servere gjennom `lsp`-delen i RAILWISE (甬算)-konfigurasjonen.
