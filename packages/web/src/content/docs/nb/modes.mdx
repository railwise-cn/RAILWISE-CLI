...
:::caution
Moduser konfigureres nå gjennom alternativet `agent` i RAILWISE-konfigurasjonen.
Alternativet `mode` er nå utdatert. [Finn ut mer](/docs/agents).
:::

Moduser i RAILWISE lar deg tilpasse oppførselen, verktøyene og prompter for ulike brukstilfeller.

Den kommer med to innebygde moduser: **bygg** og **plan**. Du kan tilpasse
disse eller konfigurer din egen gjennom RAILWISE-konfigurasjonen.
...
RAILWISE kommer med to innebygde moduser.
...
- `edit` - Kan ikke endre eksisterende filer, bortsett fra filer som ligger på `.railwise/plans/*.md` for å detaljere selve planen
...
Konfigurer moduser i din `railwise.json` konfigurasjonsfil:
...
- Globalt: `~/.config/railwise/modes/`
- Prosjekt: `.railwise/modes/`
...
Hvis ingen temperatur er spesifisert, bruker RAILWISE modellspesifikke standardinnstillinger (vanligvis 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
Denne banen er relativ til der konfigurasjonsfilen er plassert. Så dette fungerer for både den globale RAILWISE-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Opprett modusfiler i `.railwise/modes/` for prosjektspesifikke moduser eller `~/.config/railwise/modes/` for globale moduser:
