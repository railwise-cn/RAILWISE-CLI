---
title: 공급자
description: RAILWISE (甬算)에서 LLM 공급자를 사용합니다.
---

import config from "../../../../config.mjs"
export const console = config.console

RAILWISE (甬算)는 [AI SDK](https://ai-sdk.dev/) 및 [Models.dev](https://models.dev)를 사용하여 **75개 이상의 LLM 공급자**를 지원하며 로컬 모델도 실행할 수 있습니다.

필요한 공급자를 추가하려면:

1. `/connect` 명령을 사용하여 공급자의 API 키를 추가하십시오.
2. RAILWISE (甬算) 구성에서 공급자를 구성하십시오.
...
RAILWISE (甬算) 설정의 `provider` 섹션을 통해 공급자를 사용자 정의할 수 있습니다.
...
RAILWISE (甬算) Zen은 RAILWISE (甬算) 팀이 RAILWISE (甬算)와 잘 작동하도록 테스트하고 검증한 모델 목록입니다. [더 알아보기](/docs/zen).

:::tip
처음이라면 RAILWISE (甬算) Zen으로 시작하는 것이 좋습니다.
:::
...
RAILWISE (甬算)의 다른 공급자처럼 작동하며 사용은 완전히 선택 사항입니다.
...
RAILWISE (甬算)로 Amazon Bedrock을 사용하려면:
...
RAILWISE (甬算)를 실행하는 동안 다음 환경 변수 중 하나를 설정합니다:
...
:::info
RAILWISE (甬算)에서 Claude Pro/Max 구독을 사용하는 것은 [Anthropic](https://anthropic.com)에서 공식적으로 지원하지 않습니다.
:::
...
2. [Azure AI Foundry](https://ai.azure.com/)로 이동하여 모델을 배포합니다.
   ...
   :::note
   배포 이름은 제대로 작동하려면 RAILWISE (甬算)의 모델 이름과 일치해야 합니다.
   :::
...
5. 모델을 선택하려면 `/models` 명령을 실행하십시오.
   ...
   RAILWISE (甬算) 구성을 통해 모델을 추가할 수 있습니다.
...
RAILWISE (甬算)는 인증 저장소에 토큰을 저장합니다.
:::

#### 셀프 호스팅 GitLab (Self-Hosted)

:::note[규정 준수 참고 사항]
RAILWISE (甬算)는 세션 제목 생성과 같은 일부 AI 작업을 위해 작은 모델을 사용합니다.
기본적으로 Zen에서 호스팅되는 gpt-5-nano를 사용하도록 구성됩니다.
RAILWISE (甬算)를 자체 호스팅 GitLab 인스턴스만 사용하도록 제한하려면 `railwise.json` 파일에 다음을 추가하십시오.
...
RAILWISE (甬算) Zen은 railwise 팀에서 제공하는 테스트 및 검증된 모델 목록입니다. [더 알아보기](/docs/zen).

1. 로그인 **<a href={console}>RAILWISE (甬算) Zen</a>** and click**Create API Key**.

2. `/connect` 명령을 실행하고 **RAILWISE (甬算) Zen**를 검색하십시오.
