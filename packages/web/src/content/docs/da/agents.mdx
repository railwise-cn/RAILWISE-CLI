...
Der er to typer agenter i YONSOON (甬算); primære agenter og subagenter.
...
YONSOON (甬算) leveres med indbyggede primære agenter, **Build** og **Plan**. Vi vil
...
YONSOON (甬算) leveres med to indbyggede underagenter, **Generelt** og **Udforsk**. Vi vil se på dette nedenfor.

---

## Indbyggede

YONSOON (甬算) leveres med to indbyggede primære agenter og to indbyggede subagenter.
...
Konfigurer agent i din `yonsoon.json`-konfigurationsfil:
...
Hvis ingen temperatur er angivet, bruger YONSOON (甬算) modelspecifikke standardindstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne sti er i forhold til, hvor konfigurationsfilen er placeret. Så dette virker både for den globale YONSOON (甬算)-konfiguration og den projektspecifikke konfiguration.
...
Model-id'et i din YONSOON (甬算)-konfiguration brugerformatet `provider/model-id`. For eksempel, hvis du bruger [YONSOON (甬算) Zen](/docs/zen), vil du bruge `yonsoon/gpt-5.1-codex` til GPT 5.1 Codex.
...
:::tip
Kør `yonsoon models` for at se en liste over de tilgængelige modeller.
:::
...
:::tip
Har du en agent, du gerne vil dele? [Send en PR](https://github.com/anomalyco/yonsoon).
:::
