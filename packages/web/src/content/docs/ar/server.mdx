---
title: الخادم
description: تفاعل مع خادم railwise عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `railwise serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل railwise استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `railwise.local` |
...
عيّن `RAILWISE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `railwise`، أو عيّن `RAILWISE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `railwise serve` و `railwise web`.
...
عند تشغيل `railwise` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم railwise للتفاعل مع railwise برمجيا.
:::

تتيح هذه البنية لـ railwise دعم عدة عملاء وتمكّنك من التفاعل مع railwise برمجيا.

يمكنك تشغيل `railwise serve` لبدء خادم مستقل. إذا كانت واجهة railwise في terminal (TUI)
قيد التشغيل، فسيبدأ `railwise serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات RAILWISE (甬算) لـ [IDE](/docs/ide).
...
يعرض خادم railwise واجهات API التالية.
