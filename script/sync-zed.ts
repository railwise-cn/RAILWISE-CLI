...
async function main() {
  ...
  // Delete any existing branches for yonsoon updates
  ...
  const prResult =
    await $`gh pr create --repo ${UPSTREAM_REPO} --base main --head ${FORK_REPO.split("/")[0]}:${branchName} --title "Update ${EXTENSION_NAME} to v${cleanVersion}" --body "Updating YONSOON (甬算) extension to v${cleanVersion}"`
