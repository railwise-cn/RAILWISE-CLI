---
title: Eklentiler
description: RAILWISE'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek RAILWISE'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya RAILWISE'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.railwise/plugins/` - Proje düzeyinde eklentiler
- `~/.config/railwise/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/railwise/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/railwise/railwise.json`)
2. Project config (`railwise.json`)
3. Global eklenti dizini (`~/.config/railwise/plugins/`)
4. Proje eklenti dizini (`.railwise/plugins/`)
...
RAILWISE bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir RAILWISE SDK istemcisi.
...
RAILWISE'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
RAILWISE masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
RAILWISE'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca RAILWISE'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, RAILWISE'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra RAILWISE'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://railwise.ai/docs/sdk) bakın.
