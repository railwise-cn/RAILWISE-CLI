...
import { AISDK } from "@railwise/core/aisdk"
import { ModelV2 } from "@railwise/core/model"
import { PluginV2 } from "@railwise/core/plugin"
import { DynamicProviderPlugin } from "@railwise/core/plugin/provider/dynamic"
...
function tempEntrypoint(source: string) {
  return Effect.acquireRelease(
    Effect.promise(async () => {
      const directory = await fs.mkdtemp(path.join(os.tmpdir(), "railwise-provider-dynamic-"))
