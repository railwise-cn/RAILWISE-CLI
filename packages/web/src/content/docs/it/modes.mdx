...
:::caution
Le modalita ora si configurano tramite l'opzione `agent` nella configurazione di railwise. L'opzione
`mode` e ora deprecata. [Scopri di piu](/docs/agents).
:::

Le modalita in railwise ti permettono di personalizzare comportamento, strumenti e prompt per casi d'uso diversi.

Include due modalita integrate: **build** e **plan**. Puoi personalizzarle oppure configurarne di tue tramite la configurazione di railwise.
...
railwise include due modalita integrate.
...
- `edit` - Non puo modificare file esistenti, tranne i file in `.railwise/plans/*.md` per dettagliare il piano
...
Configura le modalita nel file di configurazione `railwise.json`:
...
- Globale: `~/.config/railwise/modes/`
- Progetto: `.railwise/modes/`
...
Se non viene specificata alcuna temperatura, railwise usa i default specifici del modello (tipicamente 0 per la maggior parte dei modelli, 0.55 per i modelli Qwen).
...
sia per la configurazione globale di railwise sia per quella specifica del progetto.
...
Crea file di modalita in `.railwise/modes/` per modalita specifiche del progetto oppure in `~/.config/railwise/modes/` per modalita globali:
