...
Możesz dodać dodatkowe informacje do yonsoon za pomocą _Model Context Protocol_, w skrócie MCP. yonsoon obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [yonsoon Config](https://yonsoon.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/yonsoon`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
yonsoon automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, yonsoon:
...
Jeśli serwer wymaga uwierzytelnienia, yonsoon poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `yonsoon mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji yonsoon bezpieczne przechowa tokeny w `~/.local/share/yonsoon/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w yonsoon, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie yonsoon, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć yonsoon z kontem Sentry.
