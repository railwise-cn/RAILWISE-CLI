...
Both `action` and `resource` use railwise's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.railwise` files is intentionally deferred until `.railwise` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether railwise permits an operation through its managed execution paths.
