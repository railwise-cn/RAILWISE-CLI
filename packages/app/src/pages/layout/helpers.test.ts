...
describe("layout deep links", () => {
  ...
  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("railwise://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("railwise://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("railwise://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("railwise://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "railwise://new-session?directory=/a",
      "railwise://open-project?directory=/b",
      "railwise://new-session?directory=/c&prompt=ship%20it",
    ])
