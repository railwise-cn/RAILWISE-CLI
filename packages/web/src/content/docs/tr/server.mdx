---
title: Sunucu
description: railwise sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`railwise serve` komutu, railwise istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `railwise.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `RAILWISE_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `railwise` değeridir; değiştirmek isterseniz `RAILWISE_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `railwise serve` hem de `railwise web` için geçerlidir.
...
`railwise` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
railwise ile programatik etkileşim için railwise sunucusunu kullanın.
:::

Bu mimari, railwise'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`railwise serve` ile bağımsız bir sunucu başlatabilirsiniz. railwise TUI açıksa, `railwise serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum railwise [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
railwise sunucusu aşağıdaki API'leri sunar.
