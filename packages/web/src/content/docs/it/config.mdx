...
Metti la configurazione globale di RAILWISE in `~/.config/railwise/railwise.json`. Usa la configurazione globale per preferenze valide per l'utente (ad es. temi, provider o keybind).
...
Puoi configurare il tema da usare in RAILWISE tramite l'opzione `theme`.
