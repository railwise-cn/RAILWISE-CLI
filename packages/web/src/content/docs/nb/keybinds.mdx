...
RAILWISE (甬算) har en liste over tastebindinger som du kan tilpasse gjennom RAILWISE (甬算)-konfigurasjonen.
...
RAILWISE (甬算) bruker en `leader`-tast for de fleste tastebindinger. Dette unngår konflikter i terminalen din.
...
Prompt-feltet for RAILWISE (甬算)-skrivebordsappen støtter vanlige snarveier i Readline/Emacs-stil for redigering av tekst. Disse er innebygd og foreløpig ikke konfigurerbare via `railwise.json`.
