---
title: Konfiguracja
description: Ustawienia RAILWISE (甬算) w pliku JSON.
---

Możesz dostosować RAILWISE (甬算) za pomocą pliku konfiguracyjnego JSON.

---

## Format

RAILWISE (甬算) obsługuje formaty **JSON** i **JSONC** (JSON z komentarzami).
...
Umieść swoją globalną konfigurację RAILWISE (甬算) w `~/.config/railwise/railwise.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak motywy, domyślny dostawca lub skróty klawiszowe.
...
Kiedy RAILWISE (甬算) się uruchamia, szuka pliku konfiguracyjnego w katalogu głównym repozytorium Git.
...
- `mdns` - Włącz wykrywanie usług mDNS. Dzięki temu inne urządzenia w sieci mogą łatwo znaleźć Twój serwer RAILWISE (甬算).
...
Opcja `small_model` konfiguruje oddzielny model do lżejszych zadań, takich jak generowanie tytułów. Domyślnie RAILWISE (甬算) wybiera tańszy model od tego samego dostawcy, jeśli jest dostępny, w przeciwnym razie powraca do modelu głównego.
...
Skonfiguruj motyw interfejsu RAILWISE (甬算) za pomocą opcji `theme`.
...
Agent domyślny musi być agentem typu `primary` (nie `subagent`). Może to być agent wbudowany, taki jak `"build"` lub `"plan"`, albo zdefiniowany przez Ciebie [agent niestandardowy](/docs/agents). Jeśli agent nie istnieje lub jest subagentem, RAILWISE (甬算) powróci do `"build"` z ostrzeżeniem.
...
RAILWISE (甬算) może automatycznie pobierać nowe wersje. Możesz to kontrolować za pomocą opcji `autoupdate`.
...
Działa to tylko wtedy, gdy RAILWISE (甬算) nie został zainstalowany przez menedżera pakietów, takiego jak Homebrew.
...
Domyślnie RAILWISE (甬算) **zezwala na wszystkie działania** bez konieczności zatwierdzania. Możesz to zmienić za pomocą opcji `permission`.
...
[Wtyczki](/docs/plugins) rozszerzają RAILWISE (甬算) o niestandardowe narzędzia, hooki i integracje.
...
Jest to przydatne, gdy chcesz ograniczyć użycie RAILWISE (甬算) tylko do określonych dostawców.
