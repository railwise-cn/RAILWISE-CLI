...
أنشئ الملف `.yonsoon/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات YONSOON (甬算) أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ YONSOON (甬算):
...
- على مستوى النظام: `~/.config/yonsoon/commands/`
- لكل مشروع: `.yonsoon/commands/`
...
يتضمن yonsoon عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
