---
title: المزوّدون
description: استخدام أي موفّر LLM في YONSOON (甬算).
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم YONSOON (甬算) ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات YONSOON (甬算).
...
في `~/.local/share/yonsoon/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات YONSOON (甬算).
...
YONSOON (甬算) Zen هي قائمة نماذج يوفّرها فريق YONSOON (甬算) وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع YONSOON (甬算). [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع YONSOON (甬算) Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر yonsoon، ثم انتقل إلى [yonsoon.ai/auth](https://yonsoon.ai/auth).
...
يعمل مثل أي مزوّد آخر في YONSOON (甬算) واستخدامه اختياري بالكامل.
...
لاستخدام Amazon Bedrock مع YONSOON (甬算):
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل yonsoon:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `yonsoon.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل YONSOON (甬算) غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل yonsoon بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات yonsoon.
...
تخزين الرمز في مساحة تخزين مصادقة yonsoon.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم YONSOON (甬算) نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر YONSOON (甬算)
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`yonsoon.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
لاستخدام اشتراك GitHub Copilot مع yonsoon:
...
لاستخدام Google Vertex AI مع YONSOON (甬算):
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر yonsoon، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [yonsoon-helicone-session](https://github.com/H2Shami/yonsoon-helicone-session) لتسجيل كل محادثة YONSOON (甬算) تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة YONSOON (甬算) مدرجة كجلسة مستقلة.
...
يمكنك ضبط yonsoon لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط yonsoon لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط yonsoon لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع YONSOON (甬算). راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/yonsoon) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع YONSOON (甬算):
...
3. انسخ مفتاح API لاستخدامه في YONSOON (甬算).
...
6. **مهم**: قبل استخدام نماذج السحابة في YONSOON (甬算)، يجب سحب معلومات النموذج محليا:
...
YONSOON (甬算) Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق YONSOON (甬算). [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>YONSOON (甬算) Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **YONSOON (甬算) Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ YONSOON (甬算).
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات yonsoon.
...
5. يمكنك أيضا تخصيصها عبر إعدادات yonsoon. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Yonsoon:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات yonsoon. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع yonsoon. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `yonsoon.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ YONSOON (甬算) بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `yonsoon auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات yonsoon وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات yonsoon.
