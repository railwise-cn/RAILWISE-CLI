---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di YONSOON (甬算).
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

YONSOON (甬算) fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire YONSOON (甬算) avvia la TUI per la directory corrente.
...
Quando usi la TUI di YONSOON (甬算), puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a YONSOON (甬算). Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da YONSOON (甬算). _Alias_: `/quit`, `/q`
...
Puoi personalizzare il comportamento della TUI tramite il file di config di YONSOON (甬算).
