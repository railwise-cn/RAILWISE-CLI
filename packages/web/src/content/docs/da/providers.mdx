---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i RAILWISE (甬算).
---

import config from "../../../../config.mjs"
export const console = config.console

RAILWISE (甬算) bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i RAILWISE (甬算)-konfigurationen.
...
Du kan tilpasse udbyderne gennem `provider`-delen i RAILWISE (甬算)-konfigurationen.
...
RAILWISE (甬算) Zen er en liste over modeller leveret af RAILWISE (甬算)-teamet, som er blevet testet og verificeret til at fungere godt med RAILWISE (甬算). [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med RAILWISE (甬算) Zen.
:::
...
Det fungerer som alle andre udbydere i RAILWISE (甬算) og er helt valgfrit at bruge.
...
Sådan bruger du Amazon Bedrock med RAILWISE (甬算):
...
:::info
At bruge dit Claude Pro/Max-abonnement i RAILWISE (甬算) understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
RAILWISE (甬算) bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse RAILWISE (甬算)
...
Sådan bruger du GitHub Copilot-abonnementet med RAILWISE (甬算):
...
Sådan bruger du Google Vertex AI med RAILWISE (甬算):
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [railwise-helicone-session](https://github.com/H2Shami/railwise-helicone-session) plugin for automatisk at logge hver RAILWISE (甬算)-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver RAILWISE (甬算)-samtale opført som en separat session.
...
:::tip
Ollama kan automatisk konfigurere sig selv for RAILWISE (甬算). Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/railwise) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med RAILWISE (甬算):
...
3. Kopier API-nøglen til brug i RAILWISE (甬算).
...
6. **Vigtigt**: Før du bruger skymodeller i RAILWISE (甬算), skal du hente modelinformationen lokalt:
...
RAILWISE (甬算) Zen er en liste over testede og verificerede modeller leveret af RAILWISE (甬算)-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>RAILWISE (甬算) Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **RAILWISE (甬算) Zen**.

   ```txt
   /connect
   ```

3. Indtast RAILWISE (甬算) API-nøglen.
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med RAILWISE (甬算):
...
`limit`-felterne lader RAILWISE (甬算) forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
