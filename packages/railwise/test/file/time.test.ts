...
describe("file/time", () => {
  ...
  describe("assert()", () => {
    ...
    test("skips check when RAILWISE_DISABLE_FILETIME_CHECK is true", async () => {
      await using tmp = await tmpdir()
      const filepath = path.join(tmp.path, "file.txt")
      await fs.writeFile(filepath, "content", "utf-8")
      ...
      await Instance.provide({
        ...
        fn: async () => {
          ...
          const original = Flag.RAILWISE_DISABLE_FILETIME_CHECK
          ;(Flag as { RAILWISE_DISABLE_FILETIME_CHECK: boolean }).RAILWISE_DISABLE_FILETIME_CHECK = true

          try {
            // Should not throw even though file wasn't read
            await FileTime.assert(sessionID, filepath)
          } finally {
            ;(Flag as { RAILWISE_DISABLE_FILETIME_CHECK: boolean }).RAILWISE_DISABLE_FILETIME_CHECK = original
          }
        },
      })
    })
