...
Создайте `.railwise/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию railwise или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем railwise [config](/docs/config):
...
- Глобальный: `~/.config/railwise/commands/`
- Для каждого проекта: `.railwise/commands/`
...
railwise включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
