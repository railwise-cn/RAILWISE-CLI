---
title: Formatery
description: RAILWISE (甬算) używa formaterów specyficznych dla języka.
---

RAILWISE (甬算) automatycznie formatuje pliki po ich zapisaniu lub edycji przy użyciu formaterów specyficznych dla języka. Zapewnia to, że wygenerowany kod jest zgodny ze stylem kodu Twojego projektu.

---

## Wbudowane

RAILWISE (甬算) zawiera kilka wbudowanych formaterów dla popularnych języków i frameworków. Poniższa tabela zawiera listę formaterów, obsługiwanych plików oraz wymagań konfiguracyjnych.
...
Jeśli więc Twój projekt zawiera `prettier` w `package.json`, RAILWISE (甬算) automatycznie go użyje.

---

## Jak to działa

Kiedy RAILWISE (甬算) zapisuje lub edytuje plik:
...
Możesz dostosować formatery za pomocą sekcji `formatter` w konfiguracji RAILWISE (甬算).
