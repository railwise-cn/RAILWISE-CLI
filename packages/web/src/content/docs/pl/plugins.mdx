---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć yonsoon.
---

Wtyczki umożliwiające wyłączenie yonsoon poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania yonsoon.
...
- `.yonsoon/plugins/` - Wtyczki na poziomie projektu
- `~/.config/yonsoon/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/yonsoon/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/yonsoon/yonsoon.json`)
2. Konfiguracja projektu (`yonsoon.json`)
3. Globalny katalog wtyczek (`~/.config/yonsoon/plugins/`)
4. Katalog wtyczek projektu (`.yonsoon/plugins/`)
...
yonsoon uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z yonsoon do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do yonsoon.
...
:::note
Jeśli korzystasz z aplikacji komputerowej yonsoon, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj yonsoon czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do yonsoon:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod yonsoon. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://yonsoon.ai/docs/sdk).
