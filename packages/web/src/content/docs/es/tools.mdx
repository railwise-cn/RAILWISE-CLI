...
Las herramientas permiten que LLM realice acciones en su código base. YONSOON (甬算) viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en YONSOON (甬算).
...
:::note
Esta herramienta solo está disponible cuando `YONSOON_EXPERIMENTAL_LSP_TOOL=true` (o `YONSOON_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor YONSOON (甬算) o cuando la variable de entorno `YONSOON_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar YONSOON (甬算):

```bash
YONSOON_ENABLE_EXA=1 yonsoon
```
