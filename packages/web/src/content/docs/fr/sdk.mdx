---
title: SDK
description: Client JS de type sécurisé pour le serveur yonsoon.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK yonsoon JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler yonsoon par programme.
...
Créez une instance de yonsoon :

```javascript
import { createYonsoon } from "@yonsoon/sdk"

const { client } = await createYonsoon()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `yonsoon.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${yonsoon.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de yonsoon, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@yonsoon/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*yonsoon" },
})
