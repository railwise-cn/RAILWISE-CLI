...
Du kan legge til eksterne verktøy i YONSOON (甬算) ved å bruke _Model Context Protocol_ eller MCP. YONSOON (甬算) støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [YONSOON (甬算) Config](https://yonsoon.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="yonsoon.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/yonsoon`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
YONSOON (甬算) håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil YONSOON (甬算):
...
Hvis serveren krever autentisering, vil YONSOON (甬算) be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `yonsoon mcp auth <server-name>`.
...
Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil YONSOON (甬算) lagre tokens sikkert i `~/.local/share/yonsoon/mcp-auth.json`.
...
MCP-ene dine er tilgjengelige som verktøy i YONSOON (甬算), sammen med innebygde verktøy. Så du kan administrere dem gjennom YONSOON (甬算)-konfigurasjonen som alle andre verktøy.
...
```json title="yonsoon.json" {11, 14-18}
{
  "$schema": "https://yonsoon.ai/config.json",
  "mcp": {
    "my-mcp": {
...
Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble YONSOON (甬算) til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver YONSOON (甬算)-samtale oppført som en separat økt.
