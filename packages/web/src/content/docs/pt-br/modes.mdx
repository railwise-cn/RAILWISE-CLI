...
:::caution
Os modos agora são configurados através da opção `agent` na configuração do yonsoon. A opção `mode` agora está obsoleta. [Saiba mais](/docs/agents).
:::

Os modos no yonsoon permitem que você personalize o comportamento, as ferramentas e os prompts para diferentes casos de uso.

Ele vem com dois modos integrados: **build** e **plan**. Você pode personalizar esses ou configurar os seus próprios através da configuração do yonsoon.
...
O yonsoon vem com dois modos integrados.
...
- `edit` - Não pode modificar arquivos existentes, exceto para arquivos localizados em `.yonsoon/plans/*.md` para detalhar o plano em si
...
Configure os modos no seu arquivo de configuração `yonsoon.json`:
...
- Global: `~/.config/yonsoon/modes/`
- Projeto: `.yonsoon/modes/`

```markdown title="~/.config/yonsoon/modes/review.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.1
tools:
...
Se nenhuma temperatura for especificada, o yonsoon usa padrões específicos do modelo (geralmente 0 para a maioria dos modelos, 0.55 para modelos Qwen).
...
Este caminho é relativo a onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do yonsoon quanto para a configuração específica do projeto.
...
Crie arquivos de modo em `.yonsoon/modes/` para modos específicos do projeto ou `~/.config/yonsoon/modes/` para modos globais:

```markdown title=".yonsoon/modes/debug.md"
---
temperature: 0.1
tools:
  bash: true
...
```markdown title="~/.config/yonsoon/modes/refactor.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.2
tools:
