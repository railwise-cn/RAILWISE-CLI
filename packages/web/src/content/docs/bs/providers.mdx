---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u RAILWISE.
---

import config from "../../../../config.mjs"
export const console = config.console

RAILWISE koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj RAILWISE konfiguraciji.
...
u `~/.local/share/railwise/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem RAILWISE
config.
...
RAILWISE Zen je lista modela koje je obezbedio RAILWISE tim koji su bili
testirano i potvrđeno da dobro radi sa RAILWISE. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa RAILWISE Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite railwise i idite na [railwise.ai/auth](https://railwise.ai/auth).
...
Radi kao i svaki drugi provajder u RAILWISE i potpuno je opcionalan za korištenje.

---

## RAILWISE Go

RAILWISE Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža RAILWISE tim i koji su testirani i verificirani da dobro rade s RAILWISE-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `RAILWISE Go` i idite na [railwise.ai/auth](https://railwise.ai/zen).
...
Da biste koristili Amazon Bedrock s RAILWISE:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete railwise:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `railwise.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u RAILWISE.
:::
...
Možete konfigurirati railwise za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi railwise ispravno radio.
   :::
...
Također možete dodati modele kroz svoju railwise konfiguraciju.
...
da pohrani token u railwise auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
RAILWISE koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate RAILWISE
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`railwise.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [railwise-gitlab-auth](https://www.npmjs.com/package/railwise-gitlab-auth).

##### Konfiguracija

Prilagodite putem `railwise.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s railwise:
...
Za korištenje Google Vertex AI s RAILWISE:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće railwise.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem railwisea, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [railwise-helicone-session](https://github.com/H2Shami/railwise-helicone-session) da automatski prijavite svaki RAILWISE razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki RAILWISE razgovor naveden kao zasebna sesija.
...
Možete konfigurirati railwise za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati railwise za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati railwise za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za RAILWISE. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/railwise) za detalje.
:::
...
Da biste koristili Ollama Cloud s RAILWISE:
...
3. Kopirajte API ključ za korištenje u RAILWISE.
...
6. **Važno**: Prije upotrebe modela oblaka u RAILWISE, morate lokalno povući informacije o modelu:
...
RAILWISE Zen je lista testiranih i verifikovanih modela koju je obezbedio RAILWISE tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>RAILWISE Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **RAILWISE Zen**.

```txt
   /connect
```

3. Unesite svoj RAILWISE API ključ.
...
Također možete dodati dodatne modele putem vaše railwise konfiguracije.
...
5. Također ih možete prilagoditi putem vaše railwise konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju railwise konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s railwise-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `railwise.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju RAILWISE da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `railwise auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite RAILWISE konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj railwise konfiguraciji.
