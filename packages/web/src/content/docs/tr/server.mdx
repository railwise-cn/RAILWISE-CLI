---
title: Sunucu
description: yonsoon sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`yonsoon serve` komutu, yonsoon istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `yonsoon.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `YONSOON_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `yonsoon` değeridir; değiştirmek isterseniz `YONSOON_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `yonsoon serve` hem de `yonsoon web` için geçerlidir.
...
`yonsoon` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
yonsoon ile programatik etkileşim için yonsoon sunucusunu kullanın.
:::

Bu mimari, yonsoon'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`yonsoon serve` ile bağımsız bir sunucu başlatabilirsiniz. yonsoon TUI açıksa, `yonsoon serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum yonsoon [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
yonsoon sunucusu aşağıdaki API'leri sunar.
