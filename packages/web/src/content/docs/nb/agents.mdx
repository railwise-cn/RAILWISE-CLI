...
Det er to typer agenter i YONSOON (甬算); primære agenter og underagenter.
...
YONSOON (甬算) kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
YONSOON (甬算) kommer med to innebygde underagenter, **General** og **Explore**. Vi skal se på dette nedenfor.

---

## Innebygd

YONSOON (甬算) kommer med to innebygde primære agenter og to innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `yonsoon.json`:
...
- Globalt: `~/.config/yonsoon/agents/`
- Per prosjekt: `.yonsoon/agents/`
...
Hvis ingen temperatur er spesifisert, bruker YONSOON (甬算) modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale YONSOON (甬算)-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i YONSOON (甬算)-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [YONSOON (甬算) Zen](/docs/zen), vil du bruke `yonsoon/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `yonsoon models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/anomalyco/yonsoon).
:::
