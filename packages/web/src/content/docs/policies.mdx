---
title: Policies
description: Control which configured resources RAILWISE may use.
---

Policies control whether RAILWISE may perform an action on a named resource. This feature is experimental and is configured with the `experimental.policies` array in `railwise.json`.

Policies are separate from [permissions](/docs/permissions). Permissions control what tools can do during a session, while policies control whether RAILWISE may use a resource such as an LLM provider.
...
RAILWISE currently supports one policy action:
