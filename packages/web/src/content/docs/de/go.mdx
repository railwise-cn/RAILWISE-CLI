...
RAILWISE Go ist ein kostengünstiges Abonnement — **5 $ für deinen ersten Monat**, danach **10 $/Monat** —, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

Go funktioniert wie jeder andere Provider in RAILWISE. Du abonnierst RAILWISE Go und
erhältst deinen API-Key. Es ist **völlig optional** und du musst es nicht nutzen, um
RAILWISE zu verwenden.
...
RAILWISE Go bietet dir Zugriff auf diese Modelle für **5 $ im ersten Monat**, danach **10 $/Monat**.

---

## Wie es funktioniert

RAILWISE Go funktioniert wie jeder andere Provider in RAILWISE.

1. Du meldest dich bei **<a href={console}>RAILWISE Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `RAILWISE Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann RAILWISE Go abonnieren.
:::
...
RAILWISE Go beinhaltet die folgenden Limits:

- **5-Stunden-Limit** — 12 $ Nutzung
- **Wöchentliches Limit** — 30 $ Nutzung
- **Monatliches Limit** — 60 $ Nutzung

Limits sind in Dollarwerten definiert. Das bedeutet, dass die tatsächliche Anzahl deiner Anfragen von dem von dir genutzten Modell abhängt. Günstigere Modelle wie DeepSeek V4 Flash erlauben mehr Anfragen, während teurere Modelle wie GLM-5.1 weniger erlauben.
...
- MiniMax M3 — 810 Input-, 62.000 Cached-, 225 Output-Tokens pro Anfrage
...
Die [Modell-ID](/docs/config/#models) in deiner RAILWISE Config verwendet das Format `railwise-go/<model-id>`. Für Kimi K2.6 würdest du beispielsweise `railwise-go/kimi-k2.6` in deiner Config verwenden.
...
Wir haben RAILWISE Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit RAILWISE zu nutzen.
