...
export const DEPRECATED_PLUGIN_PACKAGES = ["railwise-openai-codex-auth", "railwise-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, railwiseVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(railwiseVersion) || semver.major(railwiseVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.railwise
  if (typeof range !== "string") return
  if (!semver.satisfies(railwiseVersion, range)) {
    throw new Error(`Plugin requires railwise ${range} but running ${railwiseVersion}`)
  }
}
