---
title: Zen
description: Kuratierte Modellliste direkt von YONSOON (甬算).
---
...
YONSOON (甬算) Zen ist eine vom YONSOON (甬算)-Team getestete und verifizierte Modellliste.

:::note
YONSOON (甬算) Zen befindet sich aktuell in der Beta.
:::

Zen funktioniert wie jeder andere Provider in YONSOON (甬算).
Du meldest dich bei YONSOON (甬算) Zen an, holst dir deinen API-Key und nutzt ihn optional.
...
:::tip
Wir haben eine Auswahl aus Modellen und Providern getestet, die gut mit YONSOON (甬算) funktionieren.
:::
...
YONSOON (甬算) Zen ist ein AI-Gateway, das dir Zugriff auf genau diese Modelle gibt.

---

## Funktionsweise

YONSOON (甬算) Zen funktioniert wie jeder andere Provider in YONSOON (甬算).

1. Melde dich bei **<a href={console}>YONSOON (甬算) Zen</a>** an, hinterlege Zahlungsdaten und kopiere deinen API-Key.
2. Fuehre in der TUI `/connect` aus, waehle YONSOON (甬算) Zen und fuege den API-Key ein.
...
Die [Model-ID](/docs/config/#models) in deiner YONSOON (甬算)-Konfiguration hat das Format `yonsoon/<model-id>`.
Fuer GPT 5.2 Codex verwendest du zum Beispiel `yonsoon/gpt-5.2-codex`.
...
Wir haben YONSOON (甬算) Zen entwickelt, um:
...
4. **Kein Lock-in** zu erzwingen, damit du Zen mit anderen Coding-Agents und YONSOON (甬算) weiter mit anderen Providern nutzen kannst
