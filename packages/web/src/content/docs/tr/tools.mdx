...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. yonsoon yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
yonsoon içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `YONSOON_EXPERIMENTAL_LSP_TOOL=true` (veya `YONSOON_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca yonsoon provider kullanılırken veya `YONSOON_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

yonsoon başlatırken etkinleştirmek için:
