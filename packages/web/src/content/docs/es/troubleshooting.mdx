...
Para depurar problemas con RAILWISE (甬算), comience verificando los registros y los datos locales que almacena en el disco.
...
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\railwise\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `railwise --log-level DEBUG`.

---

## Almacenamiento

railwise almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/railwise/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\railwise`
...
RAILWISE (甬算) Desktop ejecuta un servidor RAILWISE (甬算) local (el sidecar `railwise-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `RAILWISE (甬算)` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/railwise/railwise.jsonc` (o `~/.config/railwise/railwise.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/railwise/railwise.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\railwise\railwise.jsonc`
...
RAILWISE (甬算) también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/railwise/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\railwise\plugins`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que RAILWISE (甬算) pueda reconstruirlo.

1. Salga de RAILWISE (甬算) Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/railwise`
- **Linux**: eliminar `~/.cache/railwise` (o ejecutar `rm -rf ~/.cache/railwise`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\railwise`

3. Reinicie el escritorio RAILWISE (甬算).

---

### Solucionar problemas de conexión del servidor

RAILWISE (甬算) Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `railwise.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `RAILWISE_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `RAILWISE_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio RAILWISE (甬算) requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de RAILWISE (甬算).

---

### Notificaciones que no se muestran

RAILWISE (甬算) Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para RAILWISE (甬算) en la configuración de su sistema operativo, y
...
1. Salga del escritorio RAILWISE (甬算).
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio RAILWISE (甬算)):

- `railwise.settings.dat` (URL del servidor predeterminado de escritorio)
- `railwise.global.dat` y `railwise.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
Si tiene problemas con RAILWISE (甬算):
...
3. Asegúrese de tener la última versión con `railwise upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `railwise models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\railwise`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. railwise instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\railwise`

2. Reinicie railwise para reinstalar los paquetes del proveedor más recientes.

Esto obligará a railwise a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
railwise detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
