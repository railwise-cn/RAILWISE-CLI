---
title: Provider
description: Usa qualsiasi provider LLM in RAILWISE (甬算).
---

import config from "../../../../config.mjs"
export const console = config.console

RAILWISE (甬算) usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di RAILWISE (甬算).
...
RAILWISE (甬算) Zen e una lista di modelli forniti dal team RAILWISE (甬算), testati e
verificati per funzionare bene con RAILWISE (甬算). [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da RAILWISE (甬算) Zen.
:::
...
Funziona come qualsiasi altro provider in RAILWISE (甬算) ed e totalmente opzionale.
...
Per usare Amazon Bedrock con RAILWISE (甬算):
...
:::info
L'uso dell'abbonamento Claude Pro/Max in RAILWISE (甬算) non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
RAILWISE (甬算) usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare RAILWISE (甬算)
...
Per usare Google Vertex AI con RAILWISE (甬算):
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [railwise-helicone-session](https://github.com/H2Shami/railwise-helicone-session) per loggare automaticamente ogni conversazione di RAILWISE (甬算) come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di RAILWISE (甬算) elencata come sessione separata.
...
:::tip
Ollama può auto-configurarsi per RAILWISE (甬算). Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/railwise) per dettagli.
:::
...
Per usare Ollama Cloud con RAILWISE (甬算):
...
3. Copia la chiave API da usare in RAILWISE (甬算).
...
6. **Importante**: Prima di usare modelli cloud in RAILWISE (甬算), devi scaricare le informazioni del modello localmente:
...
RAILWISE (甬算) Zen è una lista di modelli testati e verificati forniti dal team RAILWISE (甬算). [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>RAILWISE (甬算) Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **RAILWISE (甬算) Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di RAILWISE (甬算).
...
I campi `limit` permettono a RAILWISE (甬算) di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
