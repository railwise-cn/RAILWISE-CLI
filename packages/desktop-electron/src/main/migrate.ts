...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.railwise.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `railwise.settings.dat` is special: it maps to the `railwise.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"railwise.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // railwise.settings.dat → the electron settings store ("railwise.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "railwise.global.dat").
  const storeName = filename === "railwise.settings.dat" ? "railwise.settings" : filename
