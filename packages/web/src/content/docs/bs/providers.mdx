---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u RAILWISE (甬算).
---

import config from "../../../../config.mjs"
export const console = config.console

RAILWISE (甬算) koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj RAILWISE (甬算) konfiguraciji.
...
Možete prilagoditi dobavljače putem odjeljka `provider` u vašem RAILWISE (甬算)
config.
...
RAILWISE (甬算) Zen je lista modela koje je obezbedio RAILWISE (甬算) tim koji su bili
testirano i potvrđeno da dobro radi sa RAILWISE (甬算). [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa RAILWISE (甬算) Zen.
:::
...
Radi kao i svaki drugi provajder u RAILWISE (甬算) i potpuno je opcionalan za korištenje.
...
Da biste koristili Amazon Bedrock s RAILWISE (甬算):
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u RAILWISE (甬算).
:::
...
RAILWISE (甬算) koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate RAILWISE (甬算)
...
Za korištenje Google Vertex AI s RAILWISE (甬算):
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [railwise-helicone-session](https://github.com/H2Shami/railwise-helicone-session) da automatski prijavite svaki RAILWISE (甬算) razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki RAILWISE (甬算) razgovor naveden kao zasebna sesija.
...
:::tip
Ollama se može automatski konfigurirati za RAILWISE (甬算). Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/railwise) za detalje.
:::
...
Da biste koristili Ollama Cloud s RAILWISE (甬算):
...
3. Kopirajte API ključ za korištenje u RAILWISE (甬算).
...
6. **Važno**: Prije upotrebe modela oblaka u RAILWISE (甬算), morate lokalno povući informacije o modelu:
...
RAILWISE (甬算) Zen je lista testiranih i verifikovanih modela koju je obezbedio RAILWISE (甬算) tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>RAILWISE (甬算) Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **RAILWISE (甬算) Zen**.

```txt
   /connect
```

3. Unesite svoj RAILWISE (甬算) API ključ.
...
Polja `limit` omogućavaju RAILWISE (甬算) da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
2. Za prilagođene provajdere, provjerite RAILWISE (甬算) konfiguraciju i:
