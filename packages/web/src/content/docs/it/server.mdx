---
title: Server
description: Interagisci con il server yonsoon via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `yonsoon serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client yonsoon.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `yonsoon.local` |
...
Imposta `YONSOON_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `yonsoon`, oppure imposta `YONSOON_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `yonsoon serve` sia per `yonsoon web`.
...
Quando esegui `yonsoon` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server yonsoon per interagire con yonsoon in modo programmatico.
:::

Questa architettura permette a yonsoon di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `yonsoon serve` per avviare un server standalone. Se la TUI di yonsoon e gia in esecuzione, `yonsoon serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di YONSOON (甬算).
...
Il server yonsoon espone le seguenti API.
