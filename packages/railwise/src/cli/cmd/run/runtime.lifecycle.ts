...
export async function createRuntimeLifecycle(input: LifecycleInput): Promise<Lifecycle> {
  return withRunSpan(
    "RunLifecycle.boot",
    {
      "railwise.agent.name": input.agent,
      "railwise.directory": input.directory,
      "railwise.first": input.first,
      "railwise.model.provider": input.model?.providerID,
      "railwise.model.id": input.model?.modelID,
      "railwise.model.variant": input.variant,
      ...
      try {
        ...
        const close = async (next: {
          ...
          return withRunSpan(
            "RunLifecycle.close",
            {
              "railwise.show_exit": next.showExit,
