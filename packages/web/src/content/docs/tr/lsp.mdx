...
description: yonsoon, LSP sunucularınızla bütünleşir.
---

yonsoon, LLM'in kod tabanınızla etkileşime girmesine yardımcı olmak için Dil Sunucusu Protokolünüzle (LSP) bütünleşir. LLM'ye geri bildirim sağlamak için tanılamayı kullanır.

---

## Yerleşik

yonsoon, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`YONSOON_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
LSP sunucularını, yonsoon yapılandırmanızdaki `lsp` bölümü aracılığıyla özelleştirebilirsiniz.
