---
title: Serveur
description: Interagissez avec le serveur yonsoon via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `yonsoon serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client yonsoon peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `yonsoon.local` |
...
Définissez `YONSOON_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `yonsoon`, ou définissez `YONSOON_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `yonsoon serve` et à `yonsoon web`.
...
Lorsque vous exécutez `yonsoon`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur yonsoon pour interagir avec yonsoon par programmation.
:::

Cette architecture permet à yonsoon de prendre en charge plusieurs clients et vous permet d'interagir avec yonsoon par programme.

Vous pouvez exécuter `yonsoon serve` pour démarrer un serveur autonome. Si vous avez le
yonsoon TUI en cours d'exécution, `yonsoon serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins YONSOON (甬算) [IDE](/docs/ide).
...
Le serveur yonsoon expose les API suivantes.
