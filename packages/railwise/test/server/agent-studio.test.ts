    ...
    try {
      ...
      await Instance.provide({
        ...
        fn: async () => {
          ...
          expect(result.prompt).toContain("1. QA 质检员 (@qa_inspector)")
          expect(result.prompt).toContain("2. 数据分析师 (@data_analyst)")
          expect(result.prompt).toContain("3. 技术写作 (@technical_writer)")
          expect(result.prompt).toContain("4. QA 复核 (@qa_reviewer)")
          expect(result.prompt).toContain("5. 总指挥 (@chief_manager)")
          expect(result.prompt).toContain("n1 -> n2: 平差后观测数据")
          expect(result.prompt).toContain("n2 -> n3: 统计分析结果")
