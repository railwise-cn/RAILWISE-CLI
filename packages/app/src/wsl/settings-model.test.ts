...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when RAILWISE needs attention", () => {
    ...
    ).toBe("Install RAILWISE")
    expect(
      wslOpencodeAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/railwise",
        version: "1.2.2",
        expectedVersion: "1.2.3",
        matchesDesktop: false,
        error: null,
      }),
    ).toBe("Update RAILWISE")
    expect(
      wslOpencodeAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/railwise",
  ...
  test("probes the selected distro before entering the RAILWISE step", async () => {
    const calls: string[] = []
    ...
    expect(calls).toEqual(["Debian", "railwise"])
  })
})
