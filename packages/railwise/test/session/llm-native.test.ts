...
import { LLMEvent, ToolFailure } from "@railwise/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor, type LLMClientShape } from "@railwise/llm/route"
...
describe("session.llm-native.request", () => {
  ...
  test("selects native request routes for provider packages", () => {
    ...
    const compatible = LLMNative.model({
      model: {
        ...baseModel,
        providerID: ProviderV2.ID.make("railwise"),
  ...
  test("only enables native runtime for supported OpenAI API-key models", () => {
    ...
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("railwise") },
        provider: { ...providerInfo, id: ProviderV2.ID.make("railwise") },
        auth: undefined,
      }),
    ).toMatchObject({
      type: "supported",
    ...
    ).toEqual({ type: "unsupported", reason: "provider is not openai, railwise, or anthropic" })
    expect(
      LLMNativeRuntime.status({
        model: baseModel,
  ...
  test("prefers console provider api key over stored railwise auth", () => {
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("railwise") },
        provider: {
          ...providerInfo,
          id: ProviderV2.ID.make("railwise"),
