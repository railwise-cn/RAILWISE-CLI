---
title: Serwer
description: Interakcja z serwerem yonsoon za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `yonsoon serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient yonsoon.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `yonsoon.local` |
...
Ustaw `YONSOON_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `yonsoon` lub ustaw `YONSOON_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `yonsoon serve`, jak i `yonsoon web`.
...
Kiedy uruchomisz `yonsoon`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera yonsoon do programowej interakcji z yonsoon.
:::

Ta architektura umożliwia obsługę wielu klientów przez yonsoon i programową interakcję z kodem otwartym.

Możesz uruchomić `yonsoon serve`, aby uruchomić samodzielny serwer. Jeśli masz
yonsoon TUI działa, `yonsoon serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki yonsoon [IDE](/docs/ide).
...
Serwer yonsoon udostępnia następujące interfejsy API.
