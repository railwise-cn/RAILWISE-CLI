---
title: Konfigurasjon
description: Ved å bruke yonsoon JSON-konfigurasjonen.
---

Du kan konfigurere YONSOON (甬算) ved å bruke en JSON konfigurasjonsfil.

---

## Format

YONSOON (甬算) støtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
For eksempel, hvis din globale konfigurasjon setter `theme: "yonsoon"` og `autoupdate: true`, og prosjektkonfigurasjonen setter `model: "anthropic/claude-sonnet-4-5"`, vil den endelige konfigurasjonen inkludere alle tre innstillingene.
...
1. **Ekstern konfigurasjon** (fra `.well-known/yonsoon`) - organisasjonsstandarder
2. **Global konfigurasjon** (`~/.config/yonsoon/yonsoon.json`) - brukerinnstillinger
3. **Egendefinert konfigurasjon** (`YONSOON_CONFIG` env var) - egendefinerte overstyringer
4. **Prosjektkonfigurasjon** (`yonsoon.json` i prosjekt) - prosjektspesifikke innstillinger
5. **`.yonsoon` kataloger** - agenter, kommandoer, plugins
6. **Inline config** (`YONSOON_CONFIG_CONTENT` env var) - kjøretidsoverstyringer
...
:::note
`.yonsoon` og `~/.config/yonsoon` katalogene bruker **flertallsnavn** på underkataloger: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkeltnavn (f.eks. `agent/`) støttes også for bakoverkompatibilitet.
:::
...
Organisasjoner kan tilby standardkonfigurasjon via `.well-known/yonsoon`-endepunktet. Dette hentes automatisk når du autentiserer med en leverandør som støtter det.
...
Plasser din globale YONSOON (甬算)-konfigurasjon i `~/.config/yonsoon/yonsoon.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Legg til `yonsoon.json` i prosjektroten din. Prosjektkonfigurasjon har den høyeste prioritet blant standard config-filer - den overstyrer både globale og eksterne konfigurasjoner.

:::tip
Plasser prosjektspesifikk konfigurasjon i roten til prosjektet ditt.
:::

Når YONSOON (甬算) starter opp, ser den etter en konfigurasjonsfil i gjeldende katalog eller går opp til nærmeste Git-katalog.
...
Spesifiser en tilpasset konfigurasjonsfilbane ved å bruke miljøvariabelen `YONSOON_CONFIG`.
...
Spesifiser en tilpasset konfigurasjonskatalog ved å bruke `YONSOON_CONFIG_DIR`
miljøvariabel. Denne katalogen vil bli søkt etter agenter, kommandoer,
moduser og plugins akkurat som standard `.yonsoon` katalog, og bør
...
Den egendefinerte katalogen lastes inn etter den globale konfigurasjonen og `.yonsoon`-katalogene, slik at den **kan overstyre** innstillingene deres.

---

## Skjema

Konfigurasjonsfilen har et skjema som er definert i [**`yonsoon.ai/config.json`**](https://yonsoon.ai/config.json).
...
Du kan konfigurere serverinnstillinger for kommandoene `yonsoon serve` og `yonsoon web` gjennom alternativet `server`.
...
- `mdns` - Aktiver mDNS-tjenesteoppdagelse. Dette lar andre enheter på nettverket oppdage YONSOON (甬算)-serveren din.
- `mdnsDomain` - Egendefinert domenenavn for mDNS-tjeneste. Standard er `yonsoon.local`. Nyttig for å kjøre flere forekomster på samme nettverk.
...
Du kan konfigurere leverandørene og modellene du vil bruke i YONSOON (甬算)-konfigurasjonen gjennom alternativene `provider`, `model` og `small_model`.
...
Alternativet `small_model` konfigurerer en egen modell for lette oppgaver som tittelgenerering. Som standard prøver YONSOON (甬算) å bruke en rimeligere modell hvis en er tilgjengelig fra leverandøren din, ellers faller den tilbake til hovedmodellen din.
...
Du kan konfigurere temaet du vil bruke i YONSOON (甬算)-konfigurasjonen gjennom alternativet `theme`.
...
Du kan også definere agenter ved å bruke markdown-filer i `~/.config/yonsoon/agents/` eller `.yonsoon/agents/`. [Les mer her](/docs/agents).
...
Standardagenten må være en primæragent (ikke en underagent). Dette kan være en innebygd agent som `"build"` eller `"plan"`, eller en [egendefinert agent](/docs/agents) du har definert. Hvis den angitte agenten ikke eksisterer eller er en subagent, vil YONSOON (甬算) falle tilbake til `"build"` med en advarsel.

Denne innstillingen gjelder for alle grensesnitt: TUI, CLI (`yonsoon run`), skrivebordsapp og GitHub Action.
...
Du kan også definere kommandoer ved å bruke markdown-filer i `~/.config/yonsoon/commands/` eller `.yonsoon/commands/`. [Les mer her](/docs/commands).
...
YONSOON (甬算) vil automatisk laste ned eventuelle nye oppdateringer når den starter opp. Du kan deaktivere dette med alternativet `autoupdate`.
...
Som standard **tillater YONSOON (甬算) alle operasjoner** uten å kreve eksplisitt godkjenning. Du kan endre dette ved å bruke alternativet `permission`.
...
[Plugins](/docs/plugins) utvider YONSOON (甬算) med tilpassede verktøy, kroker og integrasjoner.

Plasser plugin-filer i `.yonsoon/plugins/` eller `~/.config/yonsoon/plugins/`. Du kan også laste inn plugins fra npm gjennom alternativet `plugin`.
...
Dette er nyttig når du vil begrense YONSOON (甬算) til kun å bruke spesifikke leverandører i stedet for å deaktivere dem én etter én.
