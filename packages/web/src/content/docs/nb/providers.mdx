---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i RAILWISE (甬算).
---

import config from "../../../../config.mjs"
export const console = config.console

RAILWISE (甬算) bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i RAILWISE (甬算)-konfigurasjonen.
...
Du kan tilpasse leverandørene gjennom `provider`-delen i RAILWISE (甬算)
konfig.
...
RAILWISE (甬算) Zen er en liste over modeller levert av RAILWISE (甬算)-teamet som har vært
testet og verifisert for å fungere godt med RAILWISE (甬算). [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med RAILWISE (甬算) Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg railwise og gå til [railwise.ai/auth](https://railwise.ai/auth).
...
Det fungerer som alle andre leverandører i RAILWISE (甬算) og er helt valgfritt å bruke.
...
Slik bruker du Amazon Bedrock med RAILWISE (甬算):
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører RAILWISE (甬算):
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at RAILWISE (甬算) skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom RAILWISE (甬算)-konfigurasjonen.
...
for å lagre token i RAILWISE (甬算) auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
RAILWISE (甬算) bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse RAILWISE (甬算)
...
Slik bruker du GitHub Copilot-abonnementet med RAILWISE (甬算):
...
Slik bruker du Google Vertex AI med RAILWISE (甬算):
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører RAILWISE (甬算).
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom RAILWISE (甬算), kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [railwise-helicone-session](https://github.com/H2Shami/railwise-helicone-session) plugin for automatisk å logge hver RAILWISE (甬算)-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver RAILWISE (甬算)-samtale oppført som en separat økt.
...
Du kan konfigurere RAILWISE (甬算) for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere RAILWISE (甬算) for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere RAILWISE (甬算) for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for RAILWISE (甬算). Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/railwise) for detaljer.
:::
...
Slik bruker du Ollama Cloud med RAILWISE (甬算):
...
3. Kopier API-nøkkelen for bruk i RAILWISE (甬算).
...
6. **Viktig**: Før du bruker skymodeller i RAILWISE (甬算), må du hente modellinformasjonen lokalt:
...
RAILWISE (甬算) Zen er en liste over testede og verifiserte modeller levert av RAILWISE (甬算)-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>RAILWISE (甬算) Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **RAILWISE (甬算) Zen**.

   ```txt
   /connect
   ```

3. Skriv inn RAILWISE (甬算) API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom RAILWISE (甬算)-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom RAILWISE (甬算)-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med RAILWISE (甬算):
...
Du kan også tilpasse modeller gjennom RAILWISE (甬算)-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med RAILWISE (甬算). De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
`limit`-feltene lar RAILWISE (甬算) forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
2. For tilpassede leverandører, sjekk RAILWISE (甬算)-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i RAILWISE (甬算)-konfigurasjonen.
