...
import type { AssistantMessage, Event, YonsoonClient, SessionMessageResponse } from "@yonsoon/sdk/v2"
...
export namespace ACP {
  ...
  export async function init({ sdk: _sdk }: { sdk: YonsoonClient }) {
    return {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `yonsoon auth login` in the terminal",
        name: "Login with yonsoon",
        id: "yonsoon-login",
      }
  ...
  async function defaultModel(config: ACPConfig, cwd?: string) {
    ...
    const yonsoonProvider = providers.find((p) => p.id === "yonsoon")
    if (yonsoonProvider) {
      if (yonsoonProvider.models["big-pickle"]) {
        return { providerID: "yonsoon", modelID: "big-pickle" }
      }
      const [best] = Provider.sort(Object.values(yonsoonProvider.models))
      if (best) {
        return {
