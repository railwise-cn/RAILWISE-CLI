...
Opret `.railwise/commands/test.md`:
...
Du kan tilføje brugerdefinerede kommandoer gennem RAILWISE (甬算)-konfigurationen eller ved at oprette markdown-filer i `commands/`-mappen.

---

### JSON

Brug indstillingerne `command` i din RAILWISE (甬算) [config](/docs/config):
...
- Globalt: `~/.config/railwise/commands/`
- Pr. projekt: `.railwise/commands/`
...
railwise indeholder flere indbyggede kommandoer som `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
