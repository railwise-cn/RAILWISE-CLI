...
W RAILWISE (甬算) obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
RAILWISE (甬算) zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
RAILWISE (甬算) ma dwóch wbudowanych subagentów: **General** i **Explore**. Przyjrzymy się im poniżej.

---

## Wbudowane

RAILWISE (甬算) ma dwa wbudowane agenty główne i dwa wbudowane subagenty.
...
Jeśli nie zostanie skonfigurowana, RAILWISE (甬算) używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji RAILWISE (甬算), jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji RAILWISE (甬算) w `provider/model-id`. Na przykład, [RAILWISE (甬算) Zen](/docs/zen), użyj `railwise/gpt-5.1-codex` dla GPT 5.1 Codex.
