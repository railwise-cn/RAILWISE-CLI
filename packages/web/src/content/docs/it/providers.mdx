---
title: Provider
description: Usa qualsiasi provider LLM in YONSOON (甬算).
---

import config from "../../../../config.mjs"
export const console = config.console

YONSOON (甬算) usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di YONSOON (甬算).
...
YONSOON (甬算) Zen e una lista di modelli forniti dal team YONSOON (甬算), testati e
verificati per funzionare bene con YONSOON (甬算). [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da YONSOON (甬算) Zen.
:::

1. Esegui `/connect` nella TUI, seleziona yonsoon e vai su [yonsoon.ai/auth](https://yonsoon.ai/auth).
...
Funziona come qualsiasi altro provider in YONSOON (甬算) ed e totalmente opzionale.
...
Per usare Amazon Bedrock con YONSOON (甬算):
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   Imposta una di queste variabili d'ambiente mentre esegui yonsoon:
   ...
   Per configurazione specifica del progetto o persistente, usa `yonsoon.json`:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in YONSOON (甬算) non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
2. Vai su [Azure AI Foundry](https://ai.azure.com/) e fai il deploy di un modello.
   ...
   :::note
   Il nome del deployment deve corrispondere al nome del modello affinché yonsoon funzioni correttamente.
   :::
...
5. Esegui il comando `/models` per selezionare un modello.
   ...
   Puoi anche aggiungere modelli tramite la tua configurazione di yonsoon.
...
salvare il token nello storage di auth di yonsoon.
:::

##### GitLab Self-Hosted

:::note[nota di compliance]
YONSOON (甬算) usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare YONSOON (甬算)
a usare solo la tua istanza GitLab self-hosted, aggiungi quanto segue al tuo
file `yonsoon.json`. Si raccomanda anche di disabilitare la condivisione sessioni.
...
Personalizza tramite `yonsoon.json`:
...
Per usare il tuo abbonamento GitHub Copilot con yonsoon:
...
Per usare Google Vertex AI con YONSOON (甬算):
...
2. Imposta le variabili d'ambiente richieste:
   ...
   Impostale mentre esegui yonsoon.
...
Nel caso vedessi una funzionalità o un modello di Helicone che non viene configurato automaticamente da yonsoon, puoi sempre configurarlo tu stesso.
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [yonsoon-helicone-session](https://github.com/H2Shami/yonsoon-helicone-session) per loggare automaticamente ogni conversazione di YONSOON (甬算) come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di YONSOON (甬算) elencata come sessione separata.
...
Puoi configurare yonsoon per usare modelli locali tramite l'utility llama-server di [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puoi configurare yonsoon per usare modelli locali tramite LM Studio.
...
Puoi configurare yonsoon per usare modelli locali tramite Ollama.

:::tip
Ollama può auto-configurarsi per YONSOON (甬算). Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/yonsoon) per dettagli.
:::
...
Per usare Ollama Cloud con YONSOON (甬算):
...
3. Copia la chiave API da usare in YONSOON (甬算).
...
6. **Importante**: Prima di usare modelli cloud in YONSOON (甬算), devi scaricare le informazioni del modello localmente:
...
YONSOON (甬算) Zen è una lista di modelli testati e verificati forniti dal team YONSOON (甬算). [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>YONSOON (甬算) Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **YONSOON (甬算) Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di YONSOON (甬算).
...
4. Molti modelli OpenRouter sono precaricati di default, esegui il comando `/models` per selezionare quello che vuoi.
   ...
   Puoi anche aggiungere modelli addizionali tramite la tua configurazione di yonsoon.
...
5. Puoi anche personalizzarli tramite la configurazione di yonsoon. Ecco un esempio di specifica di un provider
...
Per usare le [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) con Yonsoon:
...
Puoi anche personalizzare i modelli tramite la tua configurazione di yonsoon. Ecco un esempio di come specificare l'ordine di routing dei provider.
...
:::tip
Puoi usare qualsiasi provider OpenAI-compatible con yonsoon. La maggior parte dei provider AI moderni offre API OpenAI-compatible.
:::
...
4. Crea o aggiorna il tuo file `yonsoon.json` nella directory del progetto:
...
I campi `limit` permettono a YONSOON (甬算) di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
...
1. **Controlla il setup auth**: Esegui `yonsoon auth list` per vedere se le credenziali
...
2. Per provider personalizzati, controlla la config di yonsoon e:
   - Assicurati che l'ID provider usato nel comando `/connect` corrisponda all'ID nella tua config yonsoon.
