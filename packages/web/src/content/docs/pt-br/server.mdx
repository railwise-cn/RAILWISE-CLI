---
title: Servidor
description: Interaja com o servidor yonsoon via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `yonsoon serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente yonsoon pode usar.
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `yonsoon.local` |
...
Defina `YONSOON_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `yonsoon`, ou defina `YONSOON_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `yonsoon serve` quanto ao `yonsoon web`.

```bash
YONSOON_SERVER_PASSWORD=your-password yonsoon serve
```
...
Quando você executa `yonsoon`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor yonsoon para interagir com o yonsoon programaticamente.
:::

Essa arquitetura permite que o yonsoon suporte múltiplos clientes e permite que você interaja com o yonsoon programaticamente.

Você pode executar `yonsoon serve` para iniciar um servidor autônomo. Se você tiver o TUI do yonsoon em execução, `yonsoon serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do yonsoon [IDE](/docs/ide).
...
O servidor yonsoon expõe as seguintes APIs.
