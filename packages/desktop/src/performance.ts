...
export class StartupTimer {
  ...
  constructor(
    budgets: StartupBudgets = DEFAULT_BUDGETS,
    onBudgetExceeded?: (phase: string, phase_data: StartupPhase) => Promise<boolean>,
  ) {
  ...
  async endPhase(name: string): Promise<{ duration: number; budgetExceeded: boolean; shouldRetry: boolean }> {
    ...
    if (budgetExceeded && budget) {
      console.warn(
        `⚠️ ${name}: ${duration.toFixed(2)}ms (exceeded ${budget}ms budget by ${(duration - budget).toFixed(2)}ms)`,
      )
    } else {
      console.log(
        `✅ ${name}: ${duration.toFixed(2)}ms${budget ? ` (${budget - duration >= 0 ? "within" : "exceeded"} ${budget}ms budget)` : ""}`,
      )
    }
