---
title: Plugin
description: Scrivi plugin per estendere RAILWISE (甬算).
---

I plugin ti permettono di estendere RAILWISE (甬算) agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalita', integrare servizi esterni o modificare il comportamento predefinito di RAILWISE (甬算).
...
- `.railwise/plugins/` - plugin a livello progetto
- `~/.config/railwise/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/railwise/node_modules/`.
...
1. Config globale (`~/.config/railwise/railwise.json`)
2. Config di progetto (`railwise.json`)
3. Directory plugin globale (`~/.config/railwise/plugins/`)
4. Directory plugin di progetto (`.railwise/plugins/`)
...
RAILWISE (甬算) esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di railwise per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere railwise.
...
:::note
Se usi l'app desktop di RAILWISE (甬算), puo' inviare automaticamente notifiche di sistema quando una risposta e' pronta o quando una sessione va in errore.
:::
...
Impedisci a railwise di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a railwise:
...
L'helper `tool` crea uno strumento personalizzato che railwise puo' chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in railwise insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://railwise.ai/docs/sdk) per i dettagli.
