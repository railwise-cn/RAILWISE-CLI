...
For at fejlfinde problemer med YONSOON (甬算), start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\yonsoon\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `yonsoon --log-level DEBUG`.

---

## Lagring

yonsoon gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/yonsoon/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\yonsoon`
...
YONSOON (甬算) Desktop kører en lokal YONSOON (甬算)-server (`yonsoon-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `YONSOON (甬算)`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/yonsoon/yonsoon.jsonc` (eller `~/.config/yonsoon/yonsoon.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/yonsoon/yonsoon.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\yonsoon\yonsoon.jsonc`
...
YONSOON (甬算) kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/yonsoon/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\yonsoon\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så YONSOON (甬算) kan genopbygge den.

1. Afslut YONSOON (甬算) Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/yonsoon`
- **Linux**: slet `~/.cache/yonsoon` (eller kør `rm -rf ~/.cache/yonsoon`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\yonsoon`

3. Start YONSOON (甬算) Desktop på ny.

---

### Løs problemer med serverforbindelse

YONSOON (甬算) Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `yonsoon.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `YONSOON_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `YONSOON_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver YONSOON (甬算) Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i YONSOON (甬算).

---

### Meddelelser vises ikke

YONSOON (甬算) Desktop viser kun systemvarsler når:

- varsler er aktiveret for YONSOON (甬算) i dine OS-indstillinger, og
...
1. Afslut YONSOON (甬算) Desktop.
2. Find og slet disse filer (de findes i YONSOON (甬算) Desktop-appens datamappe):

- `yonsoon.settings.dat` (skrivebordsstandardserver URL)
- `yonsoon.global.dat` og `yonsoon.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med YONSOON (甬算):
...
For at finde ud af hvilke modeller du har adgang til, kør `yonsoon models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\yonsoon`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. yonsoon installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\yonsoon`

2. Start yonsoon på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge yonsoon til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
yonsoon vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
