...
YONSOON (甬算) a une liste de raccourcis clavier que vous pouvez personnaliser via la configuration YONSOON (甬算).
...
YONSOON (甬算) utilise une touche `leader` pour la plupart des raccourcis clavier. Cela évite les conflits dans votre terminal.
...
Le champ de saisie de l'application de bureau YONSOON (甬算) prend en charge les raccourcis courants de style Readline/Emacs pour l'édition de texte. Ceux-ci sont intégrés et actuellement non configurables via `yonsoon.json`.
