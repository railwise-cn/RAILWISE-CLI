---
title: Konfiguracija
description: Korištenje RAILWISE JSON konfiguracije.
---

Možete konfigurirati RAILWISE koristeći JSON konfiguracijski fajl.

---

## Format

RAILWISE podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/railwise`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/railwise/railwise.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`RAILWISE_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`railwise.json` u projektu) - postavke specifične za projekat
5. **`.railwise` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`RAILWISE_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.railwise` i `~/.config/railwise` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/railwise` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu RAILWISE konfiguraciju u `~/.config/railwise/railwise.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/railwise/tui.json`.
...
Dodajte `railwise.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se RAILWISE pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `RAILWISE_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `RAILWISE_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.railwise` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.railwise`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`railwise.ai/config.json`**](https://railwise.ai/config.json).

TUI konfiguracija koristi [**`railwise.ai/tui.json`**](https://railwise.ai/tui.json).
...
Koristite `RAILWISE_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `railwise.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `railwise serve` i `railwise web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš RAILWISE server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `railwise.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj RAILWISE konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, RAILWISE pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/railwise/agents/` ili `.railwise/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, RAILWISE će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`railwise run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/railwise/commands/` ili `.railwise/commands/`. [Saznajte više ovdje](/docs/commands).
...
RAILWISE će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, RAILWISE **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju RAILWISE sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.railwise/plugins/` ili `~/.config/railwise/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite RAILWISE da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
