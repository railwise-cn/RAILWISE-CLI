---
title: Config
description: Using the RAILWISE JSON config.
---

You can configure RAILWISE using a JSON config file.

---

## Format

RAILWISE supports both **JSON** and **JSONC** (JSON with Comments) formats.

```jsonc title="railwise.jsonc"
{
...
1. **Remote config** (from `.well-known/railwise`) - organizational defaults
2. **Global config** (`~/.config/railwise/railwise.json`) - user preferences
3. **Custom config** (`RAILWISE_CONFIG` env var) - custom overrides
4. **Project config** (`railwise.json` in project) - project-specific settings
5. **`.railwise` directories** - agents, commands, plugins
6. **Inline config** (`RAILWISE_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/railwise/` on macOS) - admin-controlled
...
:::note
The `.railwise` and `~/.config/railwise` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/railwise` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
```json title="Remote config from .well-known/railwise"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Place your global RAILWISE config in `~/.config/railwise/railwise.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/railwise/tui.json`.
...
Add `railwise.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When RAILWISE starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
Specify a custom config file path using the `RAILWISE_CONFIG` environment variable.

```bash
export RAILWISE_CONFIG=/path/to/my/custom-config.json
railwise run "Hello world"
```
...
Specify a custom config directory using the `RAILWISE_CONFIG_DIR`
environment variable. This directory will be searched for agents, commands,
modes, and plugins just like the standard `.railwise` directory, and should
follow the same structure.

```bash
export RAILWISE_CONFIG_DIR=/path/to/my/config-directory
railwise run "Hello world"
```

The custom directory is loaded after the global config and `.railwise` directories, so it **can override** their settings.
...
Drop an `railwise.json` or `railwise.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/railwise/` |
| Linux    | `/etc/railwise/`                         |
| Windows  | `%ProgramData%\railwise`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, RAILWISE reads managed preferences from the `ai.railwise.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

RAILWISE checks these paths:

1. `/Library/Managed Preferences/<user>/ai.railwise.managed.plist`
2. `/Library/Managed Preferences/ai.railwise.managed.plist`

The plist keys map directly to `railwise.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.railwise.managed` PayloadType. The RAILWISE config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.railwise.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.railwise.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.railwise</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
The server/runtime config schema is defined in [**`railwise.ai/config.json`**](https://railwise.ai/config.json).

TUI config uses [**`railwise.ai/tui.json`**](https://railwise.ai/tui.json).
...
Use `RAILWISE_TUI_CONFIG` to point to a custom TUI config file.

Set `attention.enabled` to turn on TUI desktop notifications and sounds. See [TUI attention](/docs/tui#attention).

Legacy `theme`, `keybinds`, and `tui` keys in `railwise.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `railwise serve` and `railwise web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your RAILWISE server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `railwise.local`. Useful for running multiple instances on the same network.
...
If not specified, RAILWISE will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your RAILWISE config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, RAILWISE tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Use the `experimental.policies` option to allow or deny RAILWISE actions on configured resources. Currently, policies can control which providers RAILWISE may use.
...
RAILWISE normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, RAILWISE omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
You can also define agents using markdown files in `~/.config/railwise/agents/` or `.railwise/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, RAILWISE will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`railwise run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/railwise/commands/` or `.railwise/commands/`. [Learn more here](/docs/commands).
...
RAILWISE uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
RAILWISE will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, railwise **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend RAILWISE with custom tools, hooks, and integrations.

Place plugin files in `.railwise/plugins/` or `~/.config/railwise/plugins/`. You can also load plugins from npm through the `plugin` option.

```json title="railwise.json"
{
  "$schema": "https://railwise.ai/config.json",
  "plugin": ["railwise-helicone-session", "@my-org/custom-plugin"]
}
```
...
This is useful when you want to restrict RAILWISE to only use specific providers rather than disabling them one by one.
