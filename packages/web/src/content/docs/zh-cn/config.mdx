...
2. **全局配置**（`~/.config/opencode/railwise.json`）- 用户偏好
...
将全局 OpenCode 配置放在 `~/.config/opencode/railwise.json` 中。使用全局配置来设置用户级别的偏好，例如主题、提供商或快捷键。
...
使用 `RAILWISE_CONFIG_DIR` 环境变量指定自定义配置目录。该目录会像标准 `.opencode` 目录一样被搜索代理、命令、模式和插件，并且应遵循相同的结构。
...
您还可以使用 `~/.config/opencode/agents/` 或 `.railwise/agents/` 中的 Markdown 文件定义代理。[在此了解更多](/docs/agents)。
...
您还可以使用 `~/.config/opencode/commands/` 或 `.railwise/commands/` 中的 Markdown 文件定义命令。[在此了解更多](/docs/commands)。
...
将插件文件放置在 `.railwise/plugins/` 或 `~/.config/opencode/plugins/` 中。您还可以通过 `plugin` 选项从 npm 加载插件。
