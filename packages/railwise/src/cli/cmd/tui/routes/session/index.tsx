...
import type { AssistantMessage, Part, ToolPart, UserMessage, TextPart, ReasoningPart } from "@railwise/sdk/v2"
...
export function Session() {
  ...
  sdk.event.on("message.part.updated", (evt) => {
    ...
    if (part.tool === "plan_exit") {
      local.agent.set(local.agent.list()[0]?.name ?? "build")
      lastSwitch = part.id
  ...
  createEffect(() => {
    ...
    return exit.message.set(
      [
        ...
        `  ${weak("Continue")}${UI.Style.TEXT_NORMAL_BOLD}railwise -s ${session()?.id}${UI.Style.TEXT_NORMAL}`,
        ``,
      ].join("\n"),
    )
  })
  ...
  return (
    <Show when={props.part.text.trim()}>
      <box id={"text-" + props.part.id} paddingLeft={3} marginTop={1} flexShrink={0}>
        <Switch>
          <Match when={Flag.RAILWISE_EXPERIMENTAL_MARKDOWN}>
            <markdown
              syntaxStyle={syntax()}
              streaming={true}
