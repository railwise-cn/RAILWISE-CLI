...
yonsoon'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
yonsoon, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
yonsoon, **General** ve **Explore** olmak üzere iki yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

yonsoon iki yerleşik birincil agent ve iki yerleşik alt agent ile birlikte gelir.
...
Agent'ları `yonsoon.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse yonsoon modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global yonsoon yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
yonsoon hesabınızdaki model kimliğini `provider/model-id` biçimini kullanır. Örneğin, [YONSOON (甬算) Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `yonsoon/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `yonsoon models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/anomalyco/yonsoon).
:::
