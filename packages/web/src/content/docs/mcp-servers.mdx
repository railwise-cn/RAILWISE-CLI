...
You can add external tools to RAILWISE using the _Model Context Protocol_, or MCP. RAILWISE supports both local and remote servers.
...
You can define MCP servers in your [RAILWISE Config](https://railwise.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
RAILWISE automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, RAILWISE will:
...
If the server requires authentication, RAILWISE will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `railwise mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, RAILWISE will store the tokens securely in `~/.local/share/railwise/mcp-auth.json`.
...
Your MCPs are available as tools in RAILWISE, alongside built-in tools. So you can manage them through the RAILWISE config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect RAILWISE to your Sentry account.
