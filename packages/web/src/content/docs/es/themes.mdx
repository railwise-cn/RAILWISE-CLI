...
Con YONSOON (甬算) puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, YONSOON (甬算) usa nuestro propio tema `yonsoon`.
...
YONSOON (甬算) viene con varios temas integrados.
...
- Quiere que YONSOON (甬算) coincida con la apariencia de su terminal
...
YONSOON (甬算) admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/yonsoon/themes/*.json` o `$XDG_CONFIG_HOME/yonsoon/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.yonsoon/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.yonsoon/themes/*.json`
