---
title: Configuration
description: Utilisation de la configuration JSON YONSOON (甬算).
---

Vous pouvez configurer YONSOON (甬算) à l'aide d'un fichier de configuration JSON.

---

## Format

YONSOON (甬算) prend en charge les formats **JSON** et **JSONC** (JSON avec commentaires).

```jsonc title="yonsoon.jsonc"
{
...
Par exemple, si votre configuration globale définit `theme: "yonsoon"` et `autoupdate: true` et que la configuration de votre projet définit `model: "anthropic/claude-sonnet-4-5"`, la configuration finale inclura les trois paramètres.
...
1. **Configuration distante** (à partir de `.well-known/yonsoon`) - paramètres par défaut de l'organisation
2. **Configuration globale** (`~/.config/yonsoon/yonsoon.json`) - préférences utilisateur
3. **Configuration personnalisée** (`YONSOON_CONFIG` env var) - remplacements personnalisés
4. **Configuration du projet** (`yonsoon.json` dans le projet) - paramètres spécifiques au projet
5. **Répertoires `.yonsoon`** - agents, commandes, plugins
6. **Configuration en ligne** (`YONSOON_CONFIG_CONTENT` env var) - remplacements d'exécution
...
:::note
Les répertoires `.yonsoon` et `~/.config/yonsoon` utilisent des **noms au pluriel** pour les sous-répertoires : `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` et `themes/`. Les noms singuliers (par exemple, `agent/`) sont également pris en charge pour une compatibilité ascendante.
:::
...
Les organisations peuvent fournir une configuration par défaut via le point de terminaison `.well-known/yonsoon`. Ceci est récupéré automatiquement lorsque vous vous authentifiez auprès d’un fournisseur qui le prend en charge.
...
```json title="Remote config from .well-known/yonsoon"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Placez votre configuration globale YONSOON (甬算) dans `~/.config/yonsoon/yonsoon.json`. Utilisez la configuration globale pour les préférences de l'utilisateur telles que les thèmes, les fournisseurs ou les raccourcis clavier.
...
Ajoutez `yonsoon.json` à la racine de votre projet. La configuration du projet a la priorité la plus élevée parmi les fichiers de configuration standard : elle remplace les configurations globales et distantes.

:::tip
Placez la configuration spécifique au projet à la racine de votre projet.
:::

Lorsque YONSOON (甬算) démarre, il recherche un fichier de configuration dans le répertoire actuel ou remonte jusqu'au répertoire Git le plus proche.
...
Spécifiez un chemin de fichier de configuration personnalisé à l'aide de la variable d'environnement `YONSOON_CONFIG`.

```bash
export YONSOON_CONFIG=/path/to/my/custom-config.json
yonsoon run "Hello world"
```
...
Spécifiez un répertoire de configuration personnalisé à l'aide de `YONSOON_CONFIG_DIR` variable d'environnement. Ce répertoire sera recherché pour les agents, les commandes, modes et plugins tout comme le répertoire standard `.yonsoon`, et devrait suivre la même structure.

```bash
export YONSOON_CONFIG_DIR=/path/to/my/config-directory
yonsoon run "Hello world"
```

Le répertoire personnalisé est chargé après les répertoires de configuration globale et `.yonsoon`, il **peut donc remplacer** leurs paramètres.

---

## Schéma

Le fichier de configuration a un schéma défini dans [**`yonsoon.ai/config.json`**](https://yonsoon.ai/config.json).
...
Vous pouvez configurer les paramètres du serveur pour les commandes `yonsoon serve` et `yonsoon web` via l'option `server`.
...
- `mdns` - Activer la découverte du service mDNS. Cela permet à d'autres appareils du réseau de découvrir votre serveur YONSOON (甬算).
- `mdnsDomain` - Nom de domaine personnalisé pour le service mDNS. La valeur par défaut est `yonsoon.local`. Utile pour exécuter plusieurs instances sur le même réseau.
...
Vous pouvez configurer les fournisseurs et les modèles que vous souhaitez utiliser dans votre configuration YONSOON (甬算) via les options `provider`, `model` et `small_model`.
...
L'option `small_model` configure un modèle distinct pour les tâches légères comme la génération de titres. Par défaut, YONSOON (甬算) essaie d'utiliser un modèle moins cher s'il est disponible auprès de votre fournisseur, sinon il revient à votre modèle principal.
...
Vous pouvez configurer le thème que vous souhaitez utiliser dans votre configuration YONSOON (甬算) via l'option `theme`.
...
Vous pouvez également définir des agents à l'aide de fichiers markdown dans `~/.config/yonsoon/agents/` ou `.yonsoon/agents/`. [En savoir plus ici](/docs/agents).
...
L'agent par défaut doit être un agent principal (et non un sous-agent). Il peut s'agir d'un agent intégré tel que `"build"` ou `"plan"`, ou d'un [agent personnalisé](/docs/agents) que vous avez défini. Si l'agent spécifié n'existe pas ou est un sous-agent, YONSOON (甬算) reviendra à `"build"` avec un avertissement.

Ce paramètre s'applique à toutes les interfaces : TUI, CLI (`yonsoon run`), application de bureau et GitHub Action.
...
Vous pouvez également définir des commandes à l'aide de fichiers markdown dans `~/.config/yonsoon/commands/` ou `.yonsoon/commands/`. [En savoir plus ici](/docs/commands).
...
YONSOON (甬算) téléchargera automatiquement toutes les nouvelles mises à jour au démarrage. Vous pouvez désactiver cela avec l'option `autoupdate`.
...
Par défaut, yonsoon **autorise toutes les opérations** sans nécessiter d'approbation explicite. Vous pouvez modifier cela en utilisant l'option `permission`.
...
[Plugins](/docs/plugins) étendent YONSOON (甬算) avec des outils, des hooks et des intégrations personnalisés.

Placez les fichiers du plugin dans `.yonsoon/plugins/` ou `~/.config/yonsoon/plugins/`. Vous pouvez également charger des plugins depuis npm via l'option `plugin`.

```json title="yonsoon.json"
{
  "$schema": "https://yonsoon.ai/config.json",
  "plugin": ["yonsoon-helicone-session", "@my-org/custom-plugin"]
}
```
...
Ceci est utile lorsque vous souhaitez restreindre YONSOON (甬算) à l'utilisation de fournisseurs spécifiques plutôt que de les désactiver un par un.
